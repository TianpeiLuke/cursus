...
class BedrockBatchProcessor(BedrockProcessor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Initialized batch processor - mode: {self.batch_mode}, threshold: {self.batch_threshold}")
        if self.batch_input_s3_path:
    ...
    def should_use_batch_processing(self, df: pd.DataFrame) -> bool:
        ...
        if self.batch_mode == 'realtime':
            ...
            return (len(df) >= self.batch_threshold and 
                   ...
                   self.input_bucket is not None and
                   self.output_bucket is not None)
    ...
    def convert_df_to_jsonl(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Converting {len(df)} records to JSONL format for batch processing")
    ...
    def convert_batch_results_to_df(
        ...
        logger.info(f"Converting {len(batch_results)} batch results back to DataFrame format")
        ...
        logger.info(f"Converted batch results to DataFrame with {len(result_df)} records")
...
if __name__ == "__main__":
    try:
        ...
        environ_vars = {
            ...
            "BEDROCK_BATCH_INPUT_S3_PATH": os.environ.get("BEDROCK_BATCH_INPUT_S3_PATH"),
            "BEDROCK_BATCH_OUTPUT_S3_PATH": os.environ.get("BEDROCK_BATCH_OUTPUT_S3_PATH"),
