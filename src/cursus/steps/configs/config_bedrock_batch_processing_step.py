...
class BedrockBatchProcessingConfig(ProcessingStepConfigBase):
    ...
    def bedrock_environment_variables(self) -> Dict[str, str]:
        ...
        if self._bedrock_environment_variables is None:
            self._bedrock_environment_variables = {
                ...
                'BEDROCK_INFERENCE_PROFILE_ARN': self.bedrock_inference_profile_arn or '',
    ...
    def batch_configuration(self) -> Dict[str, Any]:
        ...
        if self._batch_configuration is None:
            self._batch_configuration = {
                ...
                'cost_optimization_enabled': self.bedrock_batch_mode in ['auto', 'batch'],
    ...
    def cost_optimization_info(self) -> Dict[str, Any]:
        ...
        if self._cost_optimization_info is None:
            ...
            self._cost_optimization_info = {
                ...
                'cost_efficient_for_large_datasets': self.bedrock_batch_mode in ['auto', 'batch'],
    ...
    def validate_batch_mode(cls, v: str) -> str:
        ...
        if v not in valid_modes:
            raise ValueError(f"bedrock_batch_mode must be one of {valid_modes}, got: {v}")
        return v
    ...
    def validate_batch_role_arn(cls, v: str) -> str:
        ...
        if not v.startswith("arn:aws:iam::"):
            raise ValueError(f"bedrock_batch_role_arn must be a valid IAM role ARN, got: {v}")
    ...
    def get_public_init_fields(self) -> Dict[str, Any]:
        ...
        if self.bedrock_inference_profile_arn is not None:
            bedrock_fields["bedrock_inference_profile_arn"] = self.bedrock_inference_profile_arn
    ...
    def is_production_ready(self) -> bool:
        ...
        return (
            ...
            bool(self.bedrock_batch_role_arn) and
    ...
    def get_performance_estimate(self) -> Dict[str, Any]:
        ...
        if self.bedrock_concurrency_mode == "sequential":
            ...
            realtime_speedup = min(self.bedrock_max_concurrent_workers, self.bedrock_rate_limit_per_second / 2)
            realtime_throughput = f"~{int(realtime_speedup * 60 // self.bedrock_batch_size)} batches/min"
        ...
        if self.bedrock_batch_mode == 'batch':
            ...
            processing_mode = f"Auto selection (batch for >= {self.bedrock_batch_threshold} records)"
    ...
    def get_batch_processing_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "environment_variables": {
                k: v for k, v in self.bedrock_environment_variables.items() 
                if k.startswith('BEDROCK_BATCH_')
            },
            "recommendations": {
                "optimal_for_datasets": f">= {self.bedrock_batch_threshold} records" if self.bedrock_batch_mode == 'auto' else "All datasets",
