...
This module implements the configuration class for SageMaker XgboostMt Training steps
...
class XgboostMtTrainingConfig(BasePipelineConfig):
    """
    Configuration specific to the SageMaker XgboostMt Training Step.

    Uses custom XgboostMt Docker image for multi-task learning with:
    ...
    Hyperparameters are managed separately via XGBoostMtModelHyperparameters
    ...
    training_entry_point: str = Field(
        description="Entry point script for XgboostMt multi-task training."
    )
    ...
    training_instance_type: str = Field(
        ...
        description="Instance type for XgboostMt training (CPU-optimized for XGBoost).",
    )
    ...
    # Framework versions for SageMaker PyTorch container (used for XgboostMt)
    framework_version: str = Field(
        default="2.1.2",
        description="SageMaker PyTorch framework version for XgboostMt container.",
    )
    ...
    model_class: str = Field(
        default="xgboost_mt",
        description="Model class identifier for multi-task XGBoost.",
    )
    ...
    use_native_categorical: bool = Field(
        default=True,
        description="Controls categorical feature handling mode for XgboostMt. "
        "If True (default), uses XGBoost native categorical features with DictionaryEncodingProcessor for integer encoding. "
    ...
    def initialize_derived_fields(self) -> "XgboostMtTrainingConfig":
    ...
    @classmethod
    def validate_xgboost_mt_instance_type(cls, v: str) -> str:
        """
        Validate instance types suitable for XgboostMt.

        XGBoost works efficiently on CPU instances, especially for multi-task learning
        where memory and compute balance is important.
        """
        # CPU-optimized instances for XGBoost
        valid_cpu_instances = [
        ...
        if v not in valid_cpu_instances:
            raise ValueError(
                f"Invalid instance type for XgboostMt: {v}. "
                f"XGBoost requires CPU instances. "
    ...
    def get_public_init_fields(self) -> Dict[str, Any]:
        """
        Override get_public_init_fields to include XgboostMt training-specific fields.
        Gets a dictionary of public fields suitable for initializing a child config.
        Includes both base fields (from parent) and XgboostMt training-specific fields.
        ...
        # Add XgboostMt training-specific fields (Tier 1 and Tier 2)
        training_fields = {
