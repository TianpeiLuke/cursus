...
class CategoryDefinition(BaseModel):
    ...
    aliases: Optional[List[str]] = Field(
        default=None,
        description="Optional list of alternative names for this category"
    )
    ...
    @classmethod
    def examples_must_not_be_empty_strings(cls, v: Optional[List[str]]) -> Optional[List[str]]:
...
class CategoryDefinitionList(BaseModel):
    ...
    @classmethod
    def categories_must_have_unique_names(cls, v: List[CategoryDefinition]) -> List[CategoryDefinition]:
        ...
        for i, category in enumerate(v):
            if category.name in names:
                raise ValueError(f'Duplicate category name: "{category.name}" at index {i}')
...
class BedrockPromptTemplateGenerationConfig(ProcessingStepConfigBase):
    ...
    category_definitions: Optional[List[CategoryDefinition]] = Field(
        default=None,
        description="List of category definitions for template generation"
    )
    ...
    def resolved_prompt_configs_path(self) -> Optional[str]:
        ...
        if self._resolved_prompt_configs_path is None:
            ...
            if effective_source.startswith("s3://"):
                self._resolved_prompt_configs_path = f"{effective_source.rstrip('/')}/{self.prompt_configs_path}"
            else:
                self._resolved_prompt_configs_path = str(Path(effective_source) / self.prompt_configs_path)
    ...
    def initialize_derived_fields(self) -> "BedrockPromptTemplateGenerationConfig":
        ...
        self._auto_generate_config_bundle()
        
        return self

    def _auto_generate_config_bundle(self) -> None:
        """
        Automatically generate prompt configuration bundle if conditions are met.
        
        This method is called during initialization to ensure the configuration bundle
        is created when the config class is constructed, after all derived fields are ready.
        """
        try:
            ...
            if categories:
                ...
                logger.warning("No category definitions available - skipping auto-generation of config bundle")
        except Exception as e:
            ...
            logger.info("You can manually call generate_prompt_config_bundle() after providing category definitions")
...
def load_categories_from_json(json_data: str) -> CategoryDefinitionList:
    ...
    if isinstance(data, list):
        ...
        return CategoryDefinitionList(categories=[CategoryDefinition(**cat) for cat in data])
    elif isinstance(data, dict):
        # Single category dictionary
        return CategoryDefinitionList(categories=[CategoryDefinition(**data)])
    else:
        raise ValueError("JSON data must be a list of categories or a single category dictionary")
...
def load_categories_from_dict(data: Any) -> CategoryDefinitionList:
    ...
    if isinstance(data, list):
        ...
        raise ValueError("Data must be a list of categories or a single category dictionary")
