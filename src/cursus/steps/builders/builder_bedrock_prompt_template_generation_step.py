...
class BedrockPromptTemplateGenerationStepBuilder(StepBuilderBase):
    ...
    def _get_job_arguments(self) -> Optional[List[str]]:
        ...
        The script uses command-line arguments to override environment variables for specific flags.
        Since all configuration is already passed via environment variables, we only need to pass
        command-line arguments when they differ from the environment variable defaults.
        ...
        # The script uses command-line arguments to override environment variables
        # Since we're already passing all config via environment variables,
        # we only need command-line args if they provide different behavior
