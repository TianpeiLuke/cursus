...
from ..configs.config_tokenizer_training_step import TokenizerTrainingConfig
...
class TokenizerTrainingStepBuilder(StepBuilderBase):
    """
    Builder for a Tokenizer Training ProcessingStep.

    This implementation uses the specification-driven approach where dependencies, outputs,
    and script contract are defined in the tokenizer training specification.
    Uses FrameworkProcessor with PyTorch container to support source_dir parameter for
    BPE tokenizer training with compression tuning.
    """
    ...
    def __init__(
        ...
        """
        Initializes the builder with a specific configuration for the tokenizer training step.

        Args:
            config: A TokenizerTrainingConfig instance containing all necessary settings.
        ...
        self.config: TokenizerTrainingConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        self.log_info("Validating TokenizerTrainingConfig...")
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [
                ...
                raise ValueError(
                    f"TokenizerTrainingConfig missing required attribute: {attr}"
                )
        ...
        self.log_info("TokenizerTrainingConfig validation succeeded.")
    ...
    def _create_processor(self) -> FrameworkProcessor:
        ...
        This defines the execution environment for the tokenizer training script, including
        the instance type, framework version, and environment variables.

        Uses FrameworkProcessor with PyTorch estimator to support source_dir parameter
        while providing a Python environment suitable for tokenizer training.
    ...
    def _get_environment_variables(self) -> Dict[str, str]:
        ...
        This method delegates to the config's environment_variables property,
        which handles all tokenizer training-specific variables (TEXT_FIELD,
        TARGET_COMPRESSION, MIN_FREQUENCY, MAX_VOCAB_SIZE).

        Returns:
            Dict[str, str]: Environment variables dictionary
        """
        # Delegate to config's environment_variables property
        if hasattr(self.config, "environment_variables"):
            env_vars = self.config.environment_variables
        else:
            # Fallback to parent implementation if config doesn't have the property
    ...
    def _get_outputs(self, outputs: Dict[str, Any]) -> List[ProcessingOutput]:
        ...
        for _, output_spec in self.spec.outputs.items():
            ...
            if logical_name in outputs:
                ...
                destination = Join(
                    on="/",
                    values=[base_output_path, "tokenizer_training", logical_name],
                )
                self.log_info(
    ...
    def _get_job_arguments(self) -> List[str]:
        ...
        tokenizer training jobs to use different job_type values based on their configuration.
        ...
        # For tokenizer training, we always return just the job_type argument
        # This is because tokenizer_training_contract has empty expected_arguments
    ...
    def create_step(self, **kwargs) -> ProcessingStep:
        ...
        """
        self.log_info("Creating TokenizerTraining ProcessingStep...")
        ...
        # Use FrameworkProcessor with source_dir parameter for tokenizer training
