...
class ConfigClassMapper:
    ...
    def _initialize_registry(self):
        ...
        try:
            ...
            logger.info(
                f"Registry system initialized successfully with {len(self.config_classes)} config classes"
            )
    ...
    def map_dag_to_config_classes(self, dag) -> Dict[str, Type[BaseModel]]:
        ...
        for node_name in nodes:
            ...
            if config_class:
                ...
                logger.warning(
                    f"No configuration class found for DAG node: {node_name}"
                )
    ...
    def register_manual_mapping(
        self, node_name: str, config_class: Type[BaseModel]
    ) -> None:
        """
        ...
        logger.info(
            f"Manually registered mapping: {node_name} -> {config_class.__name__}"
        )

    def validate_mapping(
        self, config_class_map: Dict[str, Type[BaseModel]]
    ) -> Dict[str, str]:
        """
        ...
        for node_name, config_class in config_class_map.items():
            try:
                ...
                if not issubclass(config_class, BaseModel):
                    validation_errors[node_name] = (
                        f"Class {config_class.__name__} is not a Pydantic model"
                    )
