...
def build_lstm2risk_collate_fn(
    pad_token: int,
    ...
    Args:
        ...
        input_ids_key: Key name for text input IDs in batch dict. Configurable to
                      support different tokenizer outputs (default: "input_ids").
    ...
    Batch Output Format:
        {
            input_ids_key: (B, L) - Padded text token IDs, sorted by length descending
    ...
    Example:
        >>> collate = build_lstm2risk_collate_fn(pad_token=0, input_ids_key="input_ids")
        >>> batch = [
        ...     {"input_ids": torch.tensor([1,2,3]), "tabular": torch.randn(10), "label": torch.tensor(1)},
        ...     {"input_ids": torch.tensor([4,5]), "tabular": torch.randn(10), "label": torch.tensor(0)},
        ... ]
        >>> output = collate(batch)
        >>> output["input_ids"].shape  # (2, 3) - padded to longest (3 tokens)
        torch.Size([2, 3])
    ...
    Implementation Notes:
        ...
        - Configurable key name allows flexibility with different tokenizers
    """
    ...
    def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            batch: List of sample dictionaries with keys: input_ids_key, tabular, label

        Returns:
            Batched dictionary with sorted, padded sequences and lengths
        """
        # Extract batch components using configurable key
        texts = [item[input_ids_key] for item in batch]
...
def build_transformer2risk_collate_fn(
    pad_token: int,
    block_size: int,
    input_ids_key: str = "input_ids",
    ...
    Args:
        ...
        input_ids_key: Key name for text input IDs in batch dict. Configurable to
                      support different tokenizer outputs (default: "input_ids").
        attention_mask_key: Key name for attention mask in batch dict. Configurable to
                           support different tokenizer outputs (default: "attention_mask").
    ...
    Batch Output Format:
        {
            input_ids_key: (B, L) - Padded text token IDs (no sorting, may be truncated)
            "tabular": (B, F) - Stacked tabular features
            "label": (B,) - Stacked labels
            attention_mask_key: (B, L) - Attention mask (True = valid token, False = padding)
        }
    ...
    Example:
        >>> collate = build_transformer2risk_collate_fn(
        ...     pad_token=0, block_size=100,
        ...
        >>> output["input_ids"].shape  # (2, 3) - padded to longest (3 tokens)
        torch.Size([2, 3])
        >>> output["attention_mask"]  # [[True, True, True], [True, True, False]]
    ...
    Implementation Notes:
        ...
        - Configurable key names allow flexibility with different tokenizers
    """
    ...
    def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        """
        # Extract batch components and truncate text to block_size using configurable key
        texts = [item[input_ids_key][:block_size] for item in batch]
        tabs = [item["tabular"] for item in batch]
        labels = [item["label"] for item in batch]
