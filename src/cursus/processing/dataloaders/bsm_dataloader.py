    ...
    def collate_batch(batch):
        ...
        for key in batch[0]:
            ...
            if all(
                ...
                for item in batch:
                    input_chunks = [
                        torch.tensor(chunk[primary_input_ids_key], dtype=torch.long)
                        for chunk in item[key]
                    ]
                    mask_chunks = [
                        torch.tensor(
                            chunk[primary_attention_mask_key], dtype=torch.long
                        )
                        for chunk in item[key]
                    ]
                ...
                output[key + "_" + primary_input_ids_key] = pad_nested(all_input_ids)
                output[key + "_" + primary_attention_mask_key] = pad_nested(
                    all_attention_masks
                )
            ...
            # Check if this is a text field (secondary modality)
                ...
                for item in batch:
                    input_chunks = [
                        torch.tensor(chunk[secondary_input_ids_key], dtype=torch.long)
                        for chunk in item[key]
                    ]
                    mask_chunks = [
                        torch.tensor(
                            chunk[secondary_attention_mask_key], dtype=torch.long
                        )
                        for chunk in item[key]
                    ]
                    all_input_ids.append(pad_sequence(input_chunks, batch_first=True))
                ...
                output[key + "_" + secondary_input_ids_key] = pad_nested(all_input_ids)
                output[key + "_" + secondary_attention_mask_key] = pad_nested(
                    all_attention_masks
                )
