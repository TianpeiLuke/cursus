...
class UniversalStepBuilderTest:
    ...
    def __init__(
        ...
        if self.verbose:
            print(
                f"🔧 Initialized Refactored UniversalStepBuilderTest with {workspace_count} workspace directories"
            )
    ...
    def run_validation_for_step(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self._run_comprehensive_validation_for_step(
            ...
            if self.enable_scoring and SCORING_AVAILABLE:
                try:
                    ...
                    if self.verbose:
                        overall_score = scoring_results.get("overall", {}).get(
                            "score", 0.0
                        )
                        overall_rating = scoring_results.get("overall", {}).get(
                            "rating", "Unknown"
                        )
                        print(
                            f"📊 Quality Score: {overall_score:.1f}/100 - {overall_rating}"
                        )
    ...
    def _run_comprehensive_validation_for_step(
    ...
    def _run_fallback_core_validation(
        self, step_name: str, builder_class: Type
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            results["inheritance_check"] = {
                ...
                "error": None
                if issubclass(builder_class, StepBuilderBase)
                else "Builder does not inherit from StepBuilderBase",
            }
            ...
            for method in required_methods:
                method_results[method] = {
                    ...
                    else f"Missing required method: {method}",
                }
    ...
    def _test_integration_capabilities(
        self, step_name: str, builder_class: Type
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            all_passed = all(
                check.get("passed", False) for check in integration_checks.values()
            )
            ...
            return {"status": "ERROR", "error": f"Integration testing failed: {str(e)}"}
    ...
    def _check_config_class_exists(self, builder_class: Type) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"passed": False, "error": f"Config class check failed: {str(e)}"}
    ...
    def _check_config_import(self, builder_class: Type) -> Dict[str, Any]:
        ...
        try:
            ...
            if init_method:
                ...
                return {"passed": False, "error": "No __init__ method found"}
        except Exception as e:
            return {"passed": False, "error": f"Config import check failed: {str(e)}"}
    ...
    def _test_step_creation_capability(
        self, step_name: str, builder_class: Type
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            field_requirements = self._check_field_requirements(
                step_name, builder_class
            )

            # Determine overall capability status
            all_checks_passed = (
                config_availability.get("available", False)
                and method_availability.get("has_required_methods", False)
                and field_requirements.get("requirements_identifiable", False)
            )
            ...
            if self.step_catalog_available:
                ...
                if config_class_name in config_classes:
                    ...
                    return {
                        ...
                        "available_configs": list(config_classes.keys())[
                            :5
    ...
    def _check_required_methods(self, builder_class: Type) -> Dict[str, Any]:
        ...
        try:
            ...
            for method in required_methods:
                if hasattr(builder_class, method) and callable(
                    getattr(builder_class, method)
                ):
    ...
    def _discover_all_steps(self) -> List[str]:
        ...
        if not all_steps:
            try:
                ...
                if self.verbose:
                    print(
                        f"✅ Discovered {len(registry_steps)} steps from registry (fallback)"
                    )
    ...
    def _calculate_basic_scoring(
        ...
        passed_components = sum(
            1 for comp in components.values() if comp.get("status") == "COMPLETED"
        )
        ...
        return {
            "overall": {"score": score, "rating": rating, "status": overall_status},
            "components": {
    ...
    def generate_report(self, step_name: str):
        ...
        try:
            ...
            report = StreamlinedBuilderTestReport(
                step_name, builder_class_name, step_type
            )
    ...
    def _get_builder_class_from_catalog(self, step_name: str) -> Optional[Type]:
        ...
        if self.step_catalog_available:
            try:
                ...
                if self.verbose:
                    print(
                        f"⚠️  Step catalog failed to load builder for {step_name}: {e}"
                    )

        # Fallback: Step catalog should handle everything, but keep minimal fallback
        if self.verbose:
            print(
                f"⚠️  Step catalog could not load builder for {step_name}, no fallback available"
            )
            ...
            if self.step_catalog_available:
                try:
                    ...
                    if config_class_name in config_classes:
                        ...
                        try:
                            config_instance = ConfigClass.model_construct(
                                **minimal_data
                            )
                            if self.verbose:
                                print(
                                    f"✅ Created config for {step_name} using model_construct (bypassing validation)"
                                )
    ...
    def from_builder_class(
        cls, builder_class: Type, workspace_dirs: Optional[List[str]] = None, **kwargs
    ...
    def test_all_builders_by_type(
        ...
        try:
            ...
            for step_name, step_info in STEP_NAMES.items():
                if (
                    step_info.get("sagemaker_step_type") == sagemaker_step_type
                    and step_name not in BASE_CONFIGS
                ):
                    matching_steps.append(step_name)

            if verbose:
                print(
                    f"🔍 Found {len(matching_steps)} concrete steps of type {sagemaker_step_type}"
                )

            # Create tester instance
            tester = cls(
                workspace_dirs=["."], verbose=verbose, enable_scoring=enable_scoring
            )
    ...
    def _report_consolidated_results(self, results: Dict[str, Any]) -> None:
        ...
        if "components" in results:
            ...
            failed_tests = {
                k: v for k, v in results.items() if not v.get("passed", True)
            }
            if failed_tests:
    ...
    def _generate_structured_report(
        ...
        if "components" in raw_results:
            ...
            builder_class = (
                self.builder_class.__name__ if self.builder_class else "Unknown"
            )
    ...
    def validate_specific_script(
        self, step_name: str, skip_levels: Optional[set] = None
    ) -> Dict[str, Any]:
        """
