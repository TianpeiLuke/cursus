...
class StreamlinedStepBuilderScorer:
    ...
    def _score_alignment_validation(
        ...
        overall_status = alignment_results.get(
        ...
        if failed_tests:
            ...
            error_count = sum(
                1 for issue in failed_tests if issue.get("severity") == "ERROR"
            )
            warning_count = sum(
                1 for issue in failed_tests if issue.get("severity") == "WARNING"
            )
    ...
    def _score_integration_testing(
        self, data: Dict[str, Any]
    ) -> Tuple[float, Dict[str, Any]]:
    ...
    def _score_step_creation(
        ...
        if status == "COMPLETED" and capability_validated:
            ...
            score = (
                85.0  # Completed but capability not explicitly validated (higher score)
            )
        elif status == "ERROR":
            # Be less harsh for configuration errors - these are often fixable issues
            if any(
                keyword in error_message.lower()
                for keyword in ["config", "field required", "validation error"]
            ):
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "validation_results": {
                ...
                "validation_type": self.validation_results.get(
...
def score_builder_results(
    ...
    validation_results = {
        ...
        if any(r.get("passed", False) for r in results.values())
        else "FAILED",
        "components": {
            "alignment_validation": {
                ...
                "failed_tests": [
                    {"name": k, "error": v.get("error", "Test failed")}
