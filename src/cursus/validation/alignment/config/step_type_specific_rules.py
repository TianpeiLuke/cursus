...
class StepTypeCategory(Enum):
    ...
    CONTRACT_BASED = (
        "contract_based"  # Skip Level 1, need 2-4 (CradleDataLoading, etc.)
    )
    NON_SCRIPT = (
        "non_script"  # Skip Levels 1-2, need 3-4 (CreateModel, Transform, etc.)
    )
...
def validate_step_type_compliance(builder_class: type, step_type: str) -> List[str]:
    ...
    for method_name, method_spec in required_methods.items():
        if not hasattr(builder_class, method_name):
            issues.append(
                f"Builder {builder_class.__name__} missing required {step_type} method: {method_name}"
            )
