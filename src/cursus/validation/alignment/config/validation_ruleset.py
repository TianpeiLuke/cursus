...
class ValidationRuleset:
    ...
    level_4_validator_class: Optional[str] = (
        None  # Step-type-specific Level 4 validator
    )
...
def is_validation_level_enabled(
    sagemaker_step_type: str, level: ValidationLevel
) -> bool:
...
def validate_step_type_configuration() -> List[str]:
    ...
    for step_type, ruleset in VALIDATION_RULESETS.items():
        ...
        if (
            ruleset.category != StepTypeCategory.EXCLUDED
            and ValidationLevel.SPEC_DEPENDENCY not in ruleset.enabled_levels
        ):
...
def get_validation_ruleset_for_step_name(
    step_name: str, workspace_id: str = None
) -> Optional[ValidationRuleset]:
...
def is_validation_level_enabled_for_step_name(
    step_name: str, level: ValidationLevel, workspace_id: str = None
) -> bool:
...
def get_enabled_validation_levels_for_step_name(
    step_name: str, workspace_id: str = None
) -> Set[ValidationLevel]:
