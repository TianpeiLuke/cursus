...
class ProcessingStepBuilderValidator(StepTypeSpecificValidator):
    ...
    def _validate_step_type_specifics(
        ...
        input_output_issues = self._validate_processing_input_output_handling(
            builder_class
        )
        ...
        logger.debug(
            f"Processing-specific validation completed for {step_name}: {len(issues)} issues found"
        )
        return issues

    def _validate_create_processor_method(
        ...
        if not hasattr(builder_class, "_create_processor"):
            ...
            try:
                ...
                logger.debug(
                    f"Could not validate _create_processor signature: {str(e)}"
                )
        ...
        if hasattr(builder_class, "_get_outputs"):
            ...
            try:
                ...
                if signature.return_annotation != inspect.Signature.empty:
                    ...
                    if (
                        "List" not in return_annotation
                        and "ProcessingOutput" not in return_annotation
                    ):
                        issues.append(
                            {
                                "level": "INFO",
                ...
                logger.debug(
                    f"Could not validate _get_outputs for Processing: {str(e)}"
                )

        return issues

    def _validate_processing_input_output_handling(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _validate_job_arguments_override(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        if hasattr(builder_class, "_get_job_arguments"):
            ...
            if self._is_method_overridden(builder_class, "_get_job_arguments"):
                try:
                    ...
                    logger.debug(
                        f"Could not validate _get_job_arguments override: {str(e)}"
                    )
    ...
    def validate_processing_step_patterns(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            processor_creation_issues = self._validate_processor_creation_pattern(
                builder_class
            )
            pattern_issues.extend(processor_creation_issues)

            # Check for script execution patterns
            script_execution_issues = self._validate_script_execution_pattern(
                builder_class
            )
            ...
            logger.error(
                f"Processing pattern validation failed for {step_name}: {str(e)}"
            )
            return {"status": "ERROR", "error": str(e)}

    def _validate_processor_creation_pattern(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_processor_types = [
                ptype for ptype in processor_types if ptype in source
            ]
    ...
    def _validate_script_execution_pattern(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_patterns = [
                pattern for pattern in script_patterns if pattern in source
            ]
