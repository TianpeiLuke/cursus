...
class ValidatorFactory:
    ...
    def __init__(self, workspace_dirs: List[str]):
        ...
        logger.info(
            f"Initialized ValidatorFactory with {len(self._validator_registry)} validator types"
        )

    def get_validator(
        self, validator_class: str
    ) -> Optional[StepTypeSpecificValidator]:
        """
    ...
    def get_validator_for_step_type(
        self, step_type: str
    ) -> Optional[StepTypeSpecificValidator]:
        """
    ...
    def get_validator_for_step_name(
        self, step_name: str
    ) -> Optional[StepTypeSpecificValidator]:
        """
        ...
        try:
            ...
            if is_step_type_excluded(step_type):
                return {
                    ...
                    if ruleset
                    else f"Step type {step_type} is excluded",
    ...
    def get_validator_registry_status(self) -> Dict[str, Any]:
        ...
        """
        implemented = sum(
            1 for cls in self._validator_registry.values() if cls is not None
        )
    ...
    def validate_factory_configuration(self) -> Dict[str, Any]:
        ...
        for step_type, validator_class in step_type_mapping.items():
            if validator_class not in self._validator_registry:
                issues.append(
                    f"Step type '{step_type}' references unknown validator: {validator_class}"
                )
            elif self._validator_registry[validator_class] is None:
                issues.append(
                    f"Step type '{step_type}' references unimplemented validator: {validator_class}"
                )

        # Check for unused validators
        used_validators = set(step_type_mapping.values())
        available_validators = {
            name for name, cls in self._validator_registry.items() if cls is not None
        }
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        implemented_validators = sum(
            1 for info in available_validators.values() if info["implemented"]
        )
        ...
        return {
            ...
            "implementation_rate": implemented_validators / total_validators
...
class StepTypeValidatorIntegration:
    ...
    def __init__(self, workspace_dirs: List[str]):
        ...
        logger.info(
            "Initialized StepTypeValidatorIntegration with priority-based validation"
        )
    ...
    def validate_step_with_full_integration(self, step_name: str) -> Dict[str, Any]:
        ...
        validation_result = self.validator_factory.validate_step_with_priority_system(
            step_name
        )
