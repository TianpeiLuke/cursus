...
class TransformStepBuilderValidator(StepTypeSpecificValidator):
    ...
    def _validate_step_type_specifics(
        self, step_name: str, builder_class: Type, step_type: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        transformer_type_issues = self._validate_transformer_type_patterns(
            builder_class
        )
        issues.extend(transformer_type_issues)

        logger.debug(
            f"Transform-specific validation completed for {step_name}: {len(issues)} issues found"
        )
        return issues

    def _validate_create_transformer_method(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not hasattr(builder_class, "_create_transformer"):
            ...
            try:
                ...
                logger.debug(
                    f"Could not validate _create_transformer signature: {str(e)}"
                )
        ...
        if hasattr(builder_class, "_get_outputs"):
            ...
            try:
                ...
                if signature.return_annotation != inspect.Signature.empty:
                    ...
                    if (
                        "str" not in return_annotation
                        and "String" not in return_annotation
                    ):
                        issues.append(
                            {
                                "level": "INFO",
    ...
    def _validate_transform_configuration(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_batch_patterns = [
                pattern for pattern in batch_patterns if pattern in source
            ]
            ...
            found_format_patterns = [
                pattern for pattern in data_format_patterns if pattern in source
            ]
    ...
    def _validate_transformer_type_patterns(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_transformer_patterns = [
                pattern for pattern in transformer_patterns if pattern in source
            ]
            ...
            if not found_transformer_patterns:
                ...
                logger.debug(
                    f"Found transformer patterns: {found_transformer_patterns}"
                )

                # Additional validation for model-based transformers
                if "model.transformer" in found_transformer_patterns:
                    model_transformer_issues = (
                        self._validate_model_transformer_patterns(source)
                    )
    ...
    def _validate_model_transformer_patterns(self, source: str) -> List[Dict[str, Any]]:
        ...
        found_patterns = [
            pattern for pattern in model_transformer_patterns if pattern in source
        ]
    ...
    def validate_transform_step_patterns(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transformer_creation_issues = self._validate_transformer_creation_pattern(
                builder_class
            )
            pattern_issues.extend(transformer_creation_issues)

            # Check for batch transform job configuration patterns
            batch_config_issues = self._validate_batch_transform_configuration(
                builder_class
            )
            ...
            logger.error(
                f"Transform pattern validation failed for {step_name}: {str(e)}"
            )
            return {"status": "ERROR", "error": str(e)}

    def _validate_transformer_creation_pattern(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_patterns = [
                pattern for pattern in instantiation_patterns if pattern in source
            ]
    ...
    def _validate_batch_transform_configuration(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_patterns = [
                pattern for pattern in optimization_patterns if pattern in source
            ]
            ...
            if not found_patterns:
                ...
                logger.debug(
                    f"Found batch transform optimization patterns: {found_patterns}"
                )

            # Check for data handling patterns
            data_handling_patterns = ["assemble_with", "split_type", "compression_type"]
            found_data_patterns = [
                pattern for pattern in data_handling_patterns if pattern in source
            ]
