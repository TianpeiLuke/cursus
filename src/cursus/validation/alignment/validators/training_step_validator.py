...
class TrainingStepBuilderValidator(StepTypeSpecificValidator):
    ...
    def _validate_step_type_specifics(
        ...
        logger.debug(
            f"Training-specific validation completed for {step_name}: {len(issues)} issues found"
        )
        return issues

    def _validate_create_estimator_method(
        ...
        if not hasattr(builder_class, "_create_estimator"):
            ...
            try:
                ...
                logger.debug(
                    f"Could not validate _create_estimator signature: {str(e)}"
                )
    ...
    def _validate_training_configuration(
        ...
        try:
            ...
            found_hyperparameter_patterns = [
                pattern for pattern in hyperparameter_patterns if pattern in source
            ]
    ...
    def _validate_estimator_type_patterns(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_estimator_types = [
                etype for etype in estimator_types if etype in source
            ]
    ...
    def _validate_pytorch_patterns(self, source: str) -> List[Dict[str, Any]]:
        ...
        ):  # Expect framework_version, py_version, and source_dir/entry_point
            issues.append(
                {
                    "level": "INFO",
    ...
    def validate_training_step_patterns(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            estimator_creation_issues = self._validate_estimator_creation_pattern(
                builder_class
            )
            ...
            logger.error(
                f"Training pattern validation failed for {step_name}: {str(e)}"
            )
            return {"status": "ERROR", "error": str(e)}

    def _validate_estimator_creation_pattern(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _validate_training_job_configuration(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            found_patterns = [
                pattern for pattern in job_config_patterns if pattern in source
            ]
