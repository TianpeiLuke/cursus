...
class StepTypeSpecificValidator(ABC):
    ...
    def __init__(self, workspace_dirs: Optional[List[str]] = None):
        ...
        logger.info(
            f"Initialized {self.__class__.__name__} with priority-based validation"
        )
    ...
    def validate_builder_config_alignment(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results["priority_resolution"] = combined_result.get(
    ...
    def _apply_step_specific_validation(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            step_specific_issues = self._validate_step_type_specifics(
                step_name, builder_class, step_type
            )
    ...
    def _resolve_validation_priorities(
        self, universal_result: Dict[str, Any], step_specific_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "error_count": sum(
                1 for issue in combined_issues if issue["level"] == "ERROR"
            ),
            "warning_count": sum(
                1 for issue in combined_issues if issue["level"] == "WARNING"
            ),
    ...
    def _get_builder_class(self, step_name: str) -> Optional[Type]:
        ...
        try:
            ...
            builder_module_name = (
                f"cursus.steps.builders.builder_{step_name.lower()}_step"
            )
    ...
    def _is_method_overridden(self, builder_class: Type, method_name: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Could not check if method {method_name} is overridden: {str(e)}"
            )
            return False

    def _combine_validation_results(
        self, base_results: Dict[str, Any], specific_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if specific_results.get("issues"):
            ...
            final_result["error_count"] = sum(
                1 for issue in existing_issues if issue.get("level") == "ERROR"
            )
            final_result["warning_count"] = sum(
                1 for issue in existing_issues if issue.get("level") == "WARNING"
            )
