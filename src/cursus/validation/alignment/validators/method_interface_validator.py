...
class MethodInterfaceValidator:
    ...
    def validate_builder_interface(
        self, builder_class: Type, step_type: str
        ...
        """
        logger.info(
            f"Validating builder interface for {builder_class.__name__} (step type: {step_type})"
        )
        ...
        step_specific_issues = self._validate_step_type_methods(
            builder_class, step_type
        )
        issues.extend(step_specific_issues)

        logger.info(
            f"Method interface validation completed for {builder_class.__name__}: {len(issues)} issues found"
        )
        return issues

    def _validate_universal_methods(
        self, builder_class: Type, step_type: str
    ) -> List[ValidationIssue]:
        """
    ...
    def _validate_step_type_methods(
        self, builder_class: Type, step_type: str
    ) -> List[ValidationIssue]:
        """
    ...
    def validate_builder_by_name(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"step_name": step_name, "status": "ERROR", "error": str(e)}
