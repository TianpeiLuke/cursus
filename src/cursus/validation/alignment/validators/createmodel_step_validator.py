...
class CreateModelStepBuilderValidator(StepTypeSpecificValidator):
    ...
    def _validate_step_type_specifics(
        ...
        logger.debug(
            f"CreateModel-specific validation completed for {step_name}: {len(issues)} issues found"
        )
        return issues

    def _validate_create_model_method(
    ...
    def _validate_createmodel_outputs(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
        Validate _get_outputs method for CreateModel steps.
            ...
            try:
                ...
                if self._is_method_overridden(builder_class, "_get_outputs"):
                    ...
                    if signature.return_annotation != inspect.Signature.empty:
                        ...
                        if (
                            "None" not in return_annotation
                            and "NoneType" not in return_annotation
                        ):
                            issues.append(
                                {
                                    "level": "INFO",
                    ...
                    logger.debug(
                        "CreateModel step using inherited _get_outputs implementation"
                    )

            except Exception as e:
                logger.debug(
                    f"Could not validate _get_outputs for CreateModel: {str(e)}"
                )
    ...
    def _validate_model_configuration(
        ...
        try:
            ...
            found_patterns = [
                pattern for pattern in model_patterns if pattern in source
            ]
        ...
        if hasattr(builder_class, "_get_image_uri"):
            ...
            if self._is_method_overridden(builder_class, "_get_image_uri"):
                try:
                    ...
                    logger.debug(
                        f"Could not validate _get_image_uri override: {str(e)}"
                    )
    ...
    def validate_createmodel_step_patterns(self, step_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            container_config_issues = self._validate_container_configuration(
                builder_class
            )
            ...
            logger.error(
                f"CreateModel pattern validation failed for {step_name}: {str(e)}"
            )
            return {"status": "ERROR", "error": str(e)}

    def _validate_model_creation_pattern(
        self, builder_class: Type
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _validate_container_configuration(
        ...
        try:
            ...
            found_patterns = [
                pattern for pattern in container_patterns if pattern in source
            ]
    ...
    def _validate_pytorch_model_patterns(self, source: str) -> List[Dict[str, Any]]:
        ...
        ):  # Expect framework_version, py_version, and model_data
            issues.append(
                {
                    "level": "INFO",
