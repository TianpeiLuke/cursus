...
class ContractSpecificationAlignmentTester:
    ...
    def validate_contract(self, contract_name: str) -> Dict[str, Any]:
        ...
        for spec_key, specification in specifications.items():
            # Validate logical name alignment
            logical_issues = self._validate_logical_names(contract, specification, contract_name, spec_key)
    ...
    def _load_specification_from_file(self, spec_path: Path, spec_info: Dict[str, Any]) -> Dict[str, Any]:
        """Load specification from file using the spec info."""
        try:
            ...
            try:
                # Create a temporary module from the modified content
                module_name = f"{spec_path.stem}_temp"
                spec = importlib.util.spec_from_loader(module_name, loader=None)
                module = importlib.util.module_from_spec(spec)
                ...
                # Look for the specification constant using the extracted name
