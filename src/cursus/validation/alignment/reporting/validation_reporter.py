...
class ValidationReporter:
    ...
    def generate_report(
        self, output_file: Optional[Union[str, Path, TextIO]] = None
    ) -> str:
        """
    ...
    def _generate_json_report(self) -> str:
        ...
        for result in self.summary.results:
            if self._should_include_result(result):
                ...
                if (
                    self.config.max_issues_per_step
                    and len(result_data["issues"]) > self.config.max_issues_per_step
                ):
                    result_data["issues"] = result_data["issues"][
                        : self.config.max_issues_per_step
                    ]
    ...
    def _format_result_text(self, result: ValidationResult) -> List[str]:
        ...
        if result.total_issues > 0:
            ...
            if result.error_count > 0:
                issue_parts.append(
                    self._colorize(f"{result.error_count} errors", "red")
                )
            if result.warning_count > 0:
                issue_parts.append(
                    self._colorize(f"{result.warning_count} warnings", "yellow")
                )
    ...
    def _generate_issue_breakdown(self) -> List[str]:
        ...
        issue_counts = {IssueLevel.ERROR: 0, IssueLevel.WARNING: 0, IssueLevel.INFO: 0}
        ...
        if method_issues:
            ...
            sorted_methods = sorted(
                method_issues.items(), key=lambda x: x[1], reverse=True
            )
        ...
        if rule_type_issues:
            ...
            sorted_rules = sorted(
                rule_type_issues.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.summary.total_errors > 0:
            lines.append(
                "• Address all ERROR-level issues first - these indicate critical problems"
            )

        if self.summary.total_warnings > 0:
            lines.append(
                "• Review WARNING-level issues - these may indicate potential problems"
            )

        if self.summary.success_rate < 0.8:
            lines.append(
                "• Success rate is below 80% - consider reviewing validation rules"
            )
        ...
        if method_error_counts:
            ...
            lines.append(
                f"• Focus on '{top_method[0]}' method - has {top_method[1]} errors"
            )
    ...
    def _generate_html_results(self) -> str:
        ...
        for result in self.summary.results:
            ...
            html_parts.append(
                f"<div class='status'>Status: {result.status.value}</div>"
            )
    ...
    def _should_include_result(self, result: ValidationResult) -> bool:
        ...
        if result.status == ValidationStatus.PASSED and not self.config.include_passed:
            return False
        if (
            result.status == ValidationStatus.EXCLUDED
            and not self.config.include_excluded
        ):
            return False
        return True
    ...
    def calculate_score(self) -> Dict[str, float]:
        ...
        quality_score = max(
            0.0, 1.0 - (total_weighted_issues / max(max_possible_weighted_issues, 1))
        )
    ...
    def print_summary(self, file: TextIO = None):
        ...
        print(
            f"  Steps: {self.summary.passed_steps}/{self.summary.total_steps} passed",
            file=output,
        )
        print(f"  Success Rate: {scores['success_rate']:.1%}", file=output)
        print(f"  Overall Score: {scores['overall_score']:.1%}", file=output)
        print(
            f"  Issues: {self.summary.total_errors} errors, {self.summary.total_warnings} warnings",
            file=output,
        )
    ...
    def generate_console_report(
        self, summary: ValidationSummary, detailed: bool = False
    ) -> str:
        """
