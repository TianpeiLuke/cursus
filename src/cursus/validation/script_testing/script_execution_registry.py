...
class ScriptExecutionRegistry:
    ...
    def __init__(self, dag: PipelineDAG, step_catalog: Optional[StepCatalog] = None):
        ...
        logger.info(
            f"ScriptExecutionRegistry initialized for DAG with {len(self.dag.nodes)} nodes"
        )
    ...
    def initialize_from_dependency_matcher(self, prepared_data: Dict[str, Any]):
        ...
        logger.info(
            f"Registry initialized with {len(self._state['node_configs'])} node configurations"
        )
        logger.debug(
            f"Dependency matches: {len(self._state['dependency_graph'])} nodes with matches"
        )
    ...
    def get_dependency_outputs_for_node(self, node_name: str) -> Dict[str, str]:
        ...
        for dep_node in self.dag.get_dependencies(node_name):
            if dep_node in self._state["execution_outputs"]:
                ...
                logger.debug(
                    f"Provided {len(dep_outputs)} outputs from {dep_node} to {node_name}"
                )

        logger.info(
            f"Collected {len(dependency_outputs)} dependency outputs for {node_name}"
        )
    ...
    def store_resolved_inputs(self, node_name: str, resolved_inputs: Dict[str, Any]):
        ...
        logger.debug(
            f"Stored resolved inputs for {node_name}: {len(resolved_inputs)} items"
        )
    ...
    def commit_execution_results(
        self, node_name: str, execution_result: ScriptTestResult
    ):
        """
        ...
        if execution_result.success:
            ...
            logger.info(
                f"✅ {node_name} execution committed: {len(execution_result.output_files)} outputs"
            )
            logger.debug(f"Outputs: {list(execution_result.output_files.keys())}")
        else:
            self._state["execution_status"][node_name] = "failed"
            logger.error(
                f"❌ {node_name} execution failed: {execution_result.error_message}"
            )
    ...
    def _update_node_state(self, node_name: str) -> Dict[str, Any]:
        ...
        for dep_node in self.dag.get_dependencies(node_name):
            if self._is_node_completed(dep_node):
                ...
                message_updates = self._apply_message_passing(
                    dep_node, node_name, dep_outputs
                )
    ...
    def _apply_message_passing(
        self, from_node: str, to_node: str, outputs: Dict[str, str]
    ) -> Dict[str, str]:
        """
        ...
        for output_key, output_path in outputs.items():
            ...
            if output_key in expected_inputs:
                ...
                logger.debug(
                    f"📨 Direct mapping: {from_node}.{output_key} → {to_node}.{output_key}"
                )
            ...
            if semantic_mapping:
                ...
                logger.debug(
                    f"📨 Semantic mapping: {from_node}.{output_key} → {to_node}.{semantic_mapping}"
                )
            ...
            logger.debug(
                f"📨 Prefixed mapping: {from_node}.{output_key} → {to_node}.{prefixed_key}"
            )
    ...
    def _get_semantic_mapping(
        self, output_key: str, expected_inputs: Set[str]
    ) -> Optional[str]:
        """
    ...
    def _log_message_passing(
        self, from_node: str, to_node: str, message_data: Dict[str, str]
    ):
        """
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completed_scripts": [
                node
                for node, state in self._state["execution_status"].items()
                if state == "completed"
            ],
            "failed_scripts": [
                node
    ...
    def validate_execution_order(dag: PipelineDAG, execution_order: List[str]):
        ...
        for node in execution_order:
            ...
            if not dependencies.issubset(completed_nodes):
                ...
                raise ValueError(
                    f"Invalid execution order: {node} depends on {missing_deps} which haven't been processed yet"
                )

            completed_nodes.add(node)

        logger.info(
            f"✅ Execution order validated: {len(execution_order)} nodes in correct dependency order"
        )
    ...
    def ensure_state_consistency(registry: ScriptExecutionRegistry, node_name: str):
        ...
        for dep_node in dependencies:
            if not registry._is_node_completed(dep_node):
                raise RuntimeError(
                    f"State inconsistency: {node_name} cannot execute because dependency {dep_node} is not completed"
                )

        logger.debug(f"✅ State consistency verified for {node_name}")


def create_script_execution_registry(
    dag: PipelineDAG, step_catalog: Optional[StepCatalog] = None
