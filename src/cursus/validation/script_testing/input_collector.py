...
class ScriptTestingInputCollector:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ScriptTestingInputCollector with {len(self.loaded_configs)} configs, mode={integration_mode}, dependency_resolution={use_dependency_resolution}"
        )
    ...
    def collect_script_inputs_for_dag(self) -> Dict[str, Any]:
        ...
        if self.registry:
            ...
            return resolve_script_dependencies(
                dag=self.dag, config_path=self.config_path, step_catalog=StepCatalog()
            )
        else:
    ...
    def _collect_inputs_with_registry(self) -> Dict[str, Any]:
        ...
        for node_name in self.dag.topological_sort():
            try:
                ...
                dependency_outputs = self.registry.get_dependency_outputs_for_node(
                    node_name
                )
                ...
                logger.info(
                    f"✅ Registry-populated inputs for {node_name}: {len(script_inputs)} fields"
                )

            except Exception as e:
                logger.error(
                    f"❌ Failed to collect registry inputs for {node_name}: {e}"
                )
        ...
        logger.info(
            f"🎯 Registry-coordinated collection completed: {len(user_inputs)} scripts"
        )
        return user_inputs

    def _populate_fields_from_registry(
        self,
        node_name: str,
        ...
        """
        logger.debug(
            f"🔧 Populating fields for {node_name} with {len(dependency_outputs)} dependency outputs"
        )
        ...
        logger.debug(
            f"📨 Applied {len(dependency_outputs)} dependency outputs to {node_name}"
        )

        # 2. Add contract-based paths for missing inputs
        contract_inputs = self._get_input_paths_with_message_passing(
            node_name, dependency_outputs
        )
        ...
        script_inputs["environment_variables"] = (
            self._extract_environment_variables_with_registry(
        ...
        script_inputs["script_path"] = self._extract_script_path_from_config(
            config_instance
        )

        logger.info(
            f"🎯 Populated {node_name}: {len(script_inputs['input_paths'])} inputs, "
    ...
    def _get_input_paths_with_message_passing(
        self, node_name: str, dependency_outputs: Dict[str, str]
    ) -> Dict[str, str]:
        """
        ...
        for output_key, output_path in dependency_outputs.items():
            ...
            if semantic_mapping and semantic_mapping not in input_paths:
                ...
                logger.debug(
                    f"📨 Semantic mapping: {output_key} → {semantic_mapping} for {node_name}"
                )
    ...
    def _extract_environment_variables_with_registry(
        self, config: Any, node_name: str
    ) -> Dict[str, str]:
        """
        ...
        if self.registry:
            try:
                ...
                registry_vars = {
                    ...
                    "NODE_EXECUTION_ORDER": str(
                        self.registry.execution_order.index(node_name)
                    ),
                    "TOTAL_PIPELINE_NODES": str(len(self.dag.nodes)),
                    "COMPLETED_DEPENDENCIES": ",".join(
                        [
                            dep
                            for dep in self.dag.get_dependencies(node_name)
                ...
                logger.debug(
                    f"🔧 Added {len(registry_vars)} registry-specific env vars for {node_name}"
                )
        ...
        for field in entry_point_fields:
            if hasattr(config, field):
                ...
                if entry_point:
                    ...
                    if (
                        hasattr(config, "effective_source_dir")
                        and config.effective_source_dir
                    ):
                        import os
    ...
    def _get_validated_scripts_from_config(self) -> List[str]:
        ...
        logger.info(
            f"Phantom script elimination: {len(self.dag.nodes)} nodes -> {len(validated_scripts)} validated scripts"
        )
    ...
    def _get_default_input_paths(self, script_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            contract_adapter = ContractDiscoveryManagerAdapter(
            ...
            if contract:
                ...
                adapted_paths = contract_adapter.get_contract_input_paths(
                    contract, script_name
                )

                if adapted_paths:
                    logger.info(
                        f"SUCCESS: Using systematic contract-based input paths for {script_name}: {list(adapted_paths.keys())}"
                    )
                    return adapted_paths
                else:
                    logger.warning(
                        f"Contract found but no input paths for {script_name}"
                    )
            else:
            ...
            logger.error(
                f"Error in systematic contract-based path resolution for {script_name}: {e}"
            )
    ...
    def _get_default_output_paths(self, script_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            if contract:
                ...
                adapted_paths = contract_adapter.get_contract_output_paths(
                    contract, script_name
                )

                if adapted_paths:
                    logger.info(
                        f"SUCCESS: Using systematic contract-based output paths for {script_name}: {list(adapted_paths.keys())}"
                    )
                    return adapted_paths
                else:
                    logger.warning(
                        f"Contract found but no output paths for {script_name}"
                    )
            else:
                logger.warning(f"No contract found for {script_name}")
            ...
            logger.error(
                f"Error in systematic contract-based output path resolution for {script_name}: {e}"
            )
