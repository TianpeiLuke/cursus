...
class ResultFormatter:
    ...
    def format_execution_results(
        self, results: Dict[str, Any], format_type: str = "console"
    ) -> str:
        """
    ...
    def format_script_result(
        self, script_result: ScriptTestResult, format_type: str = "console"
    ) -> str:
        """
        ...
        if format_type == "console":
            ...
            return json.dumps(
                self._script_result_to_dict(script_result), indent=2, default=str
            )
    ...
    def create_summary_report(self, results: Dict[str, Any]) -> str:
        ...
        summary.write(
            f"Failed: {results.get('total_scripts', 0) - results.get('successful_scripts', 0)}\n"
        )

        if results.get("total_scripts", 0) > 0:
            success_rate = results.get("successful_scripts", 0) / results.get(
                "total_scripts", 1
            )
            summary.write(f"Success Rate: {success_rate:.1%}\n")

        summary.write(
            f"Pipeline Success: {'✅ YES' if results.get('pipeline_success', False) else '❌ NO'}\n\n"
        )
        ...
        if script_results:
            ...
            for node_name, script_result in script_results.items():
                ...
                time_str = (
                    f" ({script_result.execution_time:.2f}s)"
                    if script_result.execution_time
                    else ""
                )
                ...
                if not script_result.success and script_result.error_message:
                    ...
                    if len(error_msg) > self.format_options["max_error_length"]:
                        error_msg = (
                            error_msg[: self.format_options["max_error_length"]] + "..."
                        )
    ...
    def save_results_to_file(
        self, results: Dict[str, Any], output_path: str, format_type: str = "json"
    ) -> Path:
        """
    ...
    def _format_console_results(self, results: Dict[str, Any]) -> str:
        ...
        if self.format_options["show_timestamps"]:
            output.write(
                f"Generated: {datetime.now().strftime(self.format_options['date_format'])}\n"
            )
        output.write("=" * 60 + "\n\n")
        ...
        output.write(
            f"{status_icon} Pipeline Status: {'SUCCESS' if pipeline_success else 'FAILED'}\n"
        )
        ...
        if total_scripts > 0:
            output.write(
                f"📊 Scripts: {successful_scripts}/{total_scripts} successful\n"
            )
        ...
        if execution_order and script_results:
            ...
            for i, node_name in enumerate(execution_order, 1):
                if node_name in script_results:
                    ...
                    if (
                        self.format_options["show_execution_times"]
                        and script_result.execution_time
                    ):
                    ...
                    if not script_result.success and script_result.error_message:
                        if self.format_options["highlight_errors"]:
                            output.write(
                                f"     ❌ Error: {script_result.error_message}\n"
                            )

                    if (
                        script_result.output_files
                        and self.format_options["show_file_paths"]
                    ):
                        output.write(
                            f"     📁 Outputs: {len(script_result.output_files)} files\n"
                        )
        ...
        for node_name in execution_order:
            if node_name in script_results:
                ...
                row = [
                    ...
                    len(script_result.output_files)
...
""")
        ...
        if self.format_options["show_timestamps"]:
            html.write(
                f"<p>Generated: {datetime.now().strftime(self.format_options['date_format'])}</p>"
            )
        html.write("</div>")
        ...
        if total_scripts > 0:
            html.write(
                f"<p>Scripts: {successful_scripts}/{total_scripts} successful</p>"
            )
        ...
        if script_results:
            ...
            html.write("<table>")
            html.write(
                "<tr><th>Script</th><th>Status</th><th>Time (s)</th><th>Outputs</th><th>Error</th></tr>"
            )
            ...
            for node_name in execution_order:
                if node_name in script_results:
                    ...
                    html.write(
                        f'<td>{script_result.execution_time:.2f if script_result.execution_time else "N/A"}</td>'
                    )
                    html.write(
                        f"<td>{len(script_result.output_files) if script_result.output_files else 0}</td>"
                    )
    ...
    def __repr__(self) -> str:
        ...
        return f"ResultFormatter(options={len(self.format_options)} configured)"
