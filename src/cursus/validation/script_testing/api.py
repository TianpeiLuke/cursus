    ...
    try:
        ...
        if use_dependency_resolution:
            ...
            user_inputs = resolve_script_dependencies_with_registry(
                dag, config_path, step_catalog, registry
            )
            logger.info(
                f"Used registry-coordinated dependency resolution for {len(user_inputs)} scripts"
            )
        else:
            ...
            prepared_data = prepare_script_testing_inputs(
                dag, config_path, step_catalog
            )
            prepared_data[
                "dependency_matches"
            ] = {}  # Clear dependency matches for manual mode
            ...
            logger.info(
                f"Used registry-coordinated manual input collection for {len(user_inputs)} scripts"
            )
...
def collect_script_inputs_using_dag_factory(
    dag: PipelineDAG, config_path: str
) -> Dict[str, Any]:
    """
...
def get_validated_scripts_from_config(
    dag: PipelineDAG, configs: Dict[str, Any]
) -> List[str]:
    """
    ...
    for node_name in dag.nodes:
        if node_name in configs:
            ...
            if hasattr(config, "training_entry_point") or hasattr(
                config, "inference_entry_point"
            ):
    ...
    logger.info(
        f"Phantom script elimination: {len(dag.nodes)} nodes -> {len(validated_scripts)} validated scripts"
    )
            ...
            if entry_point:
                ...
                if hasattr(config, "resolve_hybrid_path"):
                    try:
                        ...
                        logger.debug(
                            f"Hybrid path resolution failed for {script_path}: {e}"
                        )
                        pass
...
def extract_environment_variables_from_config(config) -> Dict[str, str]:
    ...
    try:
        ...
        logger.debug(
            f"model_dump() failed for config, using direct attribute access: {e}"
        )
        config_data = {}
    ...
    derived_env_fields = ["pipeline_name", "pipeline_s3_loc", "aws_region"]
...
def execute_scripts_in_order(
    ...
    for node_name in execution_order:
        try:
            ...
            input_paths = node_inputs.get(
                "input_paths", {}
            )  # ✅ Auto-resolved or user-provided
            output_paths = node_inputs.get("output_paths", {})  # ✅ User-provided
            environ_vars = node_inputs.get(
            ...
            result = execute_single_script(
                script_path, input_paths, output_paths, environ_vars, job_args
            )
...
def execute_scripts_with_registry_coordination(
    ...
    for node_name in execution_order:
        try:
            ...
            error_result = ScriptTestResult(
                success=False, error_message=f"Execution failed: {e}"
            )
    ...
    return {
        "pipeline_success": successful_scripts
        == len([n for n in execution_order if registry.get_ready_node_inputs(n)]),
...
def import_and_execute_script(
    ...
    try:
        ...
        if hasattr(script_module, "main"):
            ...
            raise ValueError(
                f"Script {script_path} does not have a main function with the required signature"
            )
