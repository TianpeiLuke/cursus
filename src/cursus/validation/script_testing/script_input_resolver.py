    ...
    try:
        ...
        for dep_name, dep_spec in spec.dependencies.items():
            ...
            if dep_spec.required and dep_name not in resolved_dependencies:
                raise ValueError(
                    f"Required dependency '{dep_name}' not resolved for {node_name}"
                )
            ...
            if contract and hasattr(contract, "expected_input_paths"):
                ...
                script_inputs[dep_name] = actual_path
                logger.debug(
                    f"No contract available, used direct mapping for {dep_name}: {actual_path}"
                )

        logger.info(
            f"Successfully resolved {len(script_inputs)} script inputs for {node_name}"
        )
        ...
        raise RuntimeError(
            f"Script input resolution failed for {node_name}: {e}"
        ) from e


def adapt_step_input_patterns_for_scripts(
    node_name: str, inputs: Dict[str, Any], step_catalog: StepCatalog
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        logger.debug(
            f"Loaded specification for {node_name}: {len(spec.dependencies)} dependencies"
        )
        ...
        for dep_name, dep_spec in spec.dependencies.items():
            logger.debug(
                f"Processing dependency {dep_name} (required: {dep_spec.required})"
            )
            ...
            if dep_spec.required and dep_name not in inputs:
                raise ValueError(
                    f"Required input '{dep_name}' not provided for {node_name}"
                )
            ...
            if contract and hasattr(contract, "expected_input_paths"):
                container_path = contract.expected_input_paths.get(dep_name)
                logger.debug(
                    f"Contract container path for {dep_name}: {container_path}"
                )
        ...
        logger.info(
            f"Successfully adapted {len(script_inputs)} inputs for script {node_name}"
        )
        return script_inputs
        ...
        raise RuntimeError(
            f"Step input pattern adaptation failed for {node_name}: {e}"
        ) from e


def validate_script_input_resolution(
    node_name: str, script_inputs: Dict[str, str], step_catalog: StepCatalog
) -> bool:
    """
    ...
    try:
        ...
        for dep_name, path in script_inputs.items():
            ...
            if not isinstance(path, str) or len(path.strip()) == 0:
                logger.error(
                    f"Invalid path format for dependency '{dep_name}' in {node_name}: {path}"
                )
                return False
...
def get_script_input_resolution_summary(
    ...
    try:
        ...
        required_dependencies = (
            sum(1 for dep_spec in spec.dependencies.values() if dep_spec.required)
            if spec
            else 0
        )
        ...
        if contract and hasattr(contract, "expected_input_paths"):
            contract_paths_used = len(
                [
                    dep_name
                    for dep_name in script_inputs.keys()
        ...
        logger.error(
            f"Failed to generate script input resolution summary for {node_name}: {e}"
        )
        return {"node_name": node_name, "error": str(e), "resolution_complete": False}


def transform_logical_names_to_actual_paths(
    logical_inputs: Dict[str, str], node_name: str, step_catalog: StepCatalog
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        if not contract:
            logger.warning(
                f"No contract found for {node_name}, returning logical inputs as-is"
            )
        ...
        for logical_name, logical_path in logical_inputs.items():
            ...
            if hasattr(contract, "expected_input_paths"):
                ...
                if container_path:
                    ...
                    logger.debug(
                        f"Contract transformation for {logical_name}: {logical_path}"
                    )
                else:
                ...
                actual_paths[logical_name] = logical_path
                logger.debug(
                    f"No contract mappings, direct mapping for {logical_name}: {logical_path}"
                )

        logger.info(
            f"Successfully transformed {len(actual_paths)} logical names to actual paths for {node_name}"
        )
        return actual_paths

    except Exception as e:
        logger.error(
            f"Failed to transform logical names to actual paths for {node_name}: {e}"
        )
