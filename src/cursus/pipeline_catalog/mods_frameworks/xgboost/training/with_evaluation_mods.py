...
Example:
    ```python
    from cursus.pipeline_catalog.mods_frameworks.xgboost.training.with_evaluation_mods import create_pipeline
...
def create_pipeline(
    ...
    if pipeline_name:
        ...
        dag_compiler.pipeline_name = "MODS-XGBoost-Training-Evaluation-Pipeline"
        
    if pipeline_description:
        dag_compiler.pipeline_description = pipeline_description
    else:
        dag_compiler.pipeline_description = "MODS-enhanced XGBoost training pipeline with evaluation support"
...
if __name__ == "__main__":
    ...
    try:
        ...
        pipeline, report, dag_compiler, mods_template = create_pipeline(
            ...
            session=pipeline_session,
            role=role,
            pipeline_name="MODS-XGBoost-Training-Evaluation-Pipeline",
            pipeline_description="MODS-enhanced XGBoost training pipeline with evaluation support"
        )
        ...
        #     pipeline=pipeline, 
        #     document={"training_dataset": "my-dataset", "evaluation_dataset": "my-eval-dataset"}, 
        ...
        logger.info("Please install MODS or use the standard XGBoost training with evaluation pipeline instead")
    except Exception as e:
