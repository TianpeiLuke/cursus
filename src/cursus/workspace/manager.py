...
class WorkspaceManager:
    ...
    def __init__(self, workspace_dirs: Optional[List[Path]] = None):
        ...
        if workspace_dirs:
            self.workspace_dirs = [
                Path(wd) if not isinstance(wd, Path) else wd for wd in workspace_dirs
            ]
        else:
            self.workspace_dirs = []
        ...
        logger.info(
            f"WorkspaceManager initialized with {len(self.workspace_dirs)} workspace directories"
        )
    ...
    def discover_components(self, workspace_id: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            logger.debug(
                f"Discovered {len(components)} components for workspace_id={workspace_id}"
            )
            ...
            logger.error(
                f"Error discovering components for workspace_id={workspace_id}: {e}"
            )
            return []
    ...
    def find_component_file(
        ...
        try:
            ...
            if step_info and step_info.file_components.get(component_type):
                ...
                logger.debug(
                    f"Found {component_type} file for {step_name}: {file_path}"
                )
                return file_path
    ...
    def create_workspace_pipeline(
        self, dag: PipelineDAG, config_path: str
    ) -> Optional[Any]:
        """
        ...
        try:
            ...
            if pipeline:
                ...
                logger.info(
                    f"Successfully created workspace pipeline with {len(dag.nodes)} steps"
                )
            else:
    ...
    def get_workspace_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "catalog_metrics": self.catalog.get_metrics_report()
                if hasattr(self.catalog, "get_metrics_report")
                else {},
            }
    ...
    def validate_workspace_structure(self, workspace_dir: Path) -> Dict[str, Any]:
        ...
        try:
            validation_result = {
                ...
                "is_directory": workspace_dir.is_dir()
            ...
            if not validation_result["is_directory"]:
                validation_result["warnings"].append(
                    f"Path is not a directory: {workspace_dir}"
                )
                return validation_result
            ...
            try:
                ...
                validation_result["warnings"].append(
                    f"Permission denied accessing directory: {workspace_dir}"
                )
                return validation_result
            ...
            if validation_result["components_found"] == 0:
                validation_result["warnings"].append(
                    f"No components discovered in workspace: {workspace_dir}"
                )
            ...
            logger.error(
                f"Error validating workspace structure for {workspace_dir}: {e}"
            )
            return {
                "workspace_dir": str(workspace_dir),
                "valid": False,
    ...
    def get_cross_workspace_components(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved cross-workspace components: {len(cross_workspace_components)} workspaces"
            )
