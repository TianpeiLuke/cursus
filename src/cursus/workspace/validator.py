...
class WorkspaceValidator:
        ...
        try:
            ...
            for component in components:
                ...
                if component_result:
                    ...
                    if (
                        hasattr(component_result, "warnings")
                        and component_result.warnings
                    ):
            ...
            logger.debug(
                f"Validated workspace {workspace_id}: {len(components)} components, valid={is_valid}"
            )
            return result

        except Exception as e:
            logger.error(
                f"Error validating workspace components for {workspace_id}: {e}"
            )
            return ValidationResult(
                is_valid=False,
    ...
    def _run_existing_validation(self, step_info: Any) -> Any:
        ...
        try:
            ...
            for required_component in required_components:
                if not step_info.file_components.get(required_component):
                    validation_warnings.append(
                        f"Missing {required_component} component"
                    )
                ...
                if file_metadata and hasattr(file_metadata, "path"):
                    if not Path(file_metadata.path).exists():
                        validation_issues.append(
                            f"File not found: {file_metadata.path}"
                        )
            ...
            logger.error(
                f"Error running existing validation for {step_info.step_name}: {e}"
            )
            return ValidationResult(
                is_valid=False,
    ...
    def validate_cross_workspace_compatibility(
        ...
        try:
            ...
            for workspace_id in workspace_ids:
                components = self.catalog.list_available_steps(
                    workspace_id=workspace_id
                )
            ...
            logger.debug(
                f"Cross-workspace compatibility check: {len(workspace_ids)} workspaces, compatible={is_compatible}"
            )
            return result
            ...
            return CompatibilityResult(
                is_compatible=False, issues=[f"Compatibility check failed: {str(e)}"]
            )

    def _check_dependency_compatibility(
        self, all_components: Dict[str, List[str]]
    ) -> List[str]:
        """
    ...
    def validate_component_quality(self, step_name: str) -> ValidationResult:
        ...
        try:
            ...
            if not step_info:
                return ValidationResult(
                    is_valid=False, errors=[f"Component not found: {step_name}"]
                )
                ...
                if file_metadata and hasattr(file_metadata, "path"):
                    if not Path(file_metadata.path).exists():
                        quality_issues.append(
                            f"File not accessible: {file_metadata.path}"
                        )
            ...
            logger.debug(
                f"Quality validation for {step_name}: score={quality_score}, valid={is_valid}"
            )
            return result
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "catalog_info": {
                    "total_steps": len(self.catalog._step_index)
                    if hasattr(self.catalog, "_step_index")
                    else 0,
                    "workspaces": len(self.catalog._workspace_steps)
                    if hasattr(self.catalog, "_workspace_steps")
                    else 0,
                },
            }
            ...
            if self.metrics["validations_performed"] > 0:
                summary["validation_success_rate"] = self.metrics[
