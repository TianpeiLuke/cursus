...
class WorkspaceIntegrator:
    ...
    def promote_component_to_core(
        self, step_name: str, source_workspace_id: str, dry_run: bool = True
    ) -> IntegrationResult:
        """
        ...
        try:
            ...
            if step_info.workspace_id != source_workspace_id:
                return IntegrationResult(
                    ...
                    details={
                        "step_name": step_name,
                        "actual_workspace": step_info.workspace_id,
                    },
                )
            ...
            if promotion_result.success:
                ...
                logger.info(
                    f"Successfully promoted component {step_name} to core package"
                )
    ...
    def _validate_promotion_readiness(self, step_info: Any) -> IntegrationResult:
        ...
        try:
            ...
            if inaccessible_files:
                validation_issues.append(
                    f"Inaccessible files: {', '.join(inaccessible_files)}"
                )
            ...
            if step_info.step_name in core_components:
                validation_issues.append(
                    f"Component name conflict: {step_info.step_name} already exists in core"
                )
            ...
            logger.error(
                f"Error validating promotion readiness for {step_info.step_name}: {e}"
            )
            return IntegrationResult(
                success=False,
        ...
        try:
            ...
            if not core_steps_dir.exists():
                return IntegrationResult(
                    ...
                    details={
                        "step_name": step_info.step_name,
                        "core_steps_dir": str(core_steps_dir),
                    },
                )
            ...
            for component_type, file_metadata in step_info.file_components.items():
                if file_metadata and hasattr(file_metadata, "path"):
                    ...
                    if source_path.exists():
                        ...
                        target_dir = (
                            core_steps_dir / f"{component_type}s"
                        )  # builders, configs, etc.
                        ...
                        try:
                            ...
                            copy_errors.append(
                                f"Failed to copy {source_path}: {str(copy_error)}"
                            )
    ...
    def integrate_cross_workspace_components(
        self, target_workspace_id: str, source_components: List[Dict[str, str]]
    ) -> IntegrationResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Cross-workspace integration completed: {successful_integrations}/{len(source_components)} successful"
            )
            return result
                    ...
                    if core_steps_dir in file_path.parents:
                        try:
                            ...
                            removal_errors.append(
                                f"Failed to remove {file_path}: {str(remove_error)}"
                            )
