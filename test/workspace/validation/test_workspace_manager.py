...
class TestWorkspaceManager:
    ...
    def _create_test_workspace_structure(self, workspace_root):
        ...
        for dev_id in ["developer_1", "developer_2"]:
            dev_dir = (
                workspace_root / "developers" / dev_id / "src" / "cursus_dev" / "steps"
            )
            ...
            # Create test files
            (dev_dir / "builders" / f"{dev_id}_builder.py").write_text(
                f"# {dev_id} builder"
            )
            (dev_dir / "contracts" / f"{dev_id}_contract.py").write_text(
                f"# {dev_id} contract"
            )
        ...
        (shared_dir / "contracts" / "shared_contract.py").write_text(
            "# Shared contract"
        )
    ...
    def test_init_with_config_file(self, temp_workspace):
        ...
        manager = WorkspaceManager(config_file=config_file, auto_discover=False)
    ...
    def test_create_developer_workspace(self, temp_workspace):
        ...
        dev_workspace = manager.create_developer_workspace(
            "new_developer", workspace_root=new_workspace_root, create_structure=True
        )
    ...
    def test_create_shared_workspace(self, temp_workspace):
        ...
        shared_workspace = manager.create_shared_workspace(
            workspace_root=new_workspace_root, create_structure=True
        )
    ...
    def test_save_config_yaml(self, mock_yaml_dump, temp_workspace):
        ...
        config_file = Path(temp_dir) / "workspace.yaml"
        config = WorkspaceConfig(
            workspace_root=str(workspace_root), developer_id="developer_1"
        )
    ...
    def test_get_workspace_summary(self, temp_workspace):
        ...
        dev1_summary = next(
            dev for dev in summary["developers"] if dev["developer_id"] == "developer_1"
        )
    ...
    def test_workspace_config_dataclass(self):
        ...
        config = WorkspaceConfig(
            workspace_root="/path/to/workspace", developer_id="test_dev"
        )
    ...
    def test_workspace_info_dataclass(self):
        ...
        workspace_info = WorkspaceInfo(
            workspace_root="/path/to/workspace", has_shared=True, total_developers=2
        )
