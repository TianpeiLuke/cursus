...
from cursus.workspace.validation.workspace_file_resolver import (
...
class TestDeveloperWorkspaceFileResolver:
    ...
    def _create_test_workspace_structure(self, workspace_root):
        ...
        (dev1_dir / "builders" / "builder_test_step_step.py").write_text(
            "# Test builder"
        )
        ...
        (shared_dir / "builders" / "builder_shared_step.py").write_text(
            "# Shared builder"
        )
    ...
    def test_validate_workspace_structure_invalid_root(self, temp_workspace):
        ...
        with pytest.raises(ValueError) as exc_info:
            DeveloperWorkspaceFileResolver(
                workspace_root=invalid_root, developer_id="developer_1"
            )
    ...
    def test_validate_workspace_structure_invalid_developer(self, temp_workspace):
        ...
        with pytest.raises(ValueError) as exc_info:
            DeveloperWorkspaceFileResolver(
                workspace_root=workspace_root, developer_id="nonexistent_developer"
            )
