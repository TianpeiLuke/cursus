...
class TestWorkspaceDAGCompiler:
    ...
    def test_compiler_initialization_without_workspace_manager(
    ...
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_compile_workspace_dag_success(
        self, mock_manager_class, temp_workspace, sample_workspace_dag
    ):
        ...
        with patch.object(
            sample_workspace_dag, "to_workspace_pipeline_config"
        ) as mock_to_config, patch.object(
            sample_workspace_dag, "validate_workspace_dependencies"
        ) as mock_validate_deps, patch.object(
            sample_workspace_dag, "analyze_workspace_complexity"
        ) as mock_analyze, patch.object(
            sample_workspace_dag, "get_developers"
        ) as mock_get_devs, patch.object(
            compiler, "WorkspacePipelineAssembler", create=True
            ...
            with patch(
                "cursus.workspace.core.compiler.WorkspacePipelineAssembler",
                mock_assembler_class,
            ):
                pipeline, metadata = compiler.compile_workspace_dag(
                    sample_workspace_dag
                )
    ...
    def test_compile_workspace_dag_failure(
        self,
        mock_assembler_class,
        mock_manager_class,
        mock_registry_class,
        ...
        mock_assembler = Mock()
        mock_assembler.assemble_workspace_pipeline.side_effect = Exception(
            "Assembly failed"
        )
        mock_assembler_class.return_value = mock_assembler
    ...
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_preview_workspace_resolution(
        self, mock_manager_class, temp_workspace, sample_workspace_dag
    ):
        ...
        with patch.object(
            ...
            sample_workspace_dag, "get_workspace_summary"
    ...
    @patch("cursus.workspace.core.registry.WorkspaceComponentRegistry")
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_estimate_compilation_time(
        self,
        mock_manager_class,
        mock_registry_class,
        temp_workspace,
    ...
    @patch("cursus.workspace.core.registry.WorkspaceComponentRegistry")
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_validate_workspace_components_failure(
        self,
        mock_manager_class,
        mock_registry_class,
        temp_workspace,
    ...
    @patch("cursus.workspace.core.registry.WorkspaceComponentRegistry")
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_generate_compilation_report(
        self,
        mock_manager_class,
        mock_registry_class,
        temp_workspace,
        ...
        with patch.object(
            ...
            compiler, "validate_workspace_components"
        ) as mock_validate, patch.object(
            compiler, "preview_workspace_resolution"
        ) as mock_preview:

            mock_summary.return_value = {"total_steps": 2}
            mock_analyze.return_value = {
                "basic_metrics": {"node_count": 2, "developer_count": 2},
                "complexity_metrics": {"avg_dependencies_per_step": 1.0},
            }
    ...
    def test_from_workspace_config(
        ...
        workspace_config = WorkspacePipelineDefinition(
            pipeline_name="test_pipeline", workspace_root=temp_workspace, steps=[step]
        )

        compiler = WorkspaceDAGCompiler.from_workspace_config(
            workspace_config=workspace_config, role="test-role"
        )
    ...
    def test_from_workspace_dag(
        ...
        compiler = WorkspaceDAGCompiler.from_workspace_dag(
            workspace_dag=sample_workspace_dag, role="test-role"
        )
    ...
    @patch("cursus.workspace.core.registry.WorkspaceComponentRegistry")
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_compile_with_detailed_report_success(
        self,
        mock_manager_class,
        mock_registry_class,
        temp_workspace,
        ...
        with patch.object(
            ...
            compiler, "compile_workspace_dag"
        ) as mock_compile:
            ...
            mock_compile.return_value = (
                mock_pipeline,
                {"compilation_time": 10.5, "step_count": 2, "developer_count": 2},
            )

            pipeline, detailed_report = compiler.compile_with_detailed_report(
                sample_workspace_dag
            )
    ...
    @patch("cursus.workspace.core.registry.WorkspaceComponentRegistry")
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_compile_with_detailed_report_not_ready(
        self,
        mock_manager_class,
        mock_registry_class,
        temp_workspace,
    ...
    @patch("cursus.workspace.core.registry.WorkspaceComponentRegistry")
    @patch("cursus.workspace.core.manager.WorkspaceManager")
    def test_preview_with_blocking_issues(
        self,
        mock_manager_class,
        mock_registry_class,
        temp_workspace,
