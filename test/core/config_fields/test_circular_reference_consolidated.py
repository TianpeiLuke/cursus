...
class TestCircularReferenceConsolidated(unittest.TestCase):
        ...
        try:
            ...
            if "additional_sources" in first_source:
                ...
                self.assertTrue(
                    ...
                    or len(additional) == 0
                    or any(
                        "__circular_ref__" in str(item) for item in additional if item
                    )
                )
    ...
    def test_list_items_avoid_false_positives(self):
        ...
        container = Container(name="test_container", items=[item1, item2, item3])
        ...
        if serialized is None or (
            isinstance(serialized, dict) and serialized.get("_serialization_error")
        ):
            # This is acceptable - the serializer detected circular references
            # Check that warning was logged
            warning_messages = [
                msg
                for msg in self.log_handler.messages
                if "circular" in msg.lower() or "error serializing" in msg.lower()
            ]
            self.assertTrue(
                len(warning_messages) > 0,
                "Should have logged circular reference warning",
            )
            return

        # If serialization succeeded, verify structure
        self.assertIsInstance(
            serialized, dict, "Serialized result should be a dictionary"
        )
    ...
    def test_true_circular_references_still_detected(self):
        ...
        if "parent_container" in serialized and serialized["parent_container"]:
            ...
            if isinstance(parent, dict) and "parent_container" in parent:
                ...
                self.assertTrue(
                    nested_parent is None or "__circular_ref__" in str(nested_parent)
                )
    ...
    def test_circular_reference_error_messages(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                or "parent_container" in error_msg
                or "cycle" in error_msg.lower()
            )
    ...
    def test_performance_with_large_structures(self):
        ...
        self.assertLess(
            duration, 5.0, f"Serialization took too long: {duration:.2f} seconds"
        )
