...
class TestConfigClassDetector(unittest.TestCase):
    ...
    def test_extract_class_names_from_metadata(self):
        ...
        result = ConfigClassDetector._extract_class_names(
            self.sample_config_data, logger
        )
    ...
    def test_extract_class_names_from_specific_configs(self):
        ...
        config_data = {
            "configuration": {
                "specific": {
                    "step1": {"__model_type__": "StepConfig1", "param": "value"},
                    "step2": {"__model_type__": "StepConfig2", "param": "value"},
                }
            }
        }
    ...
    def test_extract_class_names_partial_config(self):
        ...
        metadata_only = {"metadata": {"config_types": {"step1": "ConfigA"}}}
        ...
        specific_only = {
            "configuration": {"specific": {"step1": {"__model_type__": "ConfigB"}}}
        }
        ...
        invalid_config = {
            "configuration": {
                "specific": {
                    ...
                    "step2": {"no_model_type": "value"},  # Should be ignored
                    "step3": {"__model_type__": "ValidConfig"},  # Should be extracted
                }
            }
        }
    ...
    @patch(
        "cursus.core.config_fields.config_class_detector.build_complete_config_classes"
    )
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.is_file")
    def test_detect_from_json_success(
        self, mock_is_file, mock_file, mock_build_classes
    ):
    ...
    def test_detect_from_json_file_not_found(self, mock_is_file, mock_build_classes):
        ...
        self.assertTrue(
            any("Configuration file not found" in message for message in log.output)
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.is_file")
    def test_detect_from_json_invalid_json(
        self, mock_is_file, mock_file, mock_build_classes
    ):
        ...
        self.assertTrue(
            any(
                "Error reading or parsing configuration file" in message
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.is_file")
    def test_detect_from_json_no_class_names_found(
        self, mock_is_file, mock_file, mock_build_classes
    ):
        ...
        self.assertEqual(result, self.mock_complete_classes)
        self.assertTrue(
            any("No config class names found" in message for message in log.output)
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.is_file")
    def test_detect_from_json_missing_classes(
        self, mock_is_file, mock_file, mock_build_classes
    ):
        ...
        self.assertTrue(
            any(
                "Could not load" in message and "required classes" in message
                for message in log.output
            )
        )
    ...
    def test_from_config_store_success(self, mock_is_file, mock_file, mock_store):
        ...
        mock_store.get_class.side_effect = lambda name: self.mock_complete_classes.get(
            name
        )
        mock_store.registered_names.return_value = list(
            self.mock_complete_classes.keys()
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.is_file")
    def test_from_config_store_no_class_names(
        self, mock_is_file, mock_file, mock_store
    ):
    ...
    @patch(
        "cursus.core.config_fields.config_class_detector.ConfigClassDetector.detect_from_json"
    )
    ...
    def test_edge_case_malformed_metadata(self):
        ...
        malformed_config = {
            "metadata": {"config_types": "not_a_dict"}  # Should be a dict
        }
