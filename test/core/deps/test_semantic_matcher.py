...
class TestSemanticMatcher:
    ...
    def test_individual_similarity_metrics(self):
        ...
        # Token similarity
        assert matcher._calculate_token_similarity("training data", "data training") == 1.0  # Same tokens, different order = perfect match
        
        token_sim = matcher._calculate_token_similarity("training data", "training model")
    ...
    def test_domain_specific_synonyms(self):
        ...
        for word1, word2 in high_similarity_pairs:
            ...
            assert similarity > 0.5, f"High similarity synonyms '{word1}' and '{word2}' should have high similarity"
            
        # Check medium similarity pairs (>0.4)
        for word1, word2 in medium_similarity_pairs:
            similarity = matcher.calculate_similarity(word1, word2)
            assert similarity > 0.4, f"Medium similarity synonyms '{word1}' and '{word2}' should have medium similarity"
            
        # Check low similarity pairs (≥0.2)
        for word1, word2 in low_similarity_pairs:
            similarity = matcher.calculate_similarity(word1, word2)
            assert similarity >= 0.2, f"Low similarity synonyms '{word1}' and '{word2}' should have some similarity"
