...
class TestConfigResolver:
    ...
    @pytest.fixture
    def configs(self, base_config, data_load_config, preprocessing_config, training_config, eval_config):
    ...
    def test_job_type_matching(self, resolver, configs, preprocessing_config, eval_config):
        ...
        assert len(matches) == 3  # Should match data_load, preprocess, and train configs
        
        # Check that the preprocessing config has high confidence
        preprocess_match = next((m for m in matches if m[0] == preprocessing_config), None)
    ...
    def test_semantic_matching(self, resolver, configs, preprocessing_config, training_config, eval_config):
    ...
    def test_pattern_matching(self, resolver, configs, data_load_config, training_config):
    ...
    def test_resolve_config_map(self, dag_nodes, data_load_config, preprocessing_config, training_config, eval_config):
        ...
        def mock_pattern_match(node_name, configs):
            if node_name == "evaluation":
                return [(eval_config, 0.8, "pattern")]  # Increased from 0.6 to 0.8 to exceed threshold
            return []
    ...
    def test_resolve_single_node_direct_match(self, resolver, data_load_config):
        ...
        try:
            ...
            config, confidence, method = resolver._resolve_single_node("data_loading", {})
    ...
    def test_resolve_single_node_ambiguity(self, resolver, preprocessing_config, training_config):
        ...
        try:
            ...
            config, confidence, method = resolver._resolve_single_node("preprocessing", {})
    ...
    def test_preview_resolution(self, dag_nodes, data_load_config, preprocessing_config, training_config):
