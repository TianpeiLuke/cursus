...
class TestPipelineTemplateBase:
    ...
    def setup_mocks(self, config_data, mock_configs):
        ...
        with patch('builtins.open', create=True) as mock_open, \
             ...
             patch('cursus.steps.configs.utils.build_complete_config_classes') as mock_build_complete, \
             patch('cursus.core.assembler.pipeline_template_base.create_pipeline_components') as mock_create_components, \
             patch('cursus.core.assembler.pipeline_template_base.PipelineAssembler') as mock_assembler_cls, \
    ...
    def test_generate_pipeline(self, setup_mocks):
        ...
        mocks['mock_assembler'].generate_pipeline.assert_called_once_with('test-pipeline-v1-0')
    ...
    def test_store_pipeline_metadata(self, setup_mocks):
        ...
        assert template.pipeline_metadata['cradle_loading_requests'] == {'step1': {'request': 'data'}}
    ...
    def test_config_loading_error_handling(self, setup_mocks):
        ...
        mocks['mock_open'].return_value.__enter__.return_value.read.side_effect = Exception("File read error")
...
class TestPipelineAssembler:
    ...
    def setup_assembler_mocks(self):
        with patch('cursus.core.assembler.pipeline_assembler.CONFIG_STEP_REGISTRY') as mock_step_registry, \
             patch('cursus.core.assembler.pipeline_assembler.create_dependency_resolver') as mock_create_resolver, \
             patch('cursus.core.assembler.pipeline_assembler.Pipeline') as mock_pipeline_cls:
    ...
    def test_assembler_initialization(self, mock_dag, mock_configs, mock_step_builder_map, 
                                    mock_dependency_components, setup_assembler_mocks):
        ...
        assert assembler._registry_manager == mock_dependency_components['registry_manager']
        assert assembler._dependency_resolver == mock_dependency_components['dependency_resolver']
        
        # Verify step builders were initialized
        assert len(assembler.step_builders) == 3

    def test_assembler_validation_missing_configs(self, mock_dag, mock_step_builder_map, setup_assembler_mocks):
    ...
    def test_assembler_validation_missing_builders(self, mock_dag, mock_configs, setup_assembler_mocks):
    ...
    def test_generate_pipeline(self, mock_dag, mock_configs, mock_step_builder_map, 
                             mock_dependency_components, setup_assembler_mocks):
    ...
    def test_create_with_components_class_method(self, mock_dag, mock_configs, mock_step_builder_map, 
                                               mock_dependency_components, setup_assembler_mocks):
        """Test create_with_components class method."""
