...
class CoreTestRunner:
    ...
    def _parse_pytest_output(self, output: str) -> Tuple[int, int, int, int]:
        ...
        for line in reversed(lines):  # Start from the end to find the final summary
            ...
            if ('passed' in line or 'failed' in line or 'skipped' in line or 'error' in line) and 'in ' in line and 's' in line:
    ...
    def _print_module_result(self, result: TestResult):
        ...
        status = "✅ PASSED" if result.exit_code == 0 and result.failed == 0 and result.errors == 0 else "❌ FAILED"
        
        print(f"\n{result.module}: {status}")
        print(f"  Tests: {result.passed + result.failed + result.skipped + result.errors}")
    ...
    def save_report(self, summary: TestSummary, filename: str = "core_test_report.json"):
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run all tests for cursus.core package")
    ...
    parser.add_argument("--report", default="core_test_report.json", help="Report filename")
    ...
    if args.module:
        ...
        if args.module not in runner.core_modules:
            print(f"Error: Unknown module '{args.module}'. Available modules: {runner.core_modules}")
            return 1
        ...
        return 0 if result.exit_code == 0 and result.failed == 0 and result.errors == 0 else 1
    else:
        # Run all modules
