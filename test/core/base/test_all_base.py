...
def run_individual_test_modules():
    ...
    print("=" * 80)
    print(
        f"{'Module':<20} {'Total':<8} {'Passed':<8} {'Failed':<8} {'Errors':<8} {'Skipped':<8}"
    )
    print("-" * 80)
    ...
    for module_name, stats in results.items():
        ...
        print(
            f"{module_name:<20} {stats['total']:<8} {stats['passed']:<8} {stats['failed']:<8} {stats['errors']:<8} {stats['skipped']:<8}"
        )

    print("-" * 80)
    print(
        f"{'TOTAL':<20} {total_all:<8} {passed_all:<8} {failed_all:<8} {errors_all:<8} {skipped_all:<8}"
    )
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run tests for cursus.core.base classes with pytest"
    )
    parser.add_argument(
        "--individual",
        action="store_true",
