...
class CircularImportDetector:
    ...
    def test_import(self, module_name: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if (
                "circular import" in error_msg.lower()
                or "cannot import name" in error_msg.lower()
            ):
                cycle_info = f"Circular import in {module_name}: {error_msg}"
                self.circular_imports.append(
                    (module_name, self.import_stack + [module_name])
                )
                error = cycle_info
            else:
    ...
    def test_all_imports(self) -> Dict[str, any]:
        ...
        for module_name in modules:
            ...
            results["modules_tested"].append(
                {"module": module_name, "success": success, "error": error}
            )
...
class TestCircularImports(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.cursus_root = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "src", "cursus"
        )
    ...
    def test_no_circular_imports_in_cursus_package(self):
        ...
        successful_modules = [
            test["module"] for test in results["modules_tested"] if test["success"]
        ]
        ...
        failure_rate = (
            results["failed_imports"] / results["total_modules"]
    ...
    def test_step_modules_import_successfully(self):
        ...
        circular_failures = [
            (module, error)
            for module, error in failed_step_modules
            if "circular import" in error.lower()
            or "cannot import name" in error.lower()
        ]

        self.assertEqual(
            len(circular_failures),
            0,
...
def run_circular_import_tests():
    ...
    try:
        ...
        output_file = os.path.join(
            slipbox_test_dir, f"circular_import_test_output_{timestamp}.txt"
        )
