...
def run_specific_test_module(module_name, description):
    ...
    try:
        ...
        print(f"  {status_icon} {description}: {result.testsRun} tests, "
              f"{len(result.failures)} failures, {len(result.errors)} errors")
        
        return success, result.testsRun, len(result.failures), len(result.errors), len(result.skipped) if hasattr(result, 'skipped') else 0
...
def main():
    ...
    test_modules = [
        ...
        ('test_runtime_cli', 'Runtime CLI Tests'),
    ...
    for module_name, description in test_modules:
        success, tests, failures, errors, skipped = run_specific_test_module(module_name, description)
    ...
    if discovered_tests > total_tests:
        ...
        if result.testsRun > total_tests:
            ...
            additional_skipped = (len(result.skipped) if hasattr(result, 'skipped') else 0) - total_skipped
            ...
            status_icon = "✅" if additional_failures == 0 and additional_errors == 0 else "❌"
            print(f"  {status_icon} Additional tests: {additional_run} tests, "
