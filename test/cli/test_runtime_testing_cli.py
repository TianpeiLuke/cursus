...
class TestRuntimeTestingCLI(unittest.TestCase):
    ...
    def test_test_script_success(self, mock_tester_class):
        ...
        result = self.runner.invoke(
            runtime, ["test-script", "test_script", "--workspace-dir", self.temp_dir]
        )
    ...
    def test_test_compatibility_success(self, mock_tester_class):
        ...
        mock_tester = Mock()
        mock_tester_class.return_value = mock_tester
        mock_tester._generate_sample_data.return_value = {
            "col1": [1, 2],
            "col2": ["a", "b"],
        }
        mock_tester.test_data_compatibility.return_value = (
            self.mock_compatibility_result
        )
    ...
    def test_test_pipeline_success(self, mock_exists, mock_open, mock_tester_class):
        ...
        pipeline_result = {
            ...
            "data_flow_results": {"step1->step2": self.mock_compatibility_result},
            "errors": [],
        }
        ...
        result = self.runner.invoke(
            runtime,
            ["test-pipeline", "pipeline.json", "--workspace-dir", self.temp_dir],
        )
    ...
    def test_test_pipeline_file_not_found(self, mock_exists):
        ...
        result = self.runner.invoke(
            runtime,
            ["test-pipeline", "nonexistent.json", "--workspace-dir", self.temp_dir],
        )
    ...
    def test_exception_handling(self, mock_tester_class):
        ...
        result = self.runner.invoke(runtime, ["test-script", "test_script"])
...
class TestRuntimeTestingCLIIntegration(unittest.TestCase):
    ...
    def test_full_workflow_simulation(self, mock_tester_class):
        ...
        result1 = self.runner.invoke(
            runtime, ["test-script", "script_a", "--workspace-dir", self.temp_dir]
        )
        self.assertEqual(result1.exit_code, 0)

        result2 = self.runner.invoke(
            runtime, ["test-script", "script_b", "--workspace-dir", self.temp_dir]
        )
