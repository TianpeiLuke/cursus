...
class TestAlignmentCLI(unittest.TestCase):
    ...
    def test_validate_single_script_success(self):
        ...
        with patch(
            "cursus.cli.alignment_cli.UnifiedAlignmentTester"
    ...
    def test_validate_all_scripts_success(self):
        ...
        with patch(
            ...
            with patch("pathlib.Path.exists", return_value=True), patch(
                ...
                self.assertEqual(result.exit_code, 0)
                self.assertIn(
                    "🎉 All 3 scripts passed alignment validation!", result.output
                )
    ...
    def test_validate_all_scripts_json_output_with_summary(self):
        ...
        with patch(
            ...
            with patch("pathlib.Path.exists", return_value=True), patch(
                ...
                result = self.runner.invoke(
                    alignment,
                    ["validate-all", "--format", "json", "--output-dir", self.temp_dir],
                )
            ...
            "level4": {
                ...
                "config_analysis": {
                    ...
                    "default_values": {
                        ...
                        "model_fields": {"field1": "annotation=str required=True"},
                    },
                },
            },
        }
    ...
    def test_invalid_script_name(self):
        ...
        with patch(
            ...
            mock_tester = MagicMock()
            mock_tester_class.return_value = mock_tester
            mock_tester.validate_specific_script.side_effect = Exception(
                "Script not found"
            )
    ...
    def test_output_directory_creation(self):
        ...
        with patch(
            ...
            output_file = os.path.join(
                non_existent_dir, "payload_alignment_report.json"
            )
            self.assertTrue(os.path.exists(output_file))
    ...
    def test_argument_parsing_errors(self):
        ...
        # Test invalid format
        result = self.runner.invoke(
            alignment, ["validate", "payload", "--format", "invalid"]
        )
        self.assertEqual(result.exit_code, 2)  # click error
    ...
    def test_json_serialization_edge_cases(self):
        ...
        with patch(
            ...
            result = self.runner.invoke(
                alignment,
                ["validate", "test", "--format", "json", "--output-dir", self.temp_dir],
            )
...
class TestAlignmentCLIVisualization(unittest.TestCase):
    ...
    def test_visualize_single_script_success(self):
        ...
        with patch(
            ...
            "cursus.cli.alignment_cli.AlignmentScorer"
            ...
            mock_scorer.generate_chart.return_value = (
                Path(self.temp_dir) / "payload_alignment_chart.png"
            )
            mock_scorer.generate_scoring_report.return_value = {"score": 100.0}

            result = self.runner.invoke(
                alignment, ["visualize", "payload", "--output-dir", self.temp_dir]
            )
            ...
            self.assertIn(
                "✅ Visualization generation complete for payload!", result.output
            )
    ...
    def test_visualize_all_scripts_success(self):
        ...
        with patch(
            ...
            mock_scorer.get_level_scores.return_value = {}
            mock_scorer.generate_chart.return_value = (
                Path(self.temp_dir) / "test_chart.png"
            )
            mock_scorer.generate_scoring_report.return_value = {"score": 100.0}
            ...
            with patch("pathlib.Path.exists", return_value=True), patch(
                ...
                result = self.runner.invoke(
                    alignment, ["visualize-all", "--output-dir", self.temp_dir]
                )

                self.assertEqual(result.exit_code, 0)
                self.assertIn(
                    "🎉 All 2 visualizations generated successfully!", result.output
                )
    ...
    def test_visualize_all_scripts_with_errors(self):
        ...
        with patch(
            ...
            with patch("pathlib.Path.exists", return_value=True), patch(
                ...
                self.assertIn(
                    "❌ Failed to generate visualization for payload", result.output
                )
    ...
    def test_visualize_chart_generation_failure(self):
        ...
        with patch(
            ...
            mock_scorer.get_quality_rating.return_value = "Excellent"
            mock_scorer.get_level_scores.return_value = {}
            mock_scorer.generate_chart.side_effect = Exception(
            ...
            self.assertEqual(result.exit_code, 1)
            self.assertIn(
                "❌ Error generating visualization for payload", result.output
            )
    ...
    def test_show_scoring_flag_integration(self):
        ...
        with patch(
            ...
            result = self.runner.invoke(
                alignment, ["validate", "payload", "--show-scoring"]
            )
    ...
    def test_visualize_invalid_script(self):
        ...
        with patch(
            ...
            result = self.runner.invoke(
                alignment,
                ["visualize", "nonexistent_script", "--output-dir", self.temp_dir],
            )

            self.assertEqual(result.exit_code, 1)
            self.assertIn(
                "❌ Error generating visualization for nonexistent_script",
                result.output,
            )
    ...
    def test_visualize_output_directory_creation(self):
        ...
        with patch(
            ...
            mock_scorer.get_level_scores.return_value = {}
            mock_scorer.generate_chart.return_value = (
                Path(non_existent_dir) / "payload_alignment_chart.png"
            )
            mock_scorer.generate_scoring_report.return_value = {"score": 100.0}

            result = self.runner.invoke(
                alignment, ["visualize", "payload", "--output-dir", non_existent_dir]
            )
    ...
    def test_visualize_all_no_scripts_found(self):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            ...
            self.assertIn(
                "❌ Fatal error during visualization generation:", result.output
            )
    ...
    def test_visualize_all_continue_on_error_disabled(self):
        ...
        with patch(
            ...
            with patch("pathlib.Path.exists", return_value=True), patch(
                ...
                self.assertIn(
                    "❌ Failed to generate visualization for package", result.output
                )
...
class TestAlignmentCLIScoring(unittest.TestCase):
    ...
    def test_scoring_with_partial_failures(self):
        ...
        mock_result = {
            ...
            "level2": {
                "passed": False,
                "issues": [{"severity": "ERROR", "message": "Level 2 error"}],
            },
            "level3": {"passed": True, "issues": []},
            "level4": {
                "passed": False,
                "issues": [{"severity": "WARNING", "message": "Level 4 warning"}],
            },
        }
...
class TestAlignmentCLIIntegration(unittest.TestCase):
    ...
    def test_cli_with_real_unified_tester(self):
        ...
        with patch(
            "cursus.validation.alignment.script_contract_alignment.ScriptContractAlignmentTester"
        ), patch(
            "cursus.validation.alignment.contract_spec_alignment.ContractSpecificationAlignmentTester"
        ), patch(
            "cursus.validation.alignment.spec_dependency_alignment.SpecificationDependencyAlignmentTester"
        ), patch(
            "cursus.validation.alignment.builder_config_alignment.BuilderConfigurationAlignmentTester"
        ):
