...
class TestPipelineRecommendationEngineCore:
    ...
    def mock_traverser(self):
        ...
        connections_data = {
            "xgb_simple": {
                "alternatives": [
                    PipelineConnection(
                        target_id="pytorch_nlp",
                        connection_type="alternatives",
                        annotation="Alternative framework",
                        source_id="xgb_simple",
                    )
                ],
                "related": [
                    PipelineConnection(
                        target_id="sklearn_prep",
                ...
                "used_in": [
                    PipelineConnection(
                        target_id="xgb_advanced",
                        connection_type="used_in",
                        annotation="Extended workflow",
                        source_id="xgb_simple",
                    )
                ],
            },
            "xgb_advanced": {"alternatives": [], "related": [], "used_in": []},
            "pytorch_nlp": {
                "alternatives": [
                    PipelineConnection(
                        target_id="xgb_simple",
                        connection_type="alternatives",
                        annotation="Alternative approach",
                        source_id="pytorch_nlp",
                    )
                ],
                "related": [],
                "used_in": [],
            ...
            "sklearn_prep": {
                ...
                "related": [
                    PipelineConnection(
                        target_id="xgb_simple",
                        connection_type="related",
                        annotation="Follows preprocessing",
                        source_id="sklearn_prep",
                    )
                ],
                "used_in": [],
            },
        ...
        def mock_get_all_connections(pipeline_id):
            return connections_data.get(
                pipeline_id, {"alternatives": [], "related": [], "used_in": []}
            )
    ...
    def engine(self, mock_registry, mock_traverser, mock_discovery):
        ...
        return PipelineRecommendationEngine(
            mock_registry, mock_traverser, mock_discovery
        )
    ...
    def test_recommend_for_use_case_with_constraints(self, engine):
        ...
        constraints = {"framework": "xgboost", "complexity": "simple"}
    ...
    def test_recommend_next_steps(self, engine):
        ...
        used_in_recs = [
            rec
            for rec in recommendations
            if rec.connection_path and len(rec.connection_path) == 2
        ]
        if used_in_recs:
    ...
    def test_recommend_next_steps_complexity_progression(self, engine):
        ...
        complex_recs = [
            rec for rec in recommendations if rec.complexity in ["standard", "advanced"]
        ]
    ...
    def test_get_learning_path(self, engine):
        ...
        learning_path = engine.get_learning_path(
            start_complexity="simple", target_framework="xgboost"
        )
    ...
    def test_get_learning_path_any_framework(self, engine):
        ...
        learning_path = engine.get_learning_path(
            start_complexity="simple", target_framework="any"
        )
    ...
    def test_meets_constraints(self, engine):
        ...
        assert not engine._meets_constraints(
            "test_pipeline", {"complexity": "advanced"}
        )
    ...
    def test_meets_constraints_no_node(self, engine):
        ...
        result = engine._meets_constraints(
            "nonexistent_pipeline", {"framework": "xgboost"}
        )
        assert result is False

    def test_is_logical_progression(self, engine):
        """Test logical progression detection between pipelines."""
        current_node = {"multi_dimensional_tags": {"task_tags": ["training"]}}

        next_node = {"multi_dimensional_tags": {"task_tags": ["evaluation"]}}
        ...
        same_node = {"multi_dimensional_tags": {"task_tags": ["training"]}}
    ...
    def test_is_simpler(self, engine):
        ...
        simple_node = {"zettelkasten_metadata": {"complexity": "simple"}}

        advanced_node = {"zettelkasten_metadata": {"complexity": "advanced"}}
