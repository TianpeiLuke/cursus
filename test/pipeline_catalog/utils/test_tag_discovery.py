...
class TestTagBasedDiscovery:
    ...
    def test_find_by_tags_exact_match(self, discovery):
        ...
        with patch.object(
            discovery, "_get_pipeline_tags", side_effect=mock_get_pipeline_tags
        ):
            with patch.object(
                discovery.registry,
                "get_all_pipelines",
                return_value=["xgb_simple", "pytorch_basic", "test_exact"],
            ):
                results = discovery.find_by_tags(
                    ["training", "simple"], match_mode="exact"
                )
    ...
    def test_find_by_multiple_criteria(self, discovery):
        ...
        results = discovery.find_by_multiple_criteria(
            complexity="simple", additional_tags=["training"]
        )
    ...
    def test_suggest_similar_pipelines(self, discovery):
        ...
        similar = discovery.suggest_similar_pipelines(
            "xgb_simple", similarity_threshold=0.2
        )
    ...
    def test_are_similar_tags(self, discovery):
        ...
        assert discovery._are_similar_tags(
...
class TestTagBasedDiscoveryIntegration:
    ...
    def test_comprehensive_search_workflow(self):
        ...
        registry.get_pipeline_node.side_effect = lambda pid: registry_data["nodes"].get(
            pid
        )
        ...
        advanced_tabular = discovery.find_by_multiple_criteria(
            complexity="advanced", domain="tabular"
        )
        ...
        assert (
            stats["pipeline_tag_counts"]["min"] >= 4
        )  # Each pipeline has at least 4 tags
