    ...
    def mock_registry_good_compliance(self):
        ...
        good_nodes = {
            f"pipeline_{i}": {
                ...
                "connections": {
                    "alternatives": [{"id": f"pipeline_{(i+1)%5}", "annotation": f"Alternative to {i}"}] if i < 4 else [],
                    "related": [{"id": f"pipeline_{(i+2)%5}", "annotation": f"Related to {i}"}] if i < 3 else [],
                    "used_in": []
                },
                "discovery_metadata": {
        ...
        registry_data = {
            ...
            "tag_index": {
                ...
                "task_tags": {
                    "training": [f"pipeline_{i}" for i in range(5)]
                },
                "complexity_tags": {
        ...
        registry.get_pipeline_connections.side_effect = lambda pid: good_nodes.get(pid, {}).get("connections", {})
        registry.load_registry.return_value = registry_data
        
        return registry
    
    def test_validate_zettelkasten_principles_good_compliance(self, mock_registry_good_compliance):
...
class TestRegistryValidatorIntegration:
    ...
    def test_comprehensive_validation_workflow(self):
        ...
        mixed_nodes = {
            ...
            "problematic_pipeline": {
                ...
                "atomic_properties": {
                    ...
                    "dependencies": ["dep1", "dep2", "dep3", "dep4", "dep5"]  # Too many deps
                },
                "zettelkasten_metadata": {
                    "framework": "pytorch",
                    "complexity": "advanced"
                ...
                "connections": {
                    ...
                    "used_in": [{"id": "problematic_pipeline", "annotation": "Self-reference"}]
                }
        ...
        registry_data = {
            ...
            "tag_index": {
                "framework_tags": {
                    "xgboost": ["valid_pipeline", "problematic_pipeline"],  # Inconsistent
                    "pytorch": [],
                ...
                "complexity_tags": {
                    "simple": ["valid_pipeline", "problematic_pipeline"],  # Inconsistent
                    "advanced": []
                }
            }
        }
        ...
        registry.get_pipeline_connections.side_effect = lambda pid: mixed_nodes.get(pid, {}).get("connections", {})
        registry.load_registry.return_value = registry_data
    ...
    def test_validation_workflow_with_edge_cases(self):
        ...
        edge_case_nodes = {
            ...
            "maximal_pipeline": {
                ...
                "atomic_properties": {
                    ...
                    "output_interface": [f"output_{i}" for i in range(50)],  # Many outputs
                    "independence": "fully_self_contained",
                    "side_effects": "none",
                    "dependencies": [f"dep_{i}" for i in range(20)]  # Many dependencies
                },
                "zettelkasten_metadata": {
                ...
                "connections": {
                    "alternatives": [{"id": f"alt_{i}", "annotation": f"Alt {i}"} for i in range(10)],
                    "related": [{"id": f"rel_{i}", "annotation": f"Rel {i}"} for i in range(15)],
                    "used_in": [{"id": f"used_{i}", "annotation": f"Used {i}"} for i in range(5)]
                },
                "discovery_metadata": {
            ...
            "duplicate_connections": {
                ...
                "connections": {
                    "alternatives": [
                        ...
                        {"id": "target", "annotation": "Duplicate"}  # Duplicate connection
                    ],
                    "related": [],
                    "used_in": []
                }
        ...
        registry_data = {
            ...
            "tag_index": {
                "framework_tags": {
                    "test": ["duplicate_connections"]
                },
                "task_tags": {
                    "testing": ["duplicate_connections"]
                }
            }
        }
        ...
        registry.get_pipeline_connections.side_effect = lambda pid: edge_case_nodes.get(pid, {}).get("connections", {})
        registry.load_registry.return_value = registry_data
        ...
        minimal_issues = [issue for issue in report.all_issues if issue.pipeline_id == "minimal_pipeline"]
        assert len(minimal_issues) > 0  # Should have issues with minimal data
        
        # Should handle maximal pipeline without crashing
        maximal_issues = [issue for issue in report.all_issues if issue.pipeline_id == "maximal_pipeline"]
        # May or may not have issues, but should not crash
        
        # Should detect duplicate connections
        duplicate_issues = [issue for issue in report.all_issues if issue.pipeline_id == "duplicate_connections"]
    ...
    def test_performance_with_large_registry(self):
        ...
        for i in range(100):
            large_nodes[f"pipeline_{i}"] = {
                ...
                "connections": {
                    "alternatives": [{"id": f"pipeline_{(i+1)%100}", "annotation": f"Alt to {i}"}] if i % 10 == 0 else [],
                    "related": [{"id": f"pipeline_{(i+2)%100}", "annotation": f"Related to {i}"}] if i % 5 == 0 else [],
                    "used_in": []
                },
                "discovery_metadata": {
                    "estimated_runtime": "10 minutes",
        ...
        registry_data = {
            ...
            "tag_index": {
                ...
                "task_tags": {
                    "training": [f"pipeline_{i}" for i in range(100)]
                },
                "complexity_tags": {
        ...
        registry.get_pipeline_connections.side_effect = lambda pid: large_nodes.get(pid, {}).get("connections", {})
        registry.load_registry.return_value = registry_data
