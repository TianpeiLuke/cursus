...
class TestCatalogRegistry:
                            ...
                            "related": [
                                {
                                    ...
                                    "annotation": "Similar training approach",
                                }
                            ],
                            "used_in": [],
                        },
    ...
    def test_add_or_update_enhanced_node_failure(
        self, registry, sample_enhanced_metadata
    ):
    ...
    def test_remove_connection(self, registry):
        ...
        result = registry.remove_connection(
            "xgboost_training_simple", "pytorch_training_simple", "alternatives"
        )
        assert result is True
    ...
    def test_remove_connection_not_found(self, registry):
        ...
        result = registry.remove_connection(
            "xgboost_training_simple", "nonexistent_target", "related"
        )
        assert result is False
    ...
    def test_validate_registry_integrity_with_orphaned_connection(self, registry):
        ...
        assert len(validation_result["errors"]) > 0
        assert any(
            "Orphaned connection" in error for error in validation_result["errors"]
        )
    ...
    def test_convert_zettelkasten_to_node_data(
        self, registry, sample_zettelkasten_metadata
    ):
        """Test converting ZettelkastenMetadata to node data format."""
        node_data = registry._convert_zettelkasten_to_node_data(
            sample_zettelkasten_metadata
        )

        assert node_data["id"] == sample_zettelkasten_metadata.atomic_id
        assert (
            node_data["description"]
            == sample_zettelkasten_metadata.single_responsibility
        )
        assert (
            node_data["zettelkasten_metadata"]["framework"]
            == sample_zettelkasten_metadata.framework
        )
        assert (
            node_data["multi_dimensional_tags"]["framework_tags"]
            == sample_zettelkasten_metadata.framework_tags
        )
    ...
    def test_error_handling(self, mock_logger, registry):
        ...
        with patch.object(
            registry, "load_registry", side_effect=Exception("Load failed")
        ):
    ...
    def test_remove_connections_to_pipeline(self, registry):
        ...
        xgboost_connections = registry._cache["nodes"]["xgboost_training_simple"][
            "connections"
        ]
