...
class TestEnhancedDAGMetadataIntegration:
    ...
    def test_complete_workflow(self):
        ...
        assert (
            connections["alternatives"][0]["annotation"]
            == "Simpler version without evaluation"
        )

        assert len(connections["used_in"]) == 1
        assert connections["used_in"][0]["id"] == "model_deployment_pipeline"
        assert (
            connections["used_in"][0]["annotation"]
            == "Deploy trained and calibrated model"
        )
        ...
        assert data_dict["zettelkasten_metadata"]["quality_tags"] == [
            "production_ready",
            "well_tested",
        ]
    ...
    def test_pipeline_evolution_scenario(self):
        ...
        assert (
            simple_node["connections"]["evolved_to"][0]["id"]
            == "xgboost_training_standard"
        )

        assert len(standard_node["connections"]["evolved_from"]) == 1
        assert (
            standard_node["connections"]["evolved_from"][0]["id"]
            == "xgboost_training_simple"
        )
        assert len(standard_node["connections"]["evolved_to"]) == 1
        assert (
            standard_node["connections"]["evolved_to"][0]["id"]
            == "xgboost_training_advanced"
        )

        assert len(advanced_node["connections"]["evolved_from"]) == 1
        assert (
            advanced_node["connections"]["evolved_from"][0]["id"]
            == "xgboost_training_standard"
        )
