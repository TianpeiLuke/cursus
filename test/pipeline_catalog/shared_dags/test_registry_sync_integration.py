...
class TestRegistrySyncIntegration:
    ...
    def test_multi_pipeline_sync_workflow(self, temp_registry_path):
        ...
        sync.sync_metadata_to_registry(
            pytorch_pipeline, "pipelines/pytorch_pipeline.py"
        )
        ...
        pytorch_node = registry["nodes"][
            pytorch_pipeline.zettelkasten_metadata.atomic_id
        ]
        ...
        ), f"Registry validation errors: {validation_errors}"
    ...
    def test_complex_connection_workflow(self, temp_registry_path):
        ...
        preprocessing.zettelkasten_metadata.domain_tags = [
            "preprocessing",
            "feature_engineering",
        ]
        ...
        xgb_training.zettelkasten_metadata.domain_tags = [
            "training",
            "gradient_boosting",
        ]
        ...
        pytorch_training.zettelkasten_metadata.domain_tags = [
            "training",
            "deep_learning",
        ]
        ...
        evaluation.zettelkasten_metadata.domain_tags = [
            "evaluation",
            "model_comparison",
        ]
        ...
        assert (
            len(prep_node["connections"]["used_in"]) == 2
        )  # Used by both training pipelines

        # Check training pipeline connections
        xgb_node = registry["nodes"][xgb_training.zettelkasten_metadata.atomic_id]
        assert (
            len(xgb_node["connections"]["depends_on"]) == 1
        )  # Depends on preprocessing
        assert (
            len(xgb_node["connections"]["alternatives"]) == 1
        )  # Alternative to PyTorch
        assert (
            len(xgb_node["connections"]["used_in"]) == 2
        )  # Used in evaluation and deployment

        pytorch_node = registry["nodes"][
            pytorch_training.zettelkasten_metadata.atomic_id
        ]
        assert (
            len(pytorch_node["connections"]["depends_on"]) == 1
        )  # Depends on preprocessing
        assert (
            len(pytorch_node["connections"]["alternatives"]) == 1
        )  # Alternative to XGBoost
        assert (
            len(pytorch_node["connections"]["used_in"]) == 2
        )  # Used in evaluation and deployment

        # Check evaluation connections
        eval_node = registry["nodes"][evaluation.zettelkasten_metadata.atomic_id]
        assert (
            len(eval_node["connections"]["depends_on"]) == 2
        )  # Depends on both training pipelines
        ...
        assert (
            len(deploy_node["connections"]["depends_on"]) == 1
        )  # Depends on evaluation
        assert (
            len(deploy_node["connections"]["used_in"]) == 2
        )  # Uses both training pipelines
