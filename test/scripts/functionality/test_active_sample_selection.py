...
class TestScoreColumnExtraction:
    ...
    def test_extract_explicit_score_field(self, sample_dataframe):
        ...
        score_cols = extract_score_columns(
            sample_dataframe, score_field="confidence_score"
        )
    ...
    def test_extract_ruleset_format(self):
        ...
        df = pd.DataFrame(
            {"id": ["1", "2"], "rule_score": [0.9, 0.8], "text": ["sample1", "sample2"]}
        )
    ...
    def test_extract_no_valid_columns(self):
        ...
        df = pd.DataFrame({"id": ["1", "2"], "text": ["sample1", "sample2"]})
...
class TestScoreNormalization:
    ...
    def test_normalize_already_normalized(self, sample_dataframe):
        ...
        for col in score_cols:
            ...
            np.testing.assert_array_almost_equal(
                sample_dataframe[col].values, df_norm[col].values, decimal=5
            )
    ...
    def test_normalize_single_score(self):
        ...
        df = pd.DataFrame({"id": ["1", "2", "3"], "confidence": [0.8, 0.9, 0.7]})
...
class TestSelectSamples:
    ...
    def test_top_k_per_class_strategy(self, sample_dataframe):
        ...
        strategy_config = {"k_per_class": 5, "random_seed": 42}
    ...
    def test_no_prob_columns(self):
        ...
        df = pd.DataFrame(
            {"id": ["1", "2", "3"], "text": ["sample1", "sample2", "sample3"]}
        )
...
class TestOutputManagement:
    ...
    def test_save_selection_metadata(self, temp_data_dir):
        ...
        metadata_path = save_selection_metadata(
            metadata=metadata, metadata_dir=str(temp_data_dir)
        )
...
class TestEdgeCases:
    ...
    def test_empty_dataframe(self):
        ...
        df = pd.DataFrame(
            {"id": [], "prob_class_0": [], "prob_class_1": [], "prob_class_2": []}
        )
...
class TestMainFunction:
    ...
    def test_main_confidence_threshold_strategy(self, temp_data_dir, sample_csv_file):
        ...
        input_paths = {"evaluation_data": str(sample_csv_file.parent)}
    ...
    def test_main_uncertainty_strategy(self, temp_data_dir, sample_csv_file):
        ...
        # Verify outputs
        output_files = list(
            Path(output_paths["selected_samples"]).glob("selected_samples.*")
        )
        assert len(output_files) > 0
    ...
    def test_main_format_preservation(self, temp_data_dir, sample_parquet_file):
        ...
        input_paths = {"evaluation_data": str(sample_parquet_file.parent)}
...
class TestPerformanceAndScalability:
    ...
    def test_large_dataset_selection(self):
        ...
        df = pd.DataFrame({"id": [f"sample_{i}" for i in range(n_samples)]})
