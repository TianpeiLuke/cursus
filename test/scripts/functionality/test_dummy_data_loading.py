...
class TestGenerateMetadata:
    ...
    def test_generate_metadata_single_row(self):
        ...
        df = pd.DataFrame({"col1": [42], "col2": ["single_value"]})
    ...
    def test_generate_metadata_all_null_column(self):
        ...
        df = pd.DataFrame({"all_null": [None, None, None], "normal_col": [1, 2, 3]})
    ...
    def test_generate_metadata_with_complex_types(self):
        """Test generating metadata with unhashable/complex types (lists, dicts from Parquet)."""
        # Create DataFrame with complex types that can come from Parquet files
        df = pd.DataFrame(
            {
        ...
        # Check that complex type columns are handled gracefully
        list_col_info = result["column_info"]["list_col"]
        dict_col_info = result["column_info"]["dict_col"]

        # Unique count should be None for unhashable types
        assert list_col_info["unique_count"] is None
        assert dict_col_info["unique_count"] is None

        # Should have flag indicating complex types
        assert list_col_info.get("contains_complex_types") is True
        assert dict_col_info.get("contains_complex_types") is True

        # Normal column should work fine
        normal_col_info = result["column_info"]["normal_col"]
        assert normal_col_info["unique_count"] == 3
        assert "contains_complex_types" not in normal_col_info
...
class TestProcessDataFiles:
    ...
    def create_sample_data_files(self, temp_dir):
        ...
        # Create Parquet file
        df2 = pd.DataFrame(
            {"id": [4, 5, 6], "name": ["David", "Eve", "Frank"], "value": [40, 50, 60]}
        )
...
class TestWriteOutputFiles:
    ...
    def test_write_single_data_file_csv(self, temp_dir):
        """Test write_single_data_file function with CSV format."""
        test_df = pd.DataFrame({"col1": [1, 2, 3], "col2": ["a", "b", "c"]})

        result_path = write_single_data_file(test_df, temp_dir, "CSV")
    ...
    def test_write_single_data_file_json(self, temp_dir):
        """Test write_single_data_file function with JSON format."""
        test_df = pd.DataFrame({"col1": [1, 2, 3], "col2": ["a", "b", "c"]})

        result_path = write_single_data_file(test_df, temp_dir, "JSON")

        assert result_path.exists()
        assert result_path.name == "part-00000.json"

        # Verify content
        df_read = pd.read_json(result_path, lines=True)
        assert len(df_read) == 3
        assert set(df_read.columns) == set(test_df.columns)

    def test_write_single_data_file_parquet(self, temp_dir):
        """Test write_single_data_file function with Parquet format."""
        test_df = pd.DataFrame({"col1": [1, 2, 3], "col2": ["a", "b", "c"]})

        result_path = write_single_data_file(test_df, temp_dir, "PARQUET")

        assert result_path.exists()
        assert result_path.name == "part-00000.parquet"

        # Verify content
        df_read = pd.read_parquet(result_path)
        pd.testing.assert_frame_equal(df_read, test_df)

    def test_write_single_data_file_unsupported_format(self, temp_dir):
        """Test write_single_data_file with unsupported format."""
        test_df = pd.DataFrame({"col1": [1, 2, 3], "col2": ["a", "b", "c"]})

        with pytest.raises(ValueError, match="Unsupported output format: XML"):
            write_single_data_file(test_df, temp_dir, "XML")

    def test_write_data_placeholder(self, temp_dir):
        """Test writing data in legacy mode (single file) via write_data_output."""
...
class TestCommonFailurePatterns:
    ...
    def test_corrupted_parquet_file(self, temp_dir):
        ...
        with pytest.raises(
            Exception
        ):  # pandas will raise various exceptions for invalid parquet
    ...
    def test_inconsistent_row_lengths(self, temp_dir):
        ...
        try:
            ...
            assert "Error tokenizing data" in str(e) or "ParserError" in str(
                type(e).__name__
            )
...
class TestEnhancedDataSharding:
    ...
    def test_write_data_output_enhanced_mode(self, temp_dir, sample_data):
        ...
        result = write_data_output(
            sample_data, temp_dir, write_shards=True, shard_size=3, output_format="CSV"
        )
...
class TestTabularPreprocessingCompatibility:
    ...
    def test_end_to_end_compatibility(self, temp_dir):
        ...
        pd.testing.assert_frame_equal(
            combined_df.sort_values("id").reset_index(drop=True),
