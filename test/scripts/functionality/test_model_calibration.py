...
class TestDataLoading:
    ...
    def test_find_first_data_file_no_files(self, setup_temp_dir, sample_config):
        ...
        with pytest.raises(FileNotFoundError, match="No supported data file \\(\\.csv, \\.parquet, \\.json\\) found"):
...
class TestCalibrationTraining:
    ...
    def test_train_gam_calibration_with_pygam(self, mock_gam_class, binary_calibration_data, sample_config):
        """Test GAM calibration training when pygam is available."""
        scores, labels = binary_calibration_data
        
        mock_gam = MagicMock()
        mock_gam.statistics_ = {'deviance': 1.5}
        mock_gam.predict_proba.return_value = np.linspace(0, 1, sample_config.calibration_sample_points)
    ...
    def test_train_gam_calibration_without_pygam(self, binary_calibration_data, sample_config):
...
class TestMulticlassCalibration:
    ...
    def test_train_multiclass_calibration_isotonic(self, multiclass_data, multiclass_config):
...
class TestCalibrationMethods:
    ...
    def test_isotonic_regression_improves_calibration(self, calibration_data):
        ...
        assert metrics_uncal["expected_calibration_error"] >= metrics_cal["expected_calibration_error"] - 0.05
