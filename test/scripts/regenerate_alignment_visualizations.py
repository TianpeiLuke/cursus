...
def regenerate_visualizations():
    ...
    for report_file in sorted(report_files):
        try:
            ...
            print(
                f"Pass Rate: {pass_rate:.1f}% ({scoring_report['overall']['passed']}/{scoring_report['overall']['total']} tests)"
            )
            ...
            scoring_report_path = (
                reports_dir / f"{script_name}_alignment_score_report.json"
            )
    ...
    if success_count == total_count:
        print(
            "🎉 All alignment visualizations have been successfully regenerated with corrected scoring!"
        )
    else:
