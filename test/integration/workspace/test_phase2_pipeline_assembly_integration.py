...
class TestWorkspacePipelineAssemblerIntegration:
    ...
    def test_assembler_initialization_with_consolidated_manager(
        self, temp_workspace, consolidated_workspace_manager
    ):
        ...
        assert (
            assembler.lifecycle_manager
            is consolidated_workspace_manager.lifecycle_manager
        )
        assert (
            assembler.isolation_manager
            is consolidated_workspace_manager.isolation_manager
        )
        assert (
            assembler.integration_manager
            is consolidated_workspace_manager.integration_manager
        )

        # Verify registry integration
        assert (
            assembler.workspace_registry.discovery_manager
            is consolidated_workspace_manager.discovery_manager
        )
    ...
    def test_enhanced_validation_integration(
        self, temp_workspace, consolidated_workspace_manager, sample_workspace_config
    ):
        ...
        with patch.object(
            assembler.isolation_manager, "validate_workspace_boundaries_for_pipeline"
        ) as mock_isolation, patch.object(
            assembler.workspace_registry, "validate_component_availability"
        ) as mock_registry:
    ...
    def test_cross_workspace_dependency_resolution(
        self, temp_workspace, consolidated_workspace_manager, sample_workspace_config
    ):
        ...
        with patch.object(
            assembler.workspace_manager.discovery_manager,
            "resolve_pipeline_dependencies",
        ) as mock_resolve:
            mock_resolve.return_value = {
                "valid": True,
                "resolved_dependencies": {"model_training": ["data_preprocessing"]},
                "dependency_graph": {
                    "data_preprocessing": [],
...
class TestWorkspaceComponentRegistryIntegration:
    ...
    def test_registry_initialization_with_discovery_manager(
        self, temp_workspace, consolidated_workspace_manager
    ):
        ...
        registry = WorkspaceComponentRegistry(
            workspace_root=temp_workspace, discovery_manager=discovery_manager
        )
    ...
    def test_registry_initialization_without_discovery_manager(self, temp_workspace):
        ...
        assert (
            registry.discovery_manager.workspace_manager is registry.workspace_manager
        )

    def test_enhanced_caching_integration(
        self, temp_workspace, consolidated_workspace_manager
    ):
    ...
    def test_component_discovery_delegation(
        self, temp_workspace, consolidated_workspace_manager
    ):
        ...
        with patch.object(
            registry.discovery_manager, "discover_all_workspace_components"
        ) as mock_discover:
...
class TestConfigurationModelsIntegration:
    ...
    def test_step_definition_validation_integration(
        self, temp_workspace, consolidated_workspace_manager
    ):
        ...
        with patch.object(
            consolidated_workspace_manager.isolation_manager, "validate_step_definition"
        ) as mock_isolation, patch.object(
            consolidated_workspace_manager.lifecycle_manager, "validate_step_lifecycle"
        ) as mock_lifecycle:
            ...
            # Test validation
            result = step.validate_with_workspace_manager(
                consolidated_workspace_manager
            )
    ...
    def test_step_dependency_resolution(
        ...
        # Mock discovery manager
        with patch.object(
            consolidated_workspace_manager.discovery_manager,
    ...
    def test_pipeline_definition_comprehensive_validation(
        self, temp_workspace, consolidated_workspace_manager, sample_workspace_config
    ):
        """Test pipeline definition comprehensive validation with all managers."""
        # Mock all specialized managers
        with patch.object(
            consolidated_workspace_manager.lifecycle_manager,
            "validate_pipeline_lifecycle",
        ) as mock_lifecycle, patch.object(
            consolidated_workspace_manager.isolation_manager,
            "validate_pipeline_isolation",
        ) as mock_isolation, patch.object(
            consolidated_workspace_manager.discovery_manager,
            "validate_pipeline_dependencies",
        ) as mock_discovery, patch.object(
            consolidated_workspace_manager.integration_manager,
            ...
            result = sample_workspace_config.validate_with_consolidated_managers(
                consolidated_workspace_manager
            )
    ...
    def test_pipeline_integration_preparation(
        self, temp_workspace, consolidated_workspace_manager, sample_workspace_config
    ):
        ...
        with patch.object(
            consolidated_workspace_manager.integration_manager,
            "prepare_pipeline_for_integration",
        ) as mock_prepare:
            ...
            result = sample_workspace_config.prepare_for_integration(
                consolidated_workspace_manager
            )
...
class TestWorkspaceDAGCompilerIntegration:
    ...
    def test_compiler_initialization_with_consolidated_manager(
        self, temp_workspace, consolidated_workspace_manager
    ):
        ...
        assert (
            compiler.lifecycle_manager
            is consolidated_workspace_manager.lifecycle_manager
        )
        assert (
            compiler.isolation_manager
            is consolidated_workspace_manager.isolation_manager
        )
        assert (
            compiler.integration_manager
            is consolidated_workspace_manager.integration_manager
        )

        # Verify registry integration
        assert (
            compiler.workspace_registry.discovery_manager
            is consolidated_workspace_manager.discovery_manager
        )
    ...
    def test_enhanced_compilation_with_consolidated_managers(
        self, temp_workspace, consolidated_workspace_manager
    ):
        ...
        mock_dag.analyze_workspace_complexity.return_value = {
            "basic_metrics": {"node_count": 2}
        }
...
class TestEndToEndIntegration:
    ...
    def test_complete_pipeline_assembly_workflow(
        self, temp_workspace, consolidated_workspace_manager, sample_workspace_config
    ):
        ...
        with patch.object(
            ...
            assembler.workspace_registry, "find_builder_class"
        ) as mock_find_builder, patch.object(
            assembler.workspace_registry, "find_config_class"
        ) as mock_find_config, patch.object(
    ...
    def test_performance_with_consolidated_managers(
        self, temp_workspace, consolidated_workspace_manager
    ):
