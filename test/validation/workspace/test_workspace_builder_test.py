...
class TestWorkspaceUniversalStepBuilderTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('src.cursus.validation.workspace.workspace_builder_test.WorkspaceManager') as mock_wm_class, \
             patch('src.cursus.validation.workspace.workspace_builder_test.DeveloperWorkspaceFileResolver') as mock_fr_class, \
             patch('src.cursus.validation.workspace.workspace_builder_test.WorkspaceModuleLoader') as mock_ml_class:
    ...
    def test_get_workspace_info(self):
        ...
        # Mock workspace info
        from src.cursus.validation.workspace.workspace_manager import WorkspaceInfo, DeveloperWorkspace
        from pathlib import Path
        
        mock_workspace_info = WorkspaceInfo(
            workspace_root=str(self.workspace_root),
            has_shared=True,
            developers={
                "developer_1": DeveloperWorkspace(
                    developer_id="developer_1",
                    workspace_path=self.dev1_path,
                    has_scripts=True,
                    has_contracts=True,
                    has_specs=True,
                    has_builders=True,
    ...
    def test_get_workspace_info_no_developer(self):
        ...
        info = self.tester.get_workspace_info()
        self.assertIsInstance(info, dict)
        self.assertEqual(info['developer_id'], "developer_1")  # Should still have the initialized developer
    ...
    def test_load_workspace_builder_classes(self):
        ...
        self.mock_module_loader.load_builder_class.assert_called_once_with("test")
    ...
    def test_load_workspace_builder_classes_with_errors(self):
        ...
        self.mock_module_loader.load_builder_class.side_effect = ImportError("Failed to import")
    ...
    def test_run_workspace_builder_test_single_developer(self):
        ...
        # Test the actual run_workspace_builder_test method
        with patch.object(self.tester, 'run_test') as mock_run_test:
            ...
            self.assertEqual(results["workspace_metadata"]["developer_id"], "developer_1")
    ...
    def test_run_workspace_builder_test_with_failures(self):
        ...
        with patch.object(self.tester, 'run_test') as mock_run_test:
            mock_run_test.side_effect = Exception("Builder test failed")
    ...
    def test_run_workspace_builder_test_no_developer_selected(self):
        ...
        # Test with valid developer instead
        results = self.tester.run_workspace_builder_test()
        self.assertIsNotNone(results)
        self.assertIn("workspace_metadata", results)
    ...
    def test_inheritance_from_universal_step_builder_test(self, mock_init):
        ...
        with patch('src.cursus.validation.workspace.workspace_builder_test.WorkspaceManager'), \
             patch('src.cursus.validation.workspace.workspace_builder_test.DeveloperWorkspaceFileResolver'), \
             patch('src.cursus.validation.workspace.workspace_builder_test.WorkspaceModuleLoader'):
    ...
    def test_error_handling_during_builder_loading(self):
        ...
        self.mock_module_loader.load_builder_class.side_effect = Exception("Loading failed")
