...
class TestInterfaceValidatorIntegration:
    ...
    def test_complete_validation_bad_builder(self):
        ...
        doc_violations = [
            vtype for vtype in violation_types if "documentation" in vtype
        ]
    ...
    def test_validation_with_real_step_builder(self):
        ...
        try:
            from cursus.steps.builders.builder_dummy_training_step import (
                DummyTrainingStepBuilder,
            )

            violations = self.validator.validate_step_builder_interface(
                DummyTrainingStepBuilder
            )
