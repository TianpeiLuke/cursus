...
class TestStepTypeEnhancementRouter:
    ...
    @patch(
        "cursus.validation.alignment.step_type_enhancement_router.detect_step_type_from_registry"
    )
    def test_enhance_validation_training_step(
        self, mock_detect_step_type, router, mock_validation_result
    ):
    ...
    def test_enhance_validation_processing_step(
        self, mock_detect_step_type, router, mock_validation_result
    ):
        ...
        mock_processing_enhancer.enhance_validation.return_value = (
    ...
    def test_enhance_validation_unknown_step_type(
        self, mock_detect_step_type, router, mock_validation_result
    ):
    ...
    def test_enhance_validation_none_step_type(
        self, mock_detect_step_type, router, mock_validation_result
    ):
    ...
    def test_enhance_validation_with_exception(
        self, mock_detect_step_type, router, mock_validation_result
    ):
        ...
        mock_training_enhancer = Mock()
        mock_training_enhancer.enhance_validation.side_effect = Exception(
            "Test exception"
        )
    ...
    def test_all_step_types_have_requirements(self, router):
        ...
        for step_type in router.enhancers.keys():
            ...
            if step_type != "Base":  # Base might have minimal requirements
                ...
                if (
                    requirements
                ):  # If requirements exist, they should have expected keys
                    expected_keys = [
                        "input_types",
                        "output_types",
                        "required_methods",
    ...
    def test_enhance_validation_preserves_original_issues(
        self, mock_detect_step_type, router
    ):
