...
class TestSpecificationLoader:
    ...
    def test_find_specification_files_direct_matching(
        self, loader, sample_spec_files, specs_dir
    ):
    ...
    def test_load_specification_from_python_success(
        self, mock_module_from_spec, mock_spec_from_file, loader, sample_spec_obj
    ):
        ...
        with patch("builtins.dir", return_value=["MODEL_TRAINING_SPEC"]):
            with patch.object(
                loader.file_resolver,
                "find_spec_constant_name",
                return_value="MODEL_TRAINING_SPEC",
            ):
                spec_path = Path("model_training_spec.py")
                result = loader.load_specification_from_python(
                    spec_path, "model_training", "default"
                )
    ...
    @patch("importlib.util.spec_from_file_location")
    @patch("importlib.util.module_from_spec")
    def test_load_specification_from_python_no_spec_object(
        ...
        with patch("builtins.dir", return_value=["other_attr"]):
            with patch.object(
                loader.file_resolver, "find_spec_constant_name", return_value=None
            ):
                spec_path = Path("test_spec.py")
    ...
    @patch("importlib.util.spec_from_file_location")
    @patch("importlib.util.module_from_spec")
    def test_load_specification_from_python_sys_path_management(
        self, mock_module_from_spec, mock_spec_from_file, loader, sample_spec_obj
    ):
    ...
    def test_dict_to_step_specification(
        self, mock_step_spec, mock_output_spec, mock_dep_spec, loader
    ):
    ...
    def test_load_all_specifications_with_errors(self, loader, sample_spec_files):
        ...
        with patch.object(loader, "load_specification_from_python") as mock_load:
            ...
            with patch(
                "cursus.validation.alignment.loaders.specification_loader.logger"
            ) as mock_logger:
                result = loader.load_all_specifications()
    ...
    def test_discover_specifications_ignores_init_files(
        self, loader, sample_spec_files
    ):
    ...
    def test_find_specifications_by_contract_success(self, loader, sample_spec_files):
        ...
        with patch.object(loader, "load_specification_from_python") as mock_load:
            ...
            with patch.object(
                loader, "_specification_references_contract", return_value=True
            ):
    ...
    def test_find_specifications_by_contract_no_match(self, loader, sample_spec_files):
        ...
        with patch.object(loader, "load_specification_from_python") as mock_load:
            ...
            with patch.object(
                loader, "_specification_references_contract", return_value=False
            ):
                result = loader.find_specifications_by_contract("model_training")

                assert result == {}

    def test_find_specifications_by_contract_load_error(
        self, loader, sample_spec_files
    ):
    ...
    def test_load_specification_without_spec_dict(self, loader):
        ...
        spec_info = {"spec_name": "test_spec", "job_type": "training"}
...
class TestSpecificationLoaderErrorScenarios:
    ...
    def test_dict_to_step_specification_with_missing_fields(self):
        ...
        with patch(
            "cursus.validation.alignment.loaders.specification_loader.DependencySpec"
        ) as mock_dep_spec:
            with patch(
                "cursus.validation.alignment.loaders.specification_loader.OutputSpec"
            ) as mock_output_spec:
                with patch(
                    "cursus.validation.alignment.loaders.specification_loader.StepSpecification"
                ) as mock_step_spec:
    ...
    def test_specification_references_contract_edge_cases(self):
        ...
        spec_dict = {"step_type": ""}
        result = self.loader._specification_references_contract(
            spec_dict, "model_training"
        )
        assert result is False
