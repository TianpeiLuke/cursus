...
class TestSageMakerPropertyPathValidator:
    ...
    def test_validate_specification_property_paths_training_step(self, validator):
        ...
        issues = validator.validate_specification_property_paths(
            specification, "training_contract"
        )

        # Should have validation results for both outputs
        validation_issues = [
            issue for issue in issues if issue["category"] == "property_path_validation"
        ]
        assert len(validation_issues) == 2

        # One should be valid, one invalid
        valid_issues = [
            issue for issue in validation_issues if issue["severity"] == "INFO"
        ]
        error_issues = [
    ...
    def test_validate_specification_property_paths_processing_step(self, validator):
        ...
        issues = validator.validate_specification_property_paths(
            specification, "processing_contract"
        )
    ...
    def test_validate_specification_property_paths_unknown_step_type(self, validator):
        ...
        issues = validator.validate_specification_property_paths(
            specification, "unknown_contract"
        )
    ...
    def test_step_registry_integration(
        ...
        specification = {"step_type": "CurrencyConversion_Training", "outputs": []}

        issues = validator.validate_specification_property_paths(
            specification, "registry_test"
        )

        # Should have step type resolution info
        resolution_issues = [
            issue for issue in issues if issue["category"] == "step_type_resolution"
        ]
        assert len(resolution_issues) == 1
        assert "resolved via registry" in resolution_issues[0]["message"]
        assert (
            resolution_issues[0]["details"]["resolved_sagemaker_type"] == "Processing"
        )
    ...
    def test_step_registry_unavailable(self, validator):
        ...
        specification = {"step_type": "SomeStep_Training", "outputs": []}

        issues = validator.validate_specification_property_paths(
            specification, "no_registry_test"
        )
    ...
    def test_get_valid_property_paths_for_step_type_training(self, validator):
        ...
        paths = validator._get_valid_property_paths_for_step_type(
            "training", "training"
        )
    ...
    def test_get_valid_property_paths_caching(self, validator):
        ...
        paths1 = validator._get_valid_property_paths_for_step_type(
            "training", "training"
        )

        # Second call should use cache
        paths2 = validator._get_valid_property_paths_for_step_type(
            "training", "training"
        )
    ...
    def test_validate_single_property_path_pattern_match(self, validator):
        ...
        valid_paths = {"metrics": ["properties.FinalMetricDataList[*].Value"]}
    ...
    def test_validate_single_property_path_invalid(self, validator):
        ...
        result = validator._validate_single_property_path(
            "properties.InvalidPath.NotExists", "training", "training", valid_paths
        )
    ...
    def test_matches_property_path_pattern_exact(self, validator):
        ...
        assert (
            validator._matches_property_path_pattern(
                "properties.ModelArtifacts.S3ModelArtifacts",
                "properties.ModelArtifacts.S3ModelArtifacts",
            )
            is True
    ...
    def test_matches_property_path_pattern_wildcard(self, validator):
        ...
        assert (
            validator._matches_property_path_pattern(
                "properties.FinalMetricDataList[0].Value", pattern
            )
            is True
        )
        ...
        assert (
            validator._matches_property_path_pattern(
                'properties.FinalMetricDataList["accuracy"].Value', pattern
            )
            is True
        )
        ...
        assert (
            validator._matches_property_path_pattern(
                "properties.FinalMetricDataList[*].Value", pattern
            )
            is True
        )
        ...
        assert (
            validator._matches_property_path_pattern(
                "properties.DifferentPath[0].Value", pattern
            )
            is False
        )
    ...
    def test_matches_property_path_pattern_complex(self, validator):
        ...
        assert (
            validator._matches_property_path_pattern(
                "properties.ProcessingOutputConfig.Outputs[0].S3Output.S3Uri", pattern
            )
            is True
        )
    ...
    def test_get_property_path_suggestions(self, validator):
        ...
        suggestions = validator._get_property_path_suggestions(
            "properties.ModelArtifacts.InvalidPath", all_valid_paths
        )
    ...
    def test_calculate_path_similarity(self, validator):
        ...
        similarity = validator._calculate_path_similarity(
            "properties.modelartifacts.s3modelartifacts", "outputparameters[0]"
        )
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_validate_specification_with_no_outputs(self, validator):
        ...
        specification = {"step_type": "training", "outputs": []}

        issues = validator.validate_specification_property_paths(
            specification, "no_outputs"
        )
    ...
    def test_validate_specification_with_empty_property_path(self, validator):
        ...
        specification = {
            "step_type": "training",
            "outputs": [{"logical_name": "output1", "property_path": ""}],
        }

        issues = validator.validate_specification_property_paths(
            specification, "empty_path"
        )
    ...
    def test_validate_specification_with_malformed_data(self, validator):
        ...
        try:
            ...
            validation_issues = [
                issue
                for issue in issues
                if issue["category"] == "property_path_validation"
            ]
            assert len(validation_issues) >= 1
    ...
    def test_validate_single_property_path_with_empty_valid_paths(self, validator):
        ...
        result = validator._validate_single_property_path(
            "properties.Test.Path", "training", "training", {}
        )
