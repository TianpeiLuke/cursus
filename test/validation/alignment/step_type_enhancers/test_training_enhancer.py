...
class TestTrainingStepEnhancer:
    ...
    def test_enhance_validation_xgboost_training(
        self,
        mock_get_script_analysis,
        enhancer,
        ...
        # Execute
        result = enhancer.enhance_validation(
            mock_validation_result, "xgboost_training.py"
        )
    ...
    @patch.object(TrainingStepEnhancer, "_get_script_analysis")
    def test_enhance_validation_pytorch_training(
        self, mock_get_script_analysis, enhancer, mock_validation_result
    ):
        ...
        # Execute
        result = enhancer.enhance_validation(
            mock_validation_result, "pytorch_training.py"
        )
    ...
    @patch.object(TrainingStepEnhancer, "_get_script_analysis")
    def test_enhance_validation_calls_all_validation_levels(
        self,
        mock_get_script_analysis,
        mock_validate_script,
        mock_validate_specs,
        mock_validate_deps,
        mock_validate_builder,
        enhancer,
        mock_validation_result,
        mock_script_analysis,
    ):
    ...
    def test_validate_training_script_patterns_missing_patterns(
        self,
        mock_has_hyperparams,
        mock_has_model_saving,
        mock_has_training_loop,
        enhancer,
        mock_script_analysis,
    ):
        ...
        # Execute
        issues = enhancer._validate_training_script_patterns(
            mock_script_analysis, "xgboost", "test_script.py"
        )
    ...
    @patch.object(TrainingStepEnhancer, "_has_model_saving_patterns")
    @patch.object(TrainingStepEnhancer, "_has_hyperparameter_loading_patterns")
    def test_validate_training_script_patterns_all_present(
        self,
        mock_has_hyperparams,
        mock_has_model_saving,
        mock_has_training_loop,
        ...
        assert (
            len(issues) <= 2
        )  # Allow for some issues but fewer than when all patterns are missing

    def test_has_training_loop_patterns_xgboost(self, enhancer):
        """Test detection of XGBoost training loop patterns."""
        script_analysis = {"functions": ["xgb.train", "model.fit", "train_model"]}
    ...
    def test_has_training_loop_patterns_missing(self, enhancer):
        ...
        script_analysis = {"patterns": {"training_loop": []}}
    ...
    def test_has_model_saving_patterns_missing(self, enhancer):
        ...
        script_analysis = {"patterns": {"model_saving": []}}
    ...
    def test_has_hyperparameter_loading_patterns_missing(self, enhancer):
        ...
        script_analysis = {"patterns": {"hyperparameter_loading": []}}
    ...
    @patch.object(TrainingStepEnhancer, "_get_script_analysis")
    def test_framework_specific_validation_xgboost(
        self, mock_get_script_analysis, enhancer, mock_script_analysis
    ):
    ...
    @patch.object(TrainingStepEnhancer, "_get_script_analysis")
    def test_framework_specific_validation_pytorch(
        self, mock_get_script_analysis, enhancer
    ):
        ...
        # Execute
        issues = enhancer._validate_training_script_patterns(
            pytorch_analysis, "pytorch", "test_script.py"
        )

        # Verify that issues were created (framework-specific validation is handled differently)
        assert isinstance(issues, list)

    def test_framework_specific_validation_unknown_framework(
        self, enhancer, mock_script_analysis
    ):
        """Test framework-specific validation for unknown framework."""
        # Execute with unknown framework
        issues = enhancer._validate_training_script_patterns(
            mock_script_analysis, "unknown_framework", "test_script.py"
        )
        ...
        framework_issues = [
            issue
            for issue in issues
            if hasattr(issue, "framework") and issue.framework == "unknown_framework"
        ]
        assert len(framework_issues) == 0

    @patch.object(TrainingStepEnhancer, "_get_script_analysis")
    def test_get_script_analysis_integration(
        self, mock_get_script_analysis, enhancer, mock_script_analysis
    ):
    ...
    def test_validate_training_dependencies_placeholder(self, enhancer):
        ...
        issues = enhancer._validate_training_dependencies(
            "xgboost_training.py", "xgboost"
        )
