...
def run_builder_config_tests(
    verbosity=1, collect_only=False, markers=None, pattern=None
):
...
Examples:
  ...
  python run_builder_config_tests.py --coverage        # Show coverage report
    ...
    parser.add_argument(
        "--coverage", action="store_true", help="Show test coverage report"
    )
    ...
    parser.add_argument(
        "-k", "--pattern", help="Run tests matching given substring expression"
    )
    ...
    if not args.collect_only:
        ...
        if success:
            print(
                "✅ All builder-configuration alignment tests completed successfully!"
            )
        else:
