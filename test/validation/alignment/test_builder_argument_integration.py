...
class TestBuilderArgumentIntegration:
    ...
    def test_builder_argument_detection_integration(self):
        ...
        tester = ScriptContractAlignmentTester(
            real_scripts_dir, real_contracts_dir, real_builders_dir
        )
        ...
        builder_args = extract_builder_arguments(
            "tabular_preprocess", real_builders_dir
        )
        ...
        mock_contract = {"arguments": {}}  # Empty - job_type not in contract

        # Test the argument validation through the validator
        issues = tester.script_validator.validate_argument_usage(
            mock_analysis, mock_contract, "tabular_preprocess", builder_args
        )
        ...
        job_type_issues = [
            issue for issue in issues if "job_type" in issue.get("message", "").lower()
        ]
        ...
        info_issues = [
            issue for issue in job_type_issues if issue["severity"] == "INFO"
        ]
        assert (
            len(info_issues) > 0
