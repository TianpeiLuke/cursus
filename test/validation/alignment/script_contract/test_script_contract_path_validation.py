...
'''
        ...
        with patch(
            ...
            assert result[
                "passed"
            ], f"Validation failed with issues: {result.get('issues', [])}"
...
'''
        ...
        with patch(
            ...
            path_issues = [
                ...
                if issue["category"] == "path_usage"
                and "undeclared" in issue["message"]
            ]
    ...
    def test_missing_script_file(self):
        ...
        missing_file_issues = [
            issue for issue in result["issues"] if issue["category"] == "missing_file"
        ]
...
'''
        ...
        with patch(
            ...
            mock_ops = [
                MagicMock(
                    file_path="/opt/ml/processing/input/data.csv",
                    operation_type="read",
                    method="pandas.read_csv",
                    line_number=6,
                ),
                MagicMock(
                    file_path="/opt/ml/processing/output/data.parquet",
                    operation_type="write",
                    method="dataframe.to_parquet",
                    line_number=7,
                ),
                MagicMock(
                    file_path="/opt/ml/processing/input/model.tar.gz",
                    operation_type="read",
                    method="tarfile.open",
                    line_number=10,
                ),
                ...
                MagicMock(
                    file_path="/opt/ml/processing/output/dest.txt",
                    operation_type="write",
                    method="shutil.copy",
                    line_number=14,
                ),
                MagicMock(
                    file_path="/opt/ml/processing/output",
                    operation_type="write",
                    method="pathlib.mkdir",
                    line_number=18,
                ),
            ]
            ...
            assert result[
                "passed"
            ], f"Enhanced operations validation failed: {result.get('issues', [])}"
