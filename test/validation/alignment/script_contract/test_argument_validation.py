...
class TestArgumentValidation:
    ...
    def setup_method(self):
        ...
        self.tester = ScriptContractAlignmentTester(
            str(self.scripts_dir), str(self.contracts_dir)
        )
...
'''
        ...
        with patch(
            "cursus.validation.alignment.script_contract_alignment.ScriptAnalyzer"
        ) as mock_analyzer:
            ...
            mock_arg1 = MagicMock()
            mock_arg1.argument_name = (
                "model_name"  # Python attribute name (underscores)
            )
            ...
            arg_issues = [
                issue for issue in result["issues"] if issue["category"] == "arguments"
            ]
...
'''
        ...
        with patch(
            ...
            missing_arg_issues = [
                ...
                if issue["category"] == "arguments"
                and "not defined in script" in issue["message"]
            ]
...
'''
        ...
        with patch(
            ...
            extra_arg_issues = [
                ...
                for issue in result["issues"]
                if issue["category"] == "arguments"
                and "not in contract" in issue["message"]
            ]
...
'''
        ...
        mock_contract = {
            "arguments": {
                "epochs": {"required": True, "type": "int"}  # Contract expects int
            }
        }
        ...
        issues = self.tester.script_validator.validate_argument_usage(
            mock_analysis, mock_contract, "test_script", set()
        )
    ...
    def test_argparse_hyphen_to_underscore_normalization(self):
        ...
        issues = self.tester.script_validator.validate_argument_usage(
            mock_analysis, mock_contract, "currency_conversion", set()
        )
    ...
    def test_argparse_normalization_with_missing_argument(self):
        ...
        missing_arg_issues = [
            issue for issue in issues if "not defined in script" in issue["message"]
        ]
        assert (
            len(missing_arg_issues) == 1
    ...
    def test_argparse_normalization_with_extra_argument(self):
        ...
        extra_arg_issues = [
            issue for issue in issues if "not in contract" in issue["message"]
        ]
        assert (
            len(extra_arg_issues) == 1
