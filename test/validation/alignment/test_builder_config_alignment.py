...
from cursus.validation.alignment.builder_config_alignment import (
...
"""
        ...
        with patch.object(self.tester, "flexible_resolver") as mock_flexible_resolver:
            mock_flexible_resolver.get_available_files_report.return_value = {
                "configs": {"discovered_files": [], "base_names": [], "count": 0}
            }
        ...
        issues = [
            ...
            if issue["category"] == "missing_configuration"
        ]
        assert len(issues) > 0
...
"""
        ...
        with patch.object(
            self.tester.config_analyzer, "load_config_from_python"
        ) as mock_load_config, patch.object(
            self.tester.builder_analyzer, "analyze_builder_file"
        ) as mock_analyze_builder:
            ...
            mock_analyze_builder.return_value = {
                ...
                "class_definitions": [{"class_name": "TestStepBuilder"}],
                "validation_calls": [],
            }
    ...
    def test_validate_configuration_fields_undeclared_access(self):
        ...
        config_analysis = {
            "fields": {"existing_field": {"type": "str", "required": True}},
        ...
        missing_field_issues = [
            issue for issue in error_issues if "missing_field" in issue["message"]
        ]
    ...
    def test_validate_configuration_fields_pattern_filtering(self):
            ...
            "config_accesses": [
                {
                    "field_name": "job_type",
                    "line": 10,
                },  # Common pattern, might be filtered
        ...
        with patch.object(
            self.tester.pattern_recognizer, "is_acceptable_pattern"
        ) as mock_pattern:
            mock_pattern.side_effect = (
                lambda field, builder, issue_type: field == "job_type"
            )
    ...
    def test_validate_required_fields(self):
        ...
        builder_analysis = {"validation_calls": []}  # No validation logic

        config_analysis = {"required_fields": {"required_field1", "required_field2"}}
        ...
        validation_issues = [
            issue for issue in info_issues if "validation logic" in issue["message"]
        ]
        assert len(validation_issues) > 0

    def test_validate_config_import(self):
        """Test validation of configuration import"""
        builder_analysis = {"class_definitions": [{"class_name": "TestStepBuilder"}]}

        config_analysis = {"class_name": "TestConfig"}
...
"""
    ...
    def test_error_handling_in_validation(self):
        ...
        # Mock the flexible_resolver which has the get_available_files_report method
        with patch.object(
            self.tester, "flexible_resolver"
        ) as mock_flexible_resolver, patch.object(
            self.tester.builder_analyzer, "analyze_builder_file"
        ) as mock_analyze:
