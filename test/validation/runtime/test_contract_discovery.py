...
class TestContractDiscoveryManager:
    ...
    @patch('importlib.import_module')
    def test_discover_by_direct_import_module_not_found(self, mock_import, discovery_manager):
    ...
    @patch('importlib.import_module')
    def test_discover_by_direct_import_entry_point_mismatch(self, mock_import, discovery_manager):
    ...
    def test_is_contract_match_no_entry_point_fallback(self, discovery_manager):
        ...
        with patch.object(discovery_manager, '_fallback_name_matching', return_value=True):
            result = discovery_manager._is_contract_match(mock_contract, "test_script")
    ...
    def test_fallback_name_matching_substring_match(self, discovery_manager):
        ...
        result = discovery_manager._fallback_name_matching("TestScriptContract", "testscript")
        
        assert result is True
    
    def test_fallback_name_matching_no_match(self, discovery_manager):
        """Test fallback name matching with no match"""
        result = discovery_manager._fallback_name_matching("CompletelyDifferentContract", "test_script")
    ...
    def test_get_contract_environ_vars_dict_format(self, discovery_manager):
        ...
        mock_contract = Mock(spec=ScriptContract)
        mock_contract.required_env_vars = [{"MODEL_TYPE": "xgboost", "BATCH_SIZE": "1000"}]
    ...
    def test_get_contract_job_args_from_metadata(self, discovery_manager):
        ...
        mock_contract.metadata = {
            "job_args": {"custom_param": "custom_value"}
        }
    ...
    def test_adapt_path_for_local_testing_sagemaker_patterns(self, discovery_manager, temp_dir):
    ...
    def test_adapt_path_for_local_testing_non_sagemaker(self, discovery_manager, temp_dir):
    ...
    def test_discover_contract_full_workflow_success(self, discovery_manager):
        ...
        with patch.object(discovery_manager, '_discover_by_direct_import') as mock_direct:
            mock_contract = Mock(spec=ScriptContract)
    ...
    def test_discover_contract_full_workflow_failure(self, discovery_manager):
        ...
        with patch.object(discovery_manager, '_discover_by_direct_import') as mock_direct:
            with patch.object(discovery_manager, '_discover_by_pattern_matching') as mock_pattern:
                with patch.object(discovery_manager, '_discover_by_fuzzy_search') as mock_fuzzy:
                    # All methods fail
...
class TestContractDiscoveryIntegration:
    ...
    def test_end_to_end_contract_discovery_and_adaptation(self, discovery_manager, temp_dir):
    ...
    def test_contract_discovery_with_canonical_name(self, discovery_manager):
        ...
        with patch.object(discovery_manager, '_discover_by_direct_import') as mock_direct:
            ...
            mock_direct.assert_called_once_with("tabular_preprocessing", "TabularPreprocessing")
