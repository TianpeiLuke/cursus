    ...
    try:
        ...
        for step_name, expected_type in test_cases:
            try:
                ...
                print(
                    f"   {status} {step_name}: {actual_type} (expected: {expected_type})"
                )
        ...
        for step_type, expected_valid in validation_tests:
            try:
                ...
                print(
                    f"   {status} {step_type}: {is_valid} (expected: {expected_valid})"
                )
            except Exception as e:
                print(f"   ❌ {step_type}: Error - {e}")
...
def test_sagemaker_validator():
    ...
    try:
        from cursus.validation.builders.sagemaker_step_type_validator import (
...
def test_universal_test_integration():
    ...
    try:
        ...
        try:
            ...
            print(
                f"   ✅ SageMaker validator integrated: {hasattr(tester, 'sagemaker_validator')}"
            )
