...
class TestBasicSerializationAndFields(unittest.TestCase):
    ...
    def test_static_field_detection(self):
        ...
        self.assertFalse(
            categorizer._is_likely_static("input_names", {"input1": "path1"})
        )
        self.assertFalse(
            categorizer._is_likely_static("output_names", {"output1": "path1"})
        )
        self.assertFalse(
            categorizer._is_likely_static("field_names", ["name1", "name2"])
        )
    ...
    def test_pydantic_model_serialization(self):
        ...
        self.assertFalse(
            categorizer._is_likely_static("hyperparameters", hyperparams),
        ...
        with patch(
            "src.config_field_manager.constants.SPECIAL_FIELDS_TO_KEEP_SPECIFIC",
            {"hyperparameters"},
        ):
            ...
            self.assertTrue(
                categorizer._is_special_field(
                    "hyperparameters", hyperparams, mock_config
                ),
