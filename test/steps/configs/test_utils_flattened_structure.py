...
class TestFlattenedConfigStructure(unittest.TestCase):
    ...
    def test_different_values_in_specific(self):
        ...
        try:
            ...
            self.assertTrue(
                found_descriptions,
                "Should find at least one pipeline description in specific sections",
            )
            self.assertTrue(
                found_s3_locs,
                "Should find at least one s3 location in specific sections",
            )
    ...
    def test_load_from_simplified_structure(self):
        ...
        try:
            ...
            self.assertGreaterEqual(
                len(loaded_configs), 1, "Should load at least 1 config"
            )
            ...
            for cfg in loaded_config_list:
                ...
                self.assertTrue(
                    hasattr(cfg, "pipeline_description"),
                    "Config should have pipeline_description",
                )
                self.assertTrue(
                    hasattr(cfg, "pipeline_s3_loc"),
                    "Config should have pipeline_s3_loc",
                )
