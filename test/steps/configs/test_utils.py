...
class TestSimplifiedConfigFieldCategorization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.pipeline_scripts_path = os.path.join(
            self.repo_root, "src/pipeline_scripts"
        )
        ...
        self.processing_config = CustomProcessingConfig(
            ...
            },  # This is in SPECIAL_FIELDS_TO_KEEP_SPECIFIC
        )
    ...
    def test_simplified_structure(self):
        ...
        try:
            ...
            merge_and_save_configs(
                [self.base_config, self.dummy_config, self.processing_config],
                output_path,
            )
            ...
            self.assertIsInstance(
                specific_section, dict, "'specific' section should be a dictionary"
            )
            ...
            self.assertTrue(
                len(specific_keys) > 0, "The 'specific' section should not be empty"
            )
            ...
            self.assertTrue(
                processing_found, "No processing config found in the 'specific' section"
            )
            self.assertTrue(
                non_processing_found,
                "No non-processing config found in the 'specific' section",
            )
    ...
    def test_simplified_special_fields(self):
        ...
        try:
            ...
            self.assertTrue(
                found_hyperparameters, "hyperparameters should be in a specific section"
            )
    ...
    def test_simplified_unique_fields(self):
        ...
        try:
            # Merge the configs and save to the temp file
            merge_and_save_configs(
                [self.specific_config, self.processing_config], output_path
            )
            ...
            for step_name, step_config in specific_sections.items():
                ...
                if "output_schema" in step_config:
                    ...
                    self.assertEqual(
                        step_config["output_schema"], {"fields": ["field1", "field2"]}
                    )

            # Verify we found our specific fields
            self.assertTrue(
                found_input_names, "input_names should be in a specific section"
            )
            self.assertTrue(
                found_output_schema, "output_schema should be in a specific section"
            )
    ...
    def test_simplified_non_static_fields(self):
        ...
        config = ConfigWithNonStatic(
            ...
            pipeline_s3_loc="s3://test-bucket/test-pipeline",
            input_names={
                "input1": "s3://bucket/input1"
            },  # Non-static field by name pattern
            output_names={
                "output1": "s3://bucket/output1"
            },  # Non-static field by name pattern
        ...
        try:
            ...
            self.assertIsNotNone(
                config_with_non_static, "Config with non-static fields not found"
            )
    ...
    def test_simplified_structure_with_local_configs(self):
        ...
        try:
            ...
            with open(output_path, "r") as f:
                ...
                self.assertNotIn("processing", output_json["configuration"])
                self.assertEqual(
                    set(output_json["configuration"].keys()), {"shared", "specific"}
                )
            ...
            specific_section = output_json["configuration"]["specific"]
            self.assertEqual(
                len(specific_section),
                4,
                "Should have 4 configs in the specific section",
            )
            ...
            for config_name, config_data in specific_section.items():
                ...
                if "custom_field" in config_data:
                    ...
                    self.assertEqual(
                        config_data["custom_field"], "processing_specific_value"
                    )
    ...
    def test_simplified_different_values(self):
        ...
        try:
            ...
            self.assertTrue(
                len(found_descriptions) > 0,
                "No pipeline descriptions found in specific sections",
            )
            self.assertTrue(
                all(
                    desc in {"Test Pipeline 1", "Test Pipeline 2", "Test Pipeline"}
            ...
            self.assertTrue(
                len(found_s3_locs) > 0, "No s3 locations found in specific sections"
            )
            self.assertTrue(
                all(
                    loc
                    in {
                        "s3://test-bucket/test-pipeline-1",
                        "s3://test-bucket/test-pipeline-2",
                        "s3://test-bucket/test-pipeline",
                    }
