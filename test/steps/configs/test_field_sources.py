...
class TestFieldSources(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.processing_config = CustomProcessingConfig(
            ...
            processing_source_dir=str(
                Path(__file__).parent.parent.parent / "src" / "pipeline_scripts"
            ),
        )
    ...
    def test_field_sources_categories(self):
        ...
        if "hyperparameters" in serialized:
            print(
                f"Hyperparameters found in serialized output: {type(serialized['hyperparameters'])}"
            )
    ...
    def test_field_sources_values(self):
        ...
        self.assertGreaterEqual(
            len(field_sources["processing"]["processing_instance_count"]), 2
        )
    ...
    def test_field_sources_metadata(self):
        ...
        try:
            ...
            self.assertEqual(
                len(field_sources["pipeline_name"]), 2
            )  # Both configs have this field
            self.assertEqual(
                len(field_sources["processing_instance_count"]), 1
            )  # Only processing config has this
