...
class CreateModelStepBuilderTestSuite:
    ...
    Uses registry information to automatically discover CreateModel step builders,
    making it adaptive when new steps are added.
    """

    @classmethod
    def get_createmodel_steps_from_registry(cls) -> List[str]:
        """Get all CreateModel step names from the registry."""
        return get_createmodel_steps_from_registry()

    # Expected model types for each step
    EXPECTED_MODELS = {"PyTorchModel": "PyTorchModel", "XGBoostModel": "XGBoostModel"}

    # Framework versions expected
    EXPECTED_FRAMEWORKS = {"PyTorchModel": "pytorch", "XGBoostModel": "xgboost"}
    ...
    def get_available_createmodel_builders(cls) -> List[tuple]:
        """
        Get all available CreateModel step builders for testing using registry information.

        Returns:
            List of tuples (step_name, builder_class)
        """
        available_builders = []
        createmodel_steps = cls.get_createmodel_steps_from_registry()

        for step_name in createmodel_steps:
    ...
    def run_createmodel_specific_tests(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
        """
        ...
        results["test_createmodel_environment_variables"] = (
            self._test_createmodel_environment_variables(step_name, builder_class)
        )
        ...
        results["test_createmodel_step_creation"] = (
            self._test_createmodel_step_creation(step_name, builder_class)
        )

        return results

    def _test_model_creation_method(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "error": (
                    "No model creation methods found" if not found_methods else None
                ),
                "details": {
    ...
    def _test_createmodel_io_methods(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "error": (
                    f"Missing methods: {missing_methods}" if missing_methods else None
                ),
                "details": {
    ...
    def _test_createmodel_environment_variables(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not expected_framework:
                return {
                    ...
                    "error": None,
                    "details": {
                        "note": f"No expected framework defined for {step_name}"
                    },
                }
    ...
    def _test_createmodel_step_creation(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
...
class TestCreateModelStepBuilders(unittest.TestCase):
    ...
    def test_all_createmodel_builders_universal_compliance(self):
        ...
        for step_name, builder_class in self.available_builders:
            ...
            try:
                ...
                universal_results = (
                    results.get("test_results", results)
    ...
    def _report_overall_summary(self, all_results: Dict[str, Any]):
        ...
        builder_success_rate = (
            (successful_builders / total_builders) * 100 if total_builders > 0 else 0
        )

        print(f"\nOverall Statistics:")
        print(f"  Builders tested: {total_builders}")
        print(
            f"  Builders with all tests passing: {successful_builders} ({builder_success_rate:.1f}%)"
        )
                ...
                if test_name in results:
                    ...
                    if isinstance(result, dict) and not result.get("passed", False):
                        failed_critical.append(
                            f"{step_name}.{test_name}: {result.get('error', 'Failed')}"
                        )
...
def test_individual_createmodel_builder_universal_compliance(step_name, builder_class):
    ...
    for test_name in critical_tests:
        if test_name in results:
            ...
            assert result[
                "passed"
            ], f"{test_name} failed for {step_name}: {result.get('error')}"
