#!/usr/bin/env python3
"""
Runner script for CreateModel step builder tests.

This script runs the comprehensive test suite for all CreateModel step builders
...
from .test_createmodel_step_builders import CreateModelStepBuilderTestSuite
from cursus.validation.builders.universal_test import UniversalStepBuilderTest


def generate_createmodel_reports():
    """Generate comprehensive reports for CreateModel step builders."""
    print("=" * 80)
    print("CREATEMODEL STEP BUILDERS COMPREHENSIVE TEST REPORT")
    print("=" * 80)

    # Initialize test suite
    test_suite = CreateModelStepBuilderTestSuite()
    available_builders = test_suite.get_available_createmodel_builders()

    if not available_builders:
        print("❌ No CreateModel step builders available for testing")
        return

    print(f"Found {len(available_builders)} CreateModel step builders:")
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            createmodel_results = test_suite.run_createmodel_specific_tests(
                step_name, builder_class
            )

            # Combine results
            if (
                isinstance(universal_results, dict)
                and "test_results" in universal_results
            ):
                combined_results = {
                    **universal_results["test_results"],
                    **createmodel_results,
                }
            ...
            total_tests = len(
                [r for r in combined_results.values() if isinstance(r, dict)]
            )
    ...
    # Create createmodel-specific subdirectories
    for step_name, builder_class in available_builders:
        if step_name in all_results and "error" not in all_results[step_name]:
            step_dir = Path(__file__).parent / "createmodel" / builder_class.__name__
    ...
    json_report = {
        "timestamp": timestamp,
        "test_type": "CreateModel Step Builders",
        "summary": generate_summary(all_results),
        "detailed_results": all_results,
        "scoring": detailed_scores,
    }

    json_file = reports_dir / f"createmodel_step_builders_report_{timestamp}.json"
    ...
    for step_name, builder_class in available_builders:
        if step_name in all_results and "error" not in all_results[step_name]:
            generate_score_chart(
                ...
                detailed_scores.get(step_name, {}),
                builder_class.__name__,
            )

    # Generate overall summary chart
    generate_overall_chart(all_results, "createmodel")

    print(f"\n✅ CreateModel step builders testing completed!")
...
def generate_score_chart(step_name, results, scoring_data, builder_class_name):
    ...
    fig.suptitle(
        f"{step_name} CreateModel Step Builder Test Results",
        fontsize=16,
        fontweight="bold",
    )
    ...
    failed_counts = [
        test_categories[cat]["total"] - test_categories[cat]["passed"]
    ...
    # Save chart
    chart_dir = (
        Path(__file__).parent / "createmodel" / builder_class_name / "scoring_reports"
    )
...
def generate_overall_chart(all_results, test_type):
    ...
    colors = [
        (
            "#2ecc71"
            if builder_stats[b]["status"] == "success"
            else "#f39c12" if builder_stats[b]["status"] == "partial" else "#e74c3c"
        )
        for b in builders
    ]
