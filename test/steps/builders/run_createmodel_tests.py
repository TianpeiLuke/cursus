...
def generate_createmodel_reports():
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            createmodel_results = test_suite.run_createmodel_specific_tests(step_name, builder_class)
            
            # Combine results
            if isinstance(universal_results, dict) and 'test_results' in universal_results:
                combined_results = {**universal_results['test_results'], **createmodel_results}
    ...
    for step_name in all_results.keys():
        if "error" not in all_results[step_name]:
            step_dir = Path(f"test/steps/builders/{step_name.lower()}_createmodel")
...
def generate_score_chart(step_name, results, scoring_data):
    ...
    fig.suptitle(f'{step_name} CreateModel Step Builder Test Results', fontsize=16, fontweight='bold')
    ...
    # Save chart
    chart_dir = Path(f"test/steps/builders/{step_name.lower()}_createmodel/scoring_reports")
    chart_file = chart_dir / f"{step_name}CreateModelStepBuilder_score_chart.png"
    ...
    score_file = chart_dir / f"{step_name}CreateModelStepBuilder_score_report.json"
