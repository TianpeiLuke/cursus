...
def generate_transform_reports():
    ...
    for step_name in all_results.keys():
        if "error" not in all_results[step_name]:
            step_dir = Path(f"test/steps/builders/{step_name.lower()}_transform")
...
def generate_score_chart(step_name, results, scoring_data):
    ...
    # Save chart
    chart_dir = Path(f"test/steps/builders/{step_name.lower()}_transform/scoring_reports")
    chart_file = chart_dir / f"{step_name}TransformStepBuilder_score_chart.png"
    ...
    score_file = chart_dir / f"{step_name}TransformStepBuilder_score_report.json"
