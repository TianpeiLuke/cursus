...
class StepBuilderReportGenerator:
    ...
    def _create_step_readme(self, readme_path: Path, step_name: str, canonical_name: str, step_type: str):
...
"""
    ...
    def generate_score_chart(self, results: Dict[str, Any], output_path: Path):
        ...
        try:
            ...
            test_scores = [1 if result.get('passed', False) else 0 
                          for result in test_results.values()]
            
            # Truncate long test names for display
            display_names = [name.replace('test_', '').replace('_', ' ').title()[:20] 
