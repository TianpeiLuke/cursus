...
def generate_transform_reports():
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            transform_results = test_suite.run_transform_specific_tests(
...
def generate_score_chart(step_name, results, scoring_data, builder_class_name):
    ...
    fig.suptitle(
        f"{step_name} Transform Step Builder Test Results",
        fontsize=16,
        fontweight="bold",
    )
    ...
    # Save chart
    chart_dir = (
        Path(__file__).parent / "transform" / builder_class_name / "scoring_reports"
    )
