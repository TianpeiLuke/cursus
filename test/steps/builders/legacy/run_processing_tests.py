#!/usr/bin/env python3
"""
Runner script for Processing step builder tests.

This script runs the comprehensive test suite for all Processing step builders
...
def generate_processing_reports():
    """Generate comprehensive reports for Processing step builders."""
    print("=" * 80)
    print("PROCESSING STEP BUILDERS COMPREHENSIVE TEST REPORT")
    print("=" * 80)
    ...
    if not available_builders:
        print("❌ No Processing step builders available for testing")
        return

    print(f"Found {len(available_builders)} Processing step builders:")
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            processing_results = test_suite.run_processing_specific_tests(
    ...
    # Create processing-specific subdirectories
    for step_name, builder_class in available_builders:
        if step_name in all_results and "error" not in all_results[step_name]:
            step_dir = Path(__file__).parent / "processing" / builder_class.__name__
    ...
    json_report = {
        "timestamp": timestamp,
        "test_type": "Processing Step Builders",
        "summary": generate_summary(all_results),
        "detailed_results": all_results,
        "scoring": detailed_scores,
    }

    json_file = reports_dir / f"processing_step_builders_report_{timestamp}.json"
    ...
    for step_name, results in all_results.items():
        if "error" not in results:
            # Find the builder class for this step
            builder_class = next(
                (bc for sn, bc in available_builders if sn == step_name), None
            )
            if builder_class:
    ...
    generate_overall_chart(all_results, "processing")

    print(f"\n✅ Processing step builders testing completed!")
...
def generate_score_chart(step_name, results, scoring_data, builder_class_name):
    ...
    fig.suptitle(
        f"{step_name} Processing Step Builder Test Results",
        fontsize=16,
        fontweight="bold",
    )
    ...
    # Save chart
    chart_dir = (
        Path(__file__).parent / "processing" / builder_class_name / "scoring_reports"
    )
