...
class TransformStepBuilderTestSuite:
    ...
    EXPECTED_TRANSFORMERS = {"BatchTransform": "Transformer"}
    ...
    def run_transform_specific_tests(
        ...
        results["test_transformer_creation_method"] = (
            self._test_transformer_creation_method(step_name, builder_class)
        )

        # Test 2: Expected Transformer Type
        results["test_expected_transformer_type"] = (
            self._test_expected_transformer_type(step_name, builder_class)
        )
    ...
    def _test_transformer_creation_method(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            found_methods = [
                m for m in transformer_methods if hasattr(builder_class, m)
            ]

            return {
                "passed": len(found_methods) > 0,
                "error": (
                    "No transformer creation methods found"
    ...
    def _test_expected_transformer_type(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not expected_transformer:
                return {
                    ...
                    "error": None,
                    "details": {
                        "note": f"No expected transformer defined for {step_name}"
                    },
                }
    ...
    def _test_transform_io_methods(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
    ...
    def _test_transform_step_creation(
        self, step_name: str, builder_class: Type[StepBuilderBase]
    ) -> Dict[str, Any]:
