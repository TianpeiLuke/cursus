...
def generate_processing_reports():
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            if isinstance(universal_results, dict) and 'test_results' in universal_results:
                combined_results = {**universal_results['test_results'], **processing_results}
    ...
    for step_name, results in all_results.items():
        if "error" not in results:
            ...
            builder_class = next((bc for sn, bc in available_builders if sn == step_name), None)
            if builder_class:
                generate_score_chart(step_name, results, detailed_scores.get(step_name, {}), builder_class.__name__)
...
def generate_score_chart(step_name, results, scoring_data, builder_class_name):
    ...
    fig.suptitle(f'{step_name} Processing Step Builder Test Results', fontsize=16, fontweight='bold')
    ...
    # Save chart
    chart_dir = Path(__file__).parent / "processing" / builder_class_name / "scoring_reports"
