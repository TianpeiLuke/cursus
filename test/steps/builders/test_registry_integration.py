...
class TestRegistryIntegration(unittest.TestCase):
    ...
    def test_universal_test_registry_methods(self):
        ...
        self.assertTrue(
            hasattr(UniversalStepBuilderTest, "generate_registry_discovery_report")
        )
        self.assertTrue(
            hasattr(UniversalStepBuilderTest, "validate_builder_availability")
        )
    ...
    def test_step_builder_loading(self):
        ...
        for step_name, expected_type in test_cases:
            try:
                ...
                if validation.get("loadable", False):
                    ...
                    # Get step info
                    step_info = RegistryStepDiscovery.get_step_info_from_registry(
                        step_name
                    )
                    ...
                    successful_loads += 1
                    print(
                        f"✅ Successfully loaded {step_name} ({builder_class.__name__})"
                    )
                else:
                    print(
                        f"⚠️  {step_name} not available: {validation.get('error', 'Unknown error')}"
                    )
        ...
        self.assertGreater(
            successful_loads, 0, "Should be able to load at least one step builder"
        )
        print(
    ...
    def test_universal_test_with_registry_discovery(self):
        ...
        try:
            results = UniversalStepBuilderTest.test_all_builders_by_type(
                "Processing", verbose=False, enable_scoring=False
            )
            ...
            if results and not results.get("error"):
                print(
                    f"✅ Successfully tested {len(results)} Processing builders using registry discovery"
                )
                ...
                print(
                    f"⚠️  No Processing builders available or error occurred: {results.get('error', 'Unknown')}"
                )
    ...
    def test_integration_with_existing_test_files(self):
        ...
        try:
            ...
            self.assertIsInstance(training_steps, list)
            print(
                f"✅ TrainingStepBuilderTestSuite integration works: {len(training_steps)} steps found"
            )
