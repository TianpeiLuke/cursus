#!/usr/bin/env python3
"""
Runner script for Training step builder tests.

This script runs the comprehensive test suite for all Training step builders
...
def generate_training_reports():
    """Generate comprehensive reports for Training step builders."""
    print("=" * 80)
    print("TRAINING STEP BUILDERS COMPREHENSIVE TEST REPORT")
    print("=" * 80)

    # Initialize test suite
    test_suite = TrainingStepBuilderTestSuite()
    available_builders = test_suite.get_available_training_builders()

    if not available_builders:
        print("❌ No Training step builders available for testing")
        return

    print(f"Found {len(available_builders)} Training step builders:")
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            training_results = test_suite.run_training_specific_tests(
    ...
    json_report = {
        "timestamp": timestamp,
        "test_type": "Training Step Builders",
        "summary": generate_summary(all_results),
        "detailed_results": all_results,
        "scoring": detailed_scores,
    }

    json_file = reports_dir / f"training_step_builders_report_{timestamp}.json"
    ...
    print(f"\n✅ Training step builders testing completed!")
...
def generate_score_chart(step_name, results, scoring_data, builder_class_name):
    ...
    fig.suptitle(
        f"{step_name} Training Step Builder Test Results",
        fontsize=16,
        fontweight="bold",
    )
    ...
    # Save chart
    chart_dir = (
        Path(__file__).parent / "training" / builder_class_name / "scoring_reports"
    )
