...
def generate_training_reports():
    ...
    for step_name, builder_class in available_builders:
        ...
        try:
            ...
            training_results = test_suite.run_training_specific_tests(step_name, builder_class)
            
            # Combine results
            if isinstance(universal_results, dict) and 'test_results' in universal_results:
                combined_results = {**universal_results['test_results'], **training_results}
...
def generate_score_chart(step_name, results, scoring_data):
    ...
    fig.suptitle(f'{step_name} Training Step Builder Test Results', fontsize=16, fontweight='bold')
