...
class TestOutputSpecAliases(unittest.TestCase):
    ...
    def test_alias_validation_removes_empty_strings(self):
        ...
        output_spec = OutputSpec(
            ...
            aliases=[
                ...
                "  ",
                "DATA",
            ],  # Remove None as it causes validation error
            output_type=DependencyType.PROCESSING_OUTPUT,
            property_path="properties.ProcessingOutputConfig.Outputs['processed_data'].S3Output.S3Uri",
        )
    ...
    def test_alias_validation_invalid_characters(self):
        ...
        self.assertIn(
            "should contain only alphanumeric characters", str(context.exception)
        )
    ...
    def test_step_specification_logical_name_conflicts(self):
        ...
        with self.assertRaises(
            ValueError
        ) as context:  # Changed from ValidationError to ValueError
            StepSpecification(
                step_type="TestStep",
                node_type=NodeType.INTERNAL,
                dependencies=[
    ...
    def test_case_insensitive_conflict_detection(self):
        ...
        output_spec2 = OutputSpec(
            ...
            aliases=[
                "processeddata"
            ],  # Case-insensitive conflict with output_spec1 alias
            output_type=DependencyType.MODEL_ARTIFACTS,
            property_path="properties.ModelArtifacts.S3ModelArtifacts",
        )
