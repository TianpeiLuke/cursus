...
class TestStratifiedSampler:
    ...
    def sample_data(self):
        ...
        df = pd.DataFrame({
            ...
            'variance_col': np.concatenate([class_0_data['variance_col'], class_1_data['variance_col'], class_2_data['variance_col']]),
    ...
    def test_sample_optimal_strategy_without_variance_column(self, sampler, sample_data):
    ...
    def test_sample_optimal_strategy_missing_variance_column(self, sampler, sample_data):
    ...
    def test_balanced_allocation(self, sampler, sample_data):
        ...
        assert total_allocated <= 300 + len(strata_info) * 10  # Allow for minimum constraints
...
class TestFileIOHelpers:
    ...
    def test_read_processed_data_success(self, temp_dir):
        ...
        # Test reading
        result = _read_processed_data(str(input_dir), "train")
    ...
    def test_save_sampled_data(self, temp_dir):
        ...
        _save_sampled_data(test_data, str(output_dir), "validation", mock_logger)
...
class TestMainFunction:
    ...
    def test_main_missing_strata_column_env_var(self, temp_dir, sample_data):
        ...
        with pytest.raises(RuntimeError, match="STRATA_COLUMN environment variable must be set"):
            main(
                input_paths=input_paths,
                output_paths=output_paths,
                environ_vars=environ_vars,
    ...
    def test_main_with_variance_column_warning(self, temp_dir, sample_data):
        ...
        input_dir = self.setup_input_data(temp_dir, data_without_variance, splits=["train", "val"])
    ...
    def test_main_default_logger(self, temp_dir, sample_data):
        ...
        # Set up input data with both train and val splits (required for training job_type)
        input_dir = self.setup_input_data(temp_dir, sample_data, splits=["train", "val"])
        output_dir = temp_dir / "output"
