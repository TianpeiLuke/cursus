...
class ScriptAlignmentValidator:
    ...
    def validate_single_script(self, script_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results['metadata'] = {
                ...
                'contract_mapping': self.script_mappings.get(script_name, f"{script_name}_contract"),
    ...
    def _print_script_summary(self, script_name: str, results: Dict[str, Any]):
        ...
        for level_num, level_name in enumerate([
            ...
            print(f"  {level_emoji} Level {level_num} ({level_name}): {'PASS' if level_passed else 'FAIL'}{issues_text}")
...
<body>
    ...
    def _print_final_summary(self, summary: Dict[str, Any]):
        ...
        if passed > 0:
            passing_scripts = [name for name, result in summary['script_results'].items() 
        ...
        if failed > 0:
            failing_scripts = [name for name, result in summary['script_results'].items() 
        ...
        if errors > 0:
            error_scripts = [name for name, result in summary['script_results'].items() 
