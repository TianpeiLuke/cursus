...
class VisualizedAlignmentValidator:
    ...
    def validate_script_with_visualization(self, script_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            json_output_path = (
                self.json_reports_dir / f"{script_name}_alignment_report.json"
            )
            ...
            html_output_path = (
                self.html_reports_dir / f"{script_name}_alignment_report.html"
            )
            ...
            results["metadata"] = {
                ...
                "reports": {
                    ...
                    "chart": str(
                        self.charts_dir / f"{script_name}_alignment_score_chart.png"
                    ),
                },
            }
    ...
    def run_all_visualized_validations(self):
        ...
        if all_scores:
            validation_summary["scoring_summary"]["average_score"] = sum(
                all_scores
            ) / len(all_scores)
    ...
    def _print_enhanced_final_summary(self, summary: Dict[str, Any]):
        ...
        if scoring_summary.get("average_score"):
            ...
            if score_dist:
                print(
                    f"   Score Range: {score_dist.get('min', 0):.1f} - {score_dist.get('max', 0):.1f}"
                )
