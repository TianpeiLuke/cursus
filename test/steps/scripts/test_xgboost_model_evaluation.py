...
class TestModelEvaluationHelpers(unittest.TestCase):
    ...
    def test_load_model_artifacts(self, mock_booster):
        ...
        model, risk_tables, impute_dict, feature_columns, hyperparams = load_model_artifacts(str(model_dir))
    ...
    @patch('cursus.steps.scripts.xgboost_model_evaluation.NumericalVariableImputationProcessor')
    ...
    def test_compute_metrics_binary(self):
        ...
        y_prob = np.array([[0.8, 0.2], [0.3, 0.7], [0.9, 0.1], [0.2, 0.8],
                          [0.1, 0.9], [0.7, 0.3], [0.4, 0.6], [0.6, 0.4]])
    ...
    def test_compute_metrics_multiclass(self):
        ...
        # Check that all expected metrics are present
        expected_keys = ['auc_roc_micro', 'auc_roc_macro', 'f1_score_micro', 'f1_score_macro']
        for key in expected_keys:
            self.assertIn(key, metrics)
    ...
    def test_get_id_label_columns(self):
        ...
        id_col, label_col = get_id_label_columns(df, 'nonexistent_id', 'nonexistent_label')
...
class TestModelEvaluationIntegration(unittest.TestCase):
    ...
    def test_evaluate_model_binary(self, mock_pr_curve, mock_roc_curve, mock_dmatrix):
        ...
        evaluate_model(
            mock_model, df, feature_columns, 'id', 'label', hyperparams,
            str(output_eval_dir), str(output_metrics_dir)
        )
