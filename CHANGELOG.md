...
- **New `PiperMetricGeneration` step type — `steps/{configs/config_piper_metric_generation_step.py, interfaces/piper_metric_generation.step.yaml, scripts/piper_metric_generation.py}`.** A new user-facing step (bringing the catalog to **47 steps**), a drop-in peer of `ModelMetricsComputation` that pipeline authors can add to a DAG: listed by the step catalog and buildable via a runtime-synthesized `PiperMetricGenerationStepBuilder`. The `.step.yaml` declares `node_type: internal`, `sagemaker_step_type: Processing` (sklearn compute), a required `eval_output` input (compatible sources: the XGBoost / LightGBM(MT) / PyTorch model-eval/inference steps plus `TabularPreprocessing` / `CradleDataLoading` / `RiskTableMapping` / `CurrencyConversion`), and a `metric_output` output. `PiperMetricGenerationConfig` (a `ProcessingStepConfigBase` subclass) adds three required fields — `id_name`, `label_name`, `variant_model_id` — with a validator requiring at least one of `score_field` / `score_fields`, supports single-task / multi-task / comparison (control-series) scoring, and its `get_environment_variables` emits the PIPER contract env vars (`VARIANT_MODEL_ID` / `CONTROL_MODEL_ID` / `DATASET_TYPE` / `METRICS_TO_RENDER` / `PIPELINE_NAME`). The `piper_metric_generation.py` script loads the eval predictions, requires the ground-truth label column (erroring with guidance if absent — e.g. `PyTorchModelInference` drops labels), recomputes ROC/PR curves and data statistics with scikit-learn, and writes PIPER `.metric` JSON files plus paired 2-column data CSVs flat to the output root (with `_SUCCESS` / `_HEALTH` / `_FAILURE` markers).
- **Opt-in `contract.output_path_token` override — `core/base/step_interface.py`, `core/base/builder_templates.py`, `steps/interfaces/io_view.py`, `mcp/tools/author.py`.** Re-introduces a declarable `ContractSection.output_path_token` field (`Optional[str] = None`, previously removed) as an explicit, default-off escape hatch: when set to a non-empty string it is used **verbatim** as the output-destination S3 prefix segment instead of the step-name-derived `canonical_to_snake(step_type)` token — changing only the emitted S3 output folder, not the SageMaker/DAG step name. Honored by the Processing/Training/Transform output-path handlers via `getattr(b.contract, "output_path_token", None)`, surfaced by `describe_step_patterns` (which reports an `output_path_token_source` of `"override: contract.output_path_token"` vs `"derived: canonical_to_snake(step_type)"`), and documented in the `author.rules` packaging guidance. It exists for external consumers keying off a fixed S3 folder — e.g. PIPER scanning `<pipeline>/Model_Metric_Generation_Step/` for `.metric` files (the value `piper_metric_generation.step.yaml` sets). Fully backward-compatible: the default `None` preserves every existing step's derived path.
- **Kiro workflow runtime — run the cursus dynamic workflows on `kiro-cli` — `src/cursus/mcp/workflows/kiro/`.** The shipped workflow scripts (`cursus-author-step.js`, `cursus-configure-pipeline.js`) target the Claude Code `Workflow` runtime: the `agent()` / `parallel()` / `pipeline()` / `phase()` primitives are injected by the Claude Code host, so the scripts are inert under `kiro-cli` (there is no `node <workflow>.js`). This adds a small, dependency-free Node runtime that re-implements those primitives on top of `kiro-cli`, so the **unmodified** workflow scripts run on Kiro — same phases, same gates, same MCP tools; each `agent()` call becomes one `kiro-cli` turn (a fresh sub-agent, matching Claude Code semantics). Files: `kiro-workflow-runtime.js` (the engine — primitives + `args`/`budget`, with schema-forced output emulated by a JSON-schema prompt suffix + parse/validate + bounded re-prompt, returning `null` on terminal failure), `run-workflow.js` (the CLI — loads an unmodified script, binds the primitives as globals, runs it; result to STDOUT, progress to STDERR; the Kiro counterpart to the Claude Code `Workflow` tool), `test-runtime.js` (offline tests via mock `kiro-cli`), and `README.md` (rationale, usage, transport, and a fidelity table vs the Claude Code host). Verified: offline tests pass, both real workflows load and run unmodified, and a live smoke test against `kiro-cli 2.10.0` returned schema-validated output. Fidelity caveat: the runtime *validates and re-prompts* schema output rather than tool-forcing it, so the workflows' own executable oracles (`py_compile`, `yaml.safe_load`, `json.load`, `validate.deps_resolve`) remain the real gates.
- **Schema-output shape coercion for the Kiro runtime.** Since Kiro can't tool-force structured output (the Claude Code host does), a capable model can still return the right JSON in the wrong container — e.g. `[{...}]` (single-element array) when the schema wants an object, staying stuck on that shape across generic re-prompts. The runtime now auto-coerces before validating (unwrap single-element array → object; wrap bare object → array when the schema wants one) without changing field values, and the re-prompt names a container mismatch loudly. `maxSchemaRetries` default raised 2 → 3, with a new `--schema-retries` flag. (This is distinct from a *weak* model returning prose — that needs a stronger model, e.g. `--model <id>`.)
- **Harness-conditional Resolve split for the Kiro runtime (Claude Code path unchanged).** A follow-on SAIS run surfaced a shape failure coercion cannot fix: Opus 4.8 answered `cursus-author-step`'s 15-field `PLAN_SCHEMA` (which discusses producer/NEW/consumer nodes) with a **multi-element** array `[{...},{...},...]` — one plan object *per node* — plus a per-turn timeout on the oversized turn. A genuinely multi-element array is ambiguous to unwrap, and re-prompts didn't move it. Fix is workflow-side and keyed to host capability: `run-workflow.js` now sets `__workflowHost.toolForcesSchemaOutput = false` on the sandbox, and `cursus-author-step` reads it — on the Claude Code `Workflow` host (marker undefined ⇒ tool-forcing) Resolve stays ONE `StructuredOutput`-forced `PLAN_SCHEMA` turn, byte-for-byte unchanged; only on the non-tool-forcing Kiro runtime does it split into three small single-object turns (locate → triage → identity, merged into the same `PLAN_SCHEMA`-shaped plan). Each small turn has one unambiguous object shape, so no per-node array and a far smaller/faster turn. New `kiro/test-author-step-e2e.js` runs the real workflow on the Kiro runtime to a green step and asserts the combined `PLAN_SCHEMA` turn is never emitted (a mock sentinel reproduces the SAIS array trap if it ever were); a separate harness check proves the Claude Code single-turn path is unchanged. Takeaway: a large schema a non-tool-forcing host can't reliably shape should be **decomposed by the workflow for that host**, not forced through coercion.
- **Explicit shape directive + identical-array coercion for the Kiro runtime (runtime-level, covers both workflows).** A further SAIS run showed Opus 4.8 on kiro-cli 2.5.0 wraps its answer in an array even for the *small* split `locate` schema — a general array bias the raw JSON Schema suffix does not override, appearing on the first attempt. Fixed in the runtime (not per-workflow) so it applies to every schema-gated turn in both workflows at once — `cursus-author-step`'s locate/triage/identity and `cursus-configure-pipeline`'s DagCheck + per-node Validate (which showed the same multi-element-array failure) — with no workflow edits and no model change. Three layers: (1) `shapeDirective(schema)` prepends an explicit natural-language instruction plus a `{ "field": ..., ... }` skeleton to the INITIAL prompt ("Return EXACTLY ONE JSON object … do NOT return a list of candidates or alternatives … pick the single best"); (2) `coerceToSchema` now also unwraps a multi-element array whose elements are all IDENTICAL (the model emitted the same answer N times) to the first object, while a multi-element array of DIFFERENT objects stays ambiguous and falls through; (3) the re-prompt names an object-schema-answered-with-an-N-element-array as exactly that ("a JSON ARRAY of N items … pick one, not a list"). Field values are never changed. Tests: 46 runtime (adds `shapeDirective` + multi-element coercion units and two e2e cases) + 12 author-step e2e.
- **Tolerant JSON parse + cursus MCP wiring for the Kiro runtime's post-Resolve phases (RC#3).** After the Resolve split, a SAIS run (Sonnet 4.6) cleared Resolve and authored all 3 artifacts, but the post-Resolve phases (Challenge, AlignEdges, validate, check_script, resolve, preflight, and configure-pipeline's DagCheck + per-node Validate) still failed on schema/JSON-parse. Two runtime-level fixes, covering both workflows with no workflow edits. (A) **Tolerant JSON parse** — new `tolerantParse(text)` tries strict `JSON.parse` FIRST (valid JSON is returned untouched and never mutated), and only on failure applies SAFE context-aware syntax repairs (strip `//` and `/* */` comments, remove trailing commas, convert single-quoted strings, normalize smart/curly quotes, map Python `True`/`False`/`None`/`NaN`/`Infinity`, quote unquoted keys) then re-parses; each repair provably can't corrupt valid JSON since valid JSON already returned. It reports the defects fixed, so the parse-failure re-prompt now names them instead of echoing an opaque `SyntaxError` the model repeated across retries. (B) **cursus MCP wiring** — the post-Resolve phases are *relay-tool-result* prompts ("call `author.*`/`validate.*`/`steps.io`/`catalog.*` and return its JSON"); the ACP client opened `session/new` with `mcpServers:[]` and no server was registered anywhere, so kiro-cli had no tools and fabricated the JSON. The runtime now accepts `mcpServers`/`mcpCursus` config, the ACP client encodes them into `session/new` (env as the ACP `[{name,value}]` array), and `run-workflow.js` adds `--mcp-cursus` (auto-register `cursus mcp serve`), `--mcp-python <cmd>`, and repeatable `--mcp-server '<json>'`; headless warns once if servers are set without `--agent`. Registering the server is the durable fix; the parse/coerce/re-prompt layers remain the safety net. Tests: 67 runtime (adds `tolerantParse` units for every defect class + the strict-first "valid JSON untouched" guarantee, ACP `mcpServers` wire-shape, `mcpCursus` auto-register, and an ACP e2e asserting `--mcp-cursus` reaches `session/new`) + 12 author-step e2e.
- **Guard the kiro-cli 2.5.0 ACP-MCP crash (SAIS Run 8).** The RC#3 MCP-wiring delivered via ACP `session/new` works on 2.10.0 but **crashes** the frozen 2.5.0 ACP server (it exits code 0 the moment it receives a non-empty `mcpServers` payload), and `--mcp-cursus` is a no-op in headless (kiro-cli reads MCP from its `--agent` config, not a per-call flag). Fixed by gating on `allowNewFlags` (the same version-skew pattern as `--effort`): under `--legacy-kiro`, the ACP client DROPS the `session/new` `mcpServers` payload entirely (one-time warning) rather than sending the crash trigger; and the runtime, when it can't deliver MCP (headless without `--agent`, or acp+`--legacy-kiro`), prints the exact `~/.kiro/settings/mcp.json` snippet + the working headless-`--agent` invocation instead of steering to `--transport acp`. Docs corrected: `--mcp-cursus` is kiro-cli ≥ 2.10.0 (+acp) only; on 2.5.0 the sole MCP route is a static `~/.kiro/settings/mcp.json` + `--transport headless --agent`.
- **Tool-forcing structured output — `submit_result` MCP server + SAIS probe (offline-safe half; not yet wired).** The heuristic layers (shape directive, tolerant parse, coercion, re-prompt) *salvage* free-text JSON; they don't *force* it the way the Claude Code host does (a `StructuredOutput` tool call whose parameters ARE the schema). Internal code-search research (4 independent kiro-cli ACP client implementations + the ACP spec) established that tools reach kiro-cli ONLY via MCP servers (no inline ACP tool declaration), and on 2.5.0 that must be a static config + headless `--agent`. This ships the analog: **`submit-result-server.js`**, a dependency-free stdio MCP server exposing one tool `submit_result` whose `inputSchema` IS the phase schema — kiro-cli validates the model's arguments against it before dispatch (killing the array-bias at the source), and the runtime (being the server) receives the validated arguments directly, no stdout scraping. `runtime.buildSubmitResultConfig()` emits the 2.5.0-safe static `mcp.json` + prompt suffix; **`run-toolforce-probe.js`** is a one-command SAIS probe that reports GREEN/RED on whether 2.5.0 headless actually runs the tool loop. Shipped offline-safe and mock-tested (in-process handler + a real stdio round-trip), but deliberately **NOT wired into `agent()` as the primary path** until the probe returns green on SAIS — the same "don't ship an unprobed wire mechanism" discipline the Run-8 crash taught. Tests: 85 runtime + 12 author-step e2e.
- **Drop MCP on the legacy (kiro-cli 2.5.0) path + decompose the post-Resolve phases.** A SAIS run (Run 10) proved kiro-cli 2.5.0 loads NO MCP servers in any mode — headless ignores `~/.kiro/settings/mcp.json` (built-in tools only) and its ACP crashes on registration (Run 8). So MCP tool-forcing is dead on that build; it works only on kiro-cli ≥ 2.10.0 (validated locally). Two harness-conditional changes result (Claude Code path byte-for-byte unchanged). (1) **MCP dropped on the legacy path** — with `--legacy-kiro`, `runtime._mcpServers()` returns `[]` with a one-time warning and the workflow phases use their built-in `cursus` CLI fallback (passing servers would be a no-op/crash on 2.5.0); on ≥ 2.10.0 MCP still works (headless via `--agent` config, ACP via `session/new`). (2) **Post-Resolve schema decomposition** — the AlignEdges and Validate re-resolve phases carry `edges[]` arrays-of-objects that triggered the multi-element-array bias on 2.5.0 (the AlignEdges/resolve failures in Runs 5-6); on the non-tool-forcing host `cursus-author-step` now scores ONE edge per turn (`EDGE_ALIGN_SCHEMA` + a small `ARITY_SCHEMA` turn; `EDGE_RESOLVE_SCHEMA`) and assembles the `ALIGN_SCHEMA`/`RESOLVE_SCHEMA` shape in plain code, so no `edges[]` array is ever requested from the model. Vacuously-ready for a source/sink/singular node, matching the single-turn contract. Under Claude Code both stay single tool-forced turns (gated on `HOST_TOOL_FORCES_SCHEMA`). Tests: 85 runtime + 14 author-step e2e (now asserts the Kiro path runs one per-edge align turn per real edge and never emits the single-turn array prompt; a separate harness check confirms the Claude Code single-turn path is unchanged).
- **Fix a control-flow regression in the per-edge AlignEdges decomposition (SAIS Run 11).** When the per-edge align turns returned non-JSON on 2.5.0 (Sonnet emitted `[step_type=...` / `[OPTIONS]`), the decomposition still built a non-null `align` with `ready:false`, so the workflow short-circuited before Author and produced 0 artifacts — whereas the pre-split single-turn path returned `null` on failure and PROCEEDED to Author (the authoritative edge gate is the post-write re-resolve at Stage 4c + the executable oracles). Fix: Stage 2 now distinguishes INCONCLUSIVE (fewer edges scored than expected → `align=null`, proceed) from DEFINITIVE (all edges scored, some don't resolve → short-circuit); the short-circuit condition tightened from `!align.ready` to `align.ready === false` so a null align never blocks. Stage 4c (the final `green` verdict) stays strict — `both_edges_resolve` is false when edges can't be scored, correctly withholding green. Tests: +3 e2e (17 total) including a Run-11 regression case that returns non-JSON for the per-edge align turns and asserts a step is STILL authored.
- **Reroute the failing relay-tool-result phases through the cursus CLI on kiro-cli 2.5.0 (the path to green WITHOUT MCP).** Run 11's `[OPTIONS]` / `[step_type=...` output was the model ALREADY reaching for the cursus CLI via its built-in shell/execute tool (which works on 2.5.0 — Author writes files, the parse phase runs `py_compile`), just forgetting `--format json` and args. Since MCP is dead on 2.5.0 (Run 10) but the CLI + shell tool are alive, the relay phases are rerouted to prescriptive CLI commands — harness-conditional (Claude Code keeps its MCP-tool prompts byte-for-byte; only the non-tool-forcing Kiro path reroutes). `cliJson(cmd)` returns the mandatory clean-JSON recipe (`PYTHONNOUSERSITE=1 python3 -m cursus.cli <cmd> --format json`, stderr discarded, piped through `sed -n '/^{/,$p'` — the sed filter is REQUIRED because the `sagemaker.config INFO` lines print to stdout and survive stderr-discard, verified live). Per phase: Validate/validate → `validate step-interface` (mapping `results[0]`, since top-level errors/warnings are counts); Challenge → `catalog show` + `steps io`; AlignEdges → `steps io <adjacent>`; Guide → `strategies knobs` + `steps io` + `steps patterns` + `catalog fields` (author.rules/config_constraints have no CLI → reason from the exemplar per HABIT-1); check_script → no CLI → `status:"skipped"` (publish path permits skip, PARSE oracle backstops, re-runs at CR); Validate/resolve → **deleted the phantom `cursus validate deps-resolve` command (it does not exist and hard-errored)** and reason the edge from two `steps io` JSON blobs; Preflight → reason constructibility from the parsed `.step.yaml` + specs. The `both_edges_resolve` + `constructible` green gates are reasoned/relaxed on the frozen host (noted in-run) and re-verified by the real resolver/preflight at CR. Tests: 85 runtime + 20 author-step e2e — adds a LIVE proof that the `cliJson` recipe yields parseable JSON from the real cursus CLI and that a bare stderr-discard does NOT (the sed filter is mandatory); a CC-path check confirms zero CLI-reroute leakage and all MCP calls kept.
- **Fix the missing-required-field failures the CLI reroute exposed (SAIS Run 13).** With the reroute, Run 13 was major progress — `check_script` and `preflight` passed for the first time (`constructible=True`) — and the failure mode SHIFTED from "unparseable JSON" to "required property missing" (`$.holds`/`$.edge`/`$.ok`/`$.axis`): the model now returns valid JSON but drops a required field. Two fixes. (A) **Named missing-field re-prompt** (`kiro-workflow-runtime.js`): when the only errors are missing-required-properties, the re-prompt names the exact dropped keys loudly with each one's expected type/enum ("missing REQUIRED field(s): holds (boolean); …") instead of burying `$.holds: required property missing` in an 8-line list the model ignored. (B) **Inject the edge label the runtime already knows** (`cursus-author-step.js`): the per-edge align/resolve turns kept dropping `edge` — the one field the runtime dictates ("ALIGN EXACTLY ONE EDGE: <edge>") — so `edge` was removed from `EDGE_ALIGN_SCHEMA`/`EDGE_RESOLVE_SCHEMA` `required` and is injected after parse, making a dropped `edge` un-failable (deterministic, not model-dependent; the CC single-turn schemas still require it). Tests: 86 runtime (adds a missing-field e2e that recovers only when the re-prompt names the field) + 20 author-step e2e (the mock now OMITS `edge` and the workflow still reaches green).
- **Two research-backed structured-output levers for the multi-field-synthesis failures (SAIS Run 14).** Run 14 showed the named re-prompt + edge injection did NOT move the 6 remaining phases (Challenge/Guide/validate/gaps/edge-align/edge-resolve) — the failure is whole-object SYNTHESIS (a multi-field object with array members), not a single nameable dropped field. A search (internal SAGE 372052/373135/373070 + code patterns like VelociraptorDocker `_coerce_to_schema`; external fill-in-the-blank / few-shot / json-repair) converged on two runtime-level levers, one edit covering all phases, harness-conditional, and structurally unable to fake a green. (1) **Typed-example skeleton** in `shapeDirective()` — replaces the bare `{ "holds": ... }` with a recursive TYPED skeleton (string→`"..."`, boolean→`true`, number→`0`, array→`[ <elem> ]`, enum→first literal, nested→recursed) plus "keep EVERY key, replace only the values" and an explicit "EVERY one of these keys is REQUIRED: …" line; because every key is pre-written the model can't drop the field set and the pinned braces re-anchor object-not-array (the most-cited cross-model shape lever, SAGE 373135/372052). (2) **`backfillDefaults()` before validation** — injects `[]` for omitted array props and `""` for omitted OPTIONAL string props, but NEVER a required boolean/enum/number/string (those carry the decision and must come from the model, so a dropped verdict still fails validation — cannot fake a green; unit-tested). Plus a small third lever: Guide `axis` injection (runtime-dictated, like `edge`; fixes the 0/6 `$.axis`-missing failures). Tests: 98 runtime (typedSkeleton units; backfill safety units proving required scalars are never fabricated while omitted arrays/optional-strings are filled; an e2e where the model omits an optional array and it backfills to `[]` first-try) + 20 author-step e2e; CC path re-confirmed single-turn + green.
- **`cursus dag resolve` CLI + move validate/resolve off the model into code — breaks the SAIS Run-15 "hard ceiling."** Run 15 was the 3rd consecutive negative (all three prior fixes were prompt-shaping); the two remaining green blockers (`validate.ok`, `resolve.both_edges_resolve`) turned out to be a RELAY (copy a CLI's JSON) and a DETERMINISTIC formula (the 6-component resolution score), not open-ended synthesis — so the fix is to move that burden off the model into runtime code, which no prior run tried. **New `cursus dag resolve <step>... --format json`** (`src/cursus/cli/dag_cli.py`) runs the REAL `UnifiedDependencyResolver` (same weights, 0.5 threshold, and `SemanticMatcher` the compiler/CI use) and emits per-edge `{consumer, dependency, provider, score, resolves}`. In `cursus-author-step.js` (non-tool-forcing / Kiro branch only, harness-conditional — the Claude Code path keeps its MCP `validate.step_interface` + `validate.deps_resolve` unchanged): the `validate` turn now returns `cursus validate step-interface --format json` VERBATIM and the runtime projects `results[0]` in code; the re-resolve turn FETCHES `cursus dag resolve` JSON and `matchResolvedEdge` maps the real per-edge scores onto producer→NEW / NEW→consumer, computing `both_edges_resolve` in code (a new `RELAY_JSON_SCHEMA` makes these turns parse-only). This preserves H4 — both gates run the same real oracles CI runs, no model arithmetic and no drifting JS re-implementation of the resolver. Tests: new `tests/cli/test_dag_cli.py` (5, including a drift-guard asserting the CLI score equals the resolver's score for the same specs); +6 runtime units for `matchResolvedEdge` (name-matching, best-of-ties, sub-threshold `resolves:false`, null-when-unscored) = 104 runtime; the author-step e2e reaches GREEN via the CLI-reroute path = 20.
- **`cursus dag resolve` relay accepts a bare array on frozen kiro-cli.** Follow-on to the Run-15 CLI reroute: `cursus-author-step`'s resolve-relay turn loosened `RELAY_JSON_SCHEMA` from `{ type: 'object' }` to `{}` (accept any valid JSON), and the handler now maps a top-level array (`Array.isArray(raw) ? raw : []`) in addition to `{ edges: [...] }`, so the model relaying `cursus dag resolve` output as a bare array parses on kiro-cli 2.5.0. Kiro-path only; the Claude Code path is unchanged.
- **Version-skew docs corrected for locked-down environments.** Upgrading `kiro-cli` is not always possible — a no-egress environment (e.g. a Red-certified VPC whose install pins an old version) cannot pull a newer release, so the README no longer prescribes "install >= 2.10.0" as the universal fix. In such environments the fix must be version-independent: pin a capable model with `--model <id>`, and/or reduce what one turn demands (trim the prompt, split a large schema into smaller sequential turns); upgrading is framed as a separate-machine benchmark option only.
- **Runtime warns when `--effort` is dropped on old kiro-cli, and the README documents the capability gap.** A frozen `kiro-cli 2.5.0` build has no `--effort` flag, so `--legacy-kiro` drops it — which silently under-powers any step that requested a high reasoning effort (e.g. the `cursus-author-step` **Resolve** phase, which needs a strong model for a 15-field schema off a large prompt; a weak model returns prose instead of JSON and the schema turn fails). The runtime now emits a one-time warning when this happens, and the README's "Version skew" section states plainly that a fair authoring run needs `kiro-cli >= 2.10.0` (simple/low-schema workflows still run on 2.5.0 with `--legacy-kiro`).
- **Kiro workflow runtime handles kiro-cli version skew (2.5.x ↔ 2.10.x).** The runtime was captured on `kiro-cli 2.10.0`, but frozen environments (e.g. the SAIS SageMaker snapshot) run `2.5.0`, which lacks some flags/entry points. Both transports now degrade cleanly: `--legacy-kiro` emits only the 2.5.0-safe flag set (drops `--effort` + granular `--trust-tools`; keeps `--no-interactive`/`--trust-all-tools`/`--agent`/`--model`); `--acp-entry chat-binary` (+ `--kiro-chat-bin`) drives ACP through the separate `kiro-cli-chat` binary that 2.5.0 ships instead of the `kiro-cli acp` subcommand; and the ACP `protocolVersion` is requested (`--acp-protocol`, default 1) but re-negotiated to whatever the server echoes in `initialize`. Offline arg-shape tests assert the 2.5.0-safe shape without needing a live old binary. See the runtime README's "Version skew" section.
- **Two transports for the Kiro workflow runtime — headless and ACP — selectable with `--transport`.** `headless` (default) spawns one `kiro-cli chat --no-interactive` process per `agent()` turn (independent processes → true `parallel()` fan-out). `acp` holds ONE long-lived `kiro-cli acp` process and runs one Agent-Client-Protocol session per turn (JSON-RPC 2.0 over newline-delimited stdio), via a dependency-free client (`kiro-acp-client.js`) hand-rolled after the internal `AGIArsenalConsole` pattern — no `@agentclientprotocol/sdk`. The ACP path does `initialize` (protocol version 1) → `session/new` → `session/prompt`, accumulates streamed `session/update` `agent_message_chunk` text to the `stopReason`, auto-approves `session/request_permission`, and ignores `_kiro.dev/*` notifications; the wire contract was captured live from `kiro-cli 2.10.0`. Use ACP to amortize process/agent-init cost across long sequential runs. Caveats: `--agent`/`--model`/`--effort` are session-level under ACP (per-call overrides apply only in headless), and ACP turns are serialized on the single process (headless gives true wall-clock parallelism). Verified with a live workflow run over ACP (handshake, schema-validated turn, parallel turns over one persistent process, clean teardown) plus a mock-ACP-server offline test.

## [2.2.1] - 2026-06-30

### Fixed

- **`cursus-author-step` no longer silently clones the exemplar and drops a new step's distinguishing feature.** A known-answer run (authoring `TSATabularPreprocessing` against a real pipeline whose expected interface was known) exposed a fidelity miss: the workflow's Resolve/Challenge phases correctly identified *why* the node was a NEW step (it needs a `preprocessor` output the generic `TabularPreprocessing` exemplar lacks), but the Author phase copied the exemplar shape and dropped that output — reproducing the exemplar and defeating the step's reason to exist. Fixed in three parts, none touching the compile/runtime path (the workflow is a reference `.js` orchestration script): (1) **Resolve now emits a required `divergences_from_exemplar[]`** — the concrete deltas that justified rejecting the reuse rung (new prompt step + schema field); if a step is `new_step` with no nameable divergence, the reuse ladder was mis-applied. (2) **Author is reframed from "copy the exemplar shape" to exemplar-PLUS-deltas** — the divergences are injected as REQUIRED additions the agent applies on top of the exemplar and then self-confirms, with an explicit "a dropped divergence is a BUG" instruction. (3) **Validate gains a divergence-fidelity gate** (bounded fix loop) that reads the authored artifacts, confirms every declared divergence is actually present (e.g. an "ADD a `preprocessor` output" delta needs both `contract.outputs.preprocessor` and `spec.outputs.preprocessor` plus the script writing it), and loops a targeted fix if any is missing. A step with a missing divergence is no longer reported green, and the run report surfaces `divergences_required` / `divergences_present` / `divergences_missing`.
...
- **`author.*` MCP namespace (new) — `src/cursus/mcp/tools/author.py`.** Six tools that turn the "add a step" SOP into a tool-calling loop: `author.checklist(sagemaker_step_type[, step_assembly])` (the ordered SOP as data, with the bound `PatternHandler` + a same-phase exemplar DERIVED from the live `strategy_registry` so it can't drift), `author.rules(topic)` (a restriction oracle that introspects the live enforcement objects for `naming` / `packaging` / `sdk_carveout` / `reuse_class` / `closure`), `author.config_constraints(step_name)` (each config field's `allowed_values` + `case_sensitive` + declared type + the required-no-default set), `author.preflight_config` (offline `model_validate` of a value set), `author.preflight_step` (the offline constructibility proof — the same gates CI runs: interface + registry parity + `RegistryBindingValidator` B3 + routability), and `author.check_script` (script↔contract alignment in BOTH directions — the builder's `--args` are argparse'd and required env vars are read). Registered via `mcp/registry.py`; served alongside the existing namespaces.
- **`validate.step_interface` promotion.** The author-time interface gate is a verbatim promotion of `cursus validate step-interface` into the MCP surface, so the agent gate, the CLI gate, and the CI `validate step-interface --all` gate are literally one code path (green-local ⇒ green-CI).
- **Shipped dynamic-workflow orchestrators — `src/cursus/mcp/workflows/`.** Two Claude Code dynamic-workflow scripts (reference/runnable artifacts, not importable Python) that drive the loop deterministically, plus a self-contained README. `cursus-author-step.js` is **DAG-driven**: its input is a `PipelineDAG` in which one node is a NEW step sitting between a producer and a consumer, and it aligns both edges (the new dependency-spec to the producer output-spec, the new output-spec to the consumer dependency-spec — each must resolve at the resolver's 6-component compatibility score ≥ 0.5, editing the consumer's `compatible_sources` + output aliases for room) before authoring, then gates on `validate.step_interface` + `author.check_script` + an executable py_compile/yaml parse oracle + a whole-DAG completeness critic. `cursus-configure-pipeline.js` drives `DAGConfigFactory` config.json generation gated per-node by `author.config_constraints` + `author.preflight_config`, then a cross-node **DagCheck** phase.
- **ScriptAnalyzer reverse-alignment checks — `validation/alignment/analyzer/script_analyzer.py`.** Beyond the existing forward (script-uses → contract-declares) direction, the analyzer now runs reverse checks (contract-declares → script-parses/reads): `extract_argparse_flags`, `extract_env_reads` (two-tier all-vs-consuming), and `validate_reverse_alignment(contract, sagemaker_step_type)` (Category 4 Processing-gated + Category 5), so a custom script missing the `--job_type` argparse the builder passes, or a required env var declared-but-unread, is caught at author time.
- **Field-constraint extraction — `api/factory/field_extractor.py`.** `extract_field_constraints` surfaces each config field's closed value set (from `Literal` / `Enum` + an AST scrape of `allowed={...}` in validator source) and `case_sensitive`, and `extract_field_requirements` attaches `allowed_values` — the data `author.config_constraints` serves.
- **`PipelineDAG.validate_node_declarations()` + `strict` mode — `api/dag/base_dag.py`.** `add_edge` silently auto-creates any endpoint not already `add_node`'d, so a single edge-name typo spawns a phantom unconfigured node and orphans the real one (construction never raises; the serializer's dangling-edge check is fooled because the phantom is already in `nodes`). `validate_node_declarations()` reports every edge endpoint never explicitly declared (the only reliable typo detector), and `PipelineDAG(strict=True)` turns the same condition into a `ValueError` at `add_edge`/constructor time. `PipelineDAGResolver.validate_dag_integrity` gains an `undeclared_edge_nodes` issue category so `dag.validate_integrity` surfaces it.

### Changed

- **`registry.sagemaker_step_type` is now a closed-enum Pydantic gate.** A field audit found this routing key — the one string that decides Processing-vs-Training-vs-… — loaded any typo silently. `RegistrySection._SAGEMAKER_STEP_TYPES` is pinned and validated in the model validator (with a conformance test keeping the pin equal to the live `get_valid_sagemaker_step_types()`), so a mistyped routing verb is caught at author time instead of mis-routing at build.
- **Config enum validators normalized case-insensitive.** 29 validators across the step config classes (e.g. `output_format` `{CSV,TSV,Parquet}`) now normalize case consistently so a legal value in the wrong case no longer hard-fails a config it should accept.
- **11 step scripts standardized to canonical contract keys** (xgboost / pytorch / lightgbm / lightgbmmt model_eval + inference, model_metrics_computation, model_wiki_generator, temporal_split_preprocessing) so the script's I/O keys match the `.step.yaml` contract logical names exactly.
- **`dag.*` MCP tool no longer unions edge endpoints into the node list.** `_build_dag` previously merged edge endpoints into `nodes` to avoid a KeyError; the constructor now tolerates dangling edges (via `setdefault`) without that union, so `dag.validate_integrity` can flag an edge endpoint the caller never declared instead of masking it.

## [2.1.0] - 2026-06-29

A step-script and interface hardening tranche covering **31 step scripts**, **30 `.step.yaml` interfaces**, **19 processing modules**, and **2 test files**. Three threads: (1) a defensive fail-fast sweep that turns opaque downstream crashes (NoneType, divide-by-zero, missing-file, malformed-JSON) into descriptive `ValueError` / `ImportError` / `RuntimeError` with operator guidance, and removes silent successes; (2) an env-var contract alignment pass that relaxes previously-required env vars to optional-with-defaults and enumerates the knobs each script actually reads; and (3) a Bedrock structured-output upgrade that makes model output schema-aligned by default via Anthropic `tool_use`. Predominantly additive and backward-compatible (minor bump) — but a handful of **behavior-changing default values** are called out under Changed: pipelines that relied on the old defaults must now set them explicitly.

### Added

- **Interface env-var contracts now declare the knobs their scripts read.** Across the `.step.yaml` interfaces, the `env_vars.optional` blocks were expanded to enumerate previously-undeclared environment knobs: Bedrock retry/rate-limit/truncation settings (`BEDROCK_MAX_RETRIES`, `BEDROCK_TOP_P`, `BEDROCK_ADAPTIVE_RATE_LIMITING`, `BEDROCK_BATCH_MODE`, `BEDROCK_TRUNCATION_ENABLED`, …), streaming/worker controls (`ENABLE_TRUE_STREAMING`, `MAX_WORKERS`, `BATCH_SIZE`, `STREAMING_BATCH_SIZE`), PyTorch device tuning (`DEVICE=auto`, `ACCELERATOR=auto`, `NUM_WORKERS`), eval comparison flags (`COMPARISON_METRICS`, `COMPARISON_PLOTS`), and imputation/feature/risk-table strategy knobs (`DEFAULT_TEXT_STRATEGY`, `AUTO_DETECT_CATEGORICAL`, `SMOOTH_FACTOR`, `COUNT_THRESHOLD`, …).
- **`WORKING_DIRECTORY` defaults** added to the MIMS `package` (`/tmp/mims_packaging_directory`) and `payload` (`/tmp/mims_payload_work`) contracts.
- **Runtime-discovered env vars documented.** `missing_value_imputation` now describes per-column `COLUMN_STRATEGY_<column_name>` overrides and `payload` documents `SPECIAL_FIELD_<field_name>` overrides in their contract descriptions rather than leaving them undeclared.
- **User-supplied-package boundary documentation.** The PyTorch scripts (`pytorch_training.py`, `pytorch_model_eval.py`, `pytorch_model_inference.py`) and the LightGBM-MT scripts (`lightgbmmt_training.py`, `lightgbmmt_model_eval.py`, `lightgbmmt_model_inference.py`) gain `OPEN SECTION` banners clarifying the cursus-owned vs. user-implemented boundary, with import guards that re-raise an instructive `ImportError` pointing at the package layout the user must bundle into `source_dir`.
- **`bspline_calibration.py` documented as an alternative `ModelCalibration` entry point.** A module banner clarifies it is fully interchangeable with `model_calibration.py` — it satisfies the same `model_calibration.step.yaml` interface and sets `CALIBRATION_METHOD=bspline` where `model_calibration.py` defaults to `gam` — and deliberately has no `.step.yaml`/registry entry of its own.
- **Bedrock structured output via tool-calling in batch processing.** `bedrock_batch_processing.py` now forces schema-aligned model output by issuing an Anthropic `tool_use` request — a forced `structured_response` tool whose `input_schema` is the step's `validation_schema` — bringing it to parity with `bedrock_processing.py` (0% JSON parse failures). A shared `_build_request_body` applies the same request shape to **both** the realtime invoke path and the async batch JSONL `modelInput` records, and response parsing serializes the returned `tool_use` block's `input` so downstream handling is identical. Falls back to the legacy assistant-prefill heuristic when no `validation_schema` is present or structured output is disabled.

### Changed

- **Required-to-optional env-var relaxation across interfaces.** Previously-required fields — `ID_FIELD`, `LABEL_FIELD`, `TRAIN_RATIO`, `TEST_VAL_RATIO`, `TEXT_FIELD`, `SELECTION_STRATEGY`, `CALIBRATION_METHOD`, `CURRENCY_CONVERSION_VARS`, `SEQUENCE_GROUPING_FIELD`, `TIMESTAMP_FIELD`, `TEMPORAL_FIELD`, `VALUE_FIELDS`, `TASK_LABEL_NAMES`, and others — were moved into `env_vars.optional` with inline defaults, so step contracts no longer hard-fail when these are unset.
- **Default-value changes a pipeline author would observe.** `temporal_sequence_normalization`: `SEQUENCE_LENGTH` 100 → 51, `PADDING_STRATEGY` `zero` → `pre`. `feature_selection`: `N_FEATURES_TO_SELECT` 50 → 10, `FEATURE_SELECTION_METHODS` JSON-list → comma-string `variance,correlation,mutual_info,rfe`. `active_sample_selection`: `MAX_SAMPLES` 10000 → 0. `model_metrics_computation`: `AMOUNT_FIELD` `order_amount` → empty. `model_calibration`: `MULTICLASS_CATEGORIES` `[0, 1]` → empty. **Pipelines relying on the prior defaults must set these explicitly.**
- **`BEDROCK_USE_STRUCTURED_OUTPUT` now defaults ON** in both `bedrock_processing` and `bedrock_batch_processing` (previously `false`/unset → assistant-prefill heuristic). When a `validation_schema` is present, Bedrock output is now schema-enforced via `tool_use` by default; set `BEDROCK_USE_STRUCTURED_OUTPUT=false` to restore the legacy prefill behavior.
- **`pytorch_model_inference.step.yaml` `entry_point` corrected** `pytorch_inference.py` → `pytorch_model_inference.py` (the prior value referenced a non-existent script).
- **`temporal_sequence_normalization.py` made config-driven.** Module-global constants (`MISSING_INDICATORS`, `TEMPORAL_FIELD`, `SEQUENCE_SEPARATOR`, `OUTPUT_FORMAT`, `INCLUDE_ATTENTION_MASKS`, …) were replaced with explicit parameters threaded through the operation classes and helpers (`SequencePaddingOperation` gains `include_attention_masks`), with `entity_id_field`/`id_field` derived from env-driven `sequence_grouping_field`/`record_id_field` so operations honor runtime config instead of globals.
- **Exception/diagnostic hygiene.** `dummy_data_loading.detect_file_format` replaces three bare `except: pass` blocks with specific exception types and `logger.debug` messages; `label_ruleset_execution` narrows broad `except Exception` to `(KeyError, TypeError, ValueError, AttributeError)` with row/exception context so unexpected errors propagate; `streaming_risk_table_processor` and `streaming_numerical_imputation_processor` move `logger` definition above the `tqdm` import fallback to avoid a `NameError` on its log path.
- **`TemporalMaskProcessor.__init__`** reorders the reserved, currently-unused `mask_value` to the trailing keyword position (after `padding_value`/`output_format`) so positional callers are unaffected, and documents that the mask is derived as `(input != padding_value)` — i.e. `mask_value` is presently inert.

### Fixed

- **Numeric/edge-case safety in data-stage scripts.** `currency_conversion` falls back to a 1.0 rate (with one aggregated warning) on non-positive `conversion_rate` instead of emitting inf/NaN; `dummy_training.create_tarfile` guards its compression-ratio log with `if total_size > 0`; `feature_selection` guards empty score dicts against `ZeroDivisionError` (substituting `[0.0]`) while `pseudo_label_merge.extract_split_ratios` now raises `ValueError("Base splits must contain non-empty data")` on empty input instead of dividing by zero; `missing_value_imputation` treats NaN skewness as not-skewed and recommends `mean`.
- **Fail-fast validation of required env/config.** `bedrock_processing` (`BEDROCK_PRIMARY_MODEL_ID`), `redshift_data_loading` (`CLUSTER_SPECIFICATION`/`QUERY_SPECIFICATION`), `model_wiki_generator` (`metrics_input`/`wiki_output` dirs), and `lightgbm_model_eval` (model/eval/output dirs before `os.makedirs`) now raise descriptive `ValueError` instead of failing opaquely later.
- **Malformed-JSON handling.** `payload.extract_hyperparameters_from_tarball` and `stratified_sampling` (both `reference_counts.json` and the `REFERENCE_COUNTS_JSON` env var) convert `json.JSONDecodeError` into a clear `ValueError` naming the offending source; `temporal_split_preprocessing` hardens `BATCH_SIZE` parsing (empty → default 10, non-integer → `RuntimeError`).
- **Artifact/feature validation in model scripts.** `lightgbm_model_eval.load_model_artifacts` wraps each artifact load in a descriptive `ValueError` and skips malformed `<2-field` `feature_columns.txt` lines with a warning; `xgboost_model_eval` raises when none of the model's expected feature columns are present (replacing a silent zero-column inference); `percentile_model_calibration` guards divide-by-zero on duplicate consecutive percentiles and raises when `task_calibrations` is empty (no longer reporting an all-skipped run as success); `bspline_calibration` coerces score/label via `pd.to_numeric(errors='coerce')`, drops/logs invalid rows, and fails gracefully with a `No valid numeric rows` result.
- **Dropped-conversion bug fixed in inference scripts.** `xgboost_model_inference.save_predictions` and `lightgbm_model_inference.save_predictions` now assign the float cast back (`output_df_json[col] = output_df_json[col].astype(float)`) so float columns are actually converted before JSON export.
- **Missing runtime dependency pins** added to `xgboost_training` (`xgboost==1.7.6`, `numpy>=1.19.0`, `boto3>=1.26.0`, `matplotlib>=3.0.0`).
- **Falsy-value correctness in processing.** `categorical_imputation_processor` drops the placeholder `"My Text String"` default and uses explicit `key in` checks so legitimately-falsy configured defaults (`0`, `False`, `""`) are honored; `multiclass_label_processor` switches its guard to `label_list is not None` and adds `_normalize()` so `1`, `1.0`, and `"1"` map to the same key; `pipeline_iterable_datasets` removes the `if self.cat_field_list` gate so an empty list no longer skips numeric coercion.
- **NaN-safe fitting and fitted-state integrity.** `numerical_categorical_processor` switches to `nanmin`/`nanmax`/`nanpercentile` with all-NaN and zero-variance guards, and `minmax_scaling_processor` switches to `nanmin`/`nanmax` with a zero-variance/divide-by-zero guard, so a single NaN can't corrupt learned params; `fit()`/from-params paths in `numerical_categorical_processor`, `numerical_binning_processor`, and `numerical_imputation_processor` refuse to set `is_fitted` unless all state `transform()` needs is present (raising `ValueError` for missing custom bins/thresholds, all-NaN columns, NaN imputation values, or missing `actual_labels_`), and `numerical_imputation_processor` makes tied-mode selection deterministic.
- **Aliasing/mutation and cardinality fixes in text/temporal processors.** `bpe_tokenizer` copies via `list(texts)` before `random.shuffle` so the caller's list isn't mutated; `gensim_tokenize_processor` builds independent zero pad-vectors instead of shared references; `bert_tokenize_processor` keeps empty/whitespace chunks as `""` for 1:1 alignment; `dialogue_processor` recomputes `token_count` from the re-encoded truncated message; `time_delta_processor`, `sequence_padding_processor`, and `sequence_ordering_processor` add explicit `ValueError`/`KeyError` guards (unknown `reference_strategy`, non-positive `target_length`, non-2D input) in place of opaque downstream errors; `IdentityProcessor` and `CSAdapter` add missing `super().__init__()` calls.
- **`ValidationResult.__dict__()` renamed to `to_dict()`** in `label_ruleset_generation` (the dunder shadowed the built-in attribute); the four call sites and the corresponding test were updated, with identical output.

### Removed

- **Legacy env vars dropped from contracts.** `USE_SECURE_PYPI` (`pytorch_training`, `tokenizer_training`, `model_calibration`), `USE_PRECOMPUTED_FEATURES` (`pytorch_training`), and `AWS_STS_REGIONAL_ENDPOINTS`/`AWS_DEFAULT_REGION` (`edx_uploading`) are no longer set by their steps; pipelines that depended on these being injected must supply them another way.
- **Dead code.** Unused helpers `find_input_shards`/`aggregate_shard_results` (`risk_table_mapping.py`), unused `random`/`unicodedata` imports (`tokenizer_training.py`), and unused `numpy` imports (`model_wiki_generator.py`, `missing_value_imputation.py`).

## [2.0.0] - 2026-06-29

The **classless-factory** release — the second half of the specification-unification arc begun in 1.8.0. Where 1.8.0 collapsed the per-step *data* (the `*_contract.py` + `*_spec.py` pairs) into one declarative `.step.yaml`, 2.0.0 collapses the per-step *code*: **all 45 hand-written `builder_*.py` step-builder classes are deleted** and synthesized at runtime from the `.step.yaml` interface onto a single shared `TemplateStepBuilder` facade, routed by five `PatternHandler` construction strategies. The registry stops being a maintained artifact and is *derived* from the interface files; the alignment-validation framework is re-grounded on the three boundaries construction can't self-check; and two new agent/CLI surfaces (`steps`, `strategies`) expose the now-data-driven build so it stays introspectable. Net across `src/cursus` since 1.9.0: **185 files changed, +5,914 / −22,340 (≈ −16,400 LOC), 53 modules deleted, 10 added.** This is a **major** version bump: the per-step `*StepBuilder` source files no longer exist (they remain importable by name — see below — but are no longer files you can open, subclass in place, or monkeypatch), `cursus.steps.builders` is now a lazy import surface, and the `ValidationLevel` enum drops a member. The public pipeline-authoring API (`PipelineDAGCompiler` / `compile_dag_to_pipeline` / `compile_with_report`) is **unchanged** — the same DAG + the same config compiles to the same pipeline, which is the whole point of the rewrite (validated end-to-end on real SageMaker/SAIS infrastructure).

### The invariance guarantee

The rewrite is designed to be **invisible at the public API**. The DAG compiler, the assembler, the dependency resolver, and `pipeline.definition()` behave identically to 1.9.0 for any given DAG + config, because:

- The assembler instantiates a builder by calling `builder_cls(**five_kwargs)` with **no `isinstance` gate** — a runtime-synthesized class is indistinguishable from a hand-written one at that call site.
- The step↔step wiring graph keys entirely on `.step.yaml` **spec data** (`spec.step_type`, `compatible_sources`, `property_path`, `logical_name`) carried on `builder.spec`, plus the DAG node name — never on a Python class — so collapsing 45 classes into one facade leaves every edge intact.

A full compile of a real 11-node pipeline through a SAIS session was confirmed to produce the same SageMaker pipeline definition with only the per-step builder *source* gone.

### Added

- **`core/base/builder_templates.py` (new) — the `TemplateStepBuilder` facade + five `PatternHandler` strategies.** A step builder is now one shared facade class whose abstract methods (`create_step` / `_get_inputs` / `_get_outputs` / `_get_environment_variables`) delegate to a construction-verb handler bound at `__init__`. The five handlers are `ProcessingHandler` (covers both the `code=` "2A" and `processor.run()`→`step_args` "2B" assembly modes), `TrainingHandler`, `ModelCreationHandler`, `TransformHandler`, and `SDKDelegationHandler` (for the MODS/SAIS-predefined steps). `resolve_handler(sagemaker_step_type, step_assembly)` is the dispatch table; per-step behavior that isn't shared is expressed as declarative **knobs** read from the `.step.yaml` `patterns:` section, not as overridden Python.
- **Runtime builder synthesis (`step_catalog/builder_discovery.py::_synthesize_builder`).** For any registry step that has a `.step.yaml` interface, routes via `resolve_handler`, and has no physical builder file, the catalog fabricates `type(f"{Name}StepBuilder", (TemplateStepBuilder,), {"STEP_NAME": Name})` on demand and caches it per process. This is the deletion mechanism for the 45 shells: a step needs no `builder_*.py` at all.
- **PEP-562 lazy import surface for `steps/builders/__init__.py`.** `from cursus.steps.builders import XGBoostTrainingStepBuilder` still works — resolved lazily by a module-level `__getattr__` that routes through the same synthesizer the runtime uses, so the name resolves to a synthesized `TemplateStepBuilder` subclass with the correct `STEP_NAME`. `__all__` is data-derived (registry ∩ actually-loadable), so star-imports and introspection keep working and never trip over an offline-only SDK builder.
- **`registry/strategy_registry.py` (new) — the construction-strategy routing table.** `resolve_handler` / `axis_name_for_step_type` map a step's `sagemaker_step_type` (+ the `Processing` sub-discriminator `step_assembly`) onto a handler verb, backed by a self-registering `STRATEGY_REGISTRY`. This is the single source both the build path and the introspection tools read, so the two cannot drift.
- **`registry/interface_registry_loader.py` (new) — registry-by-construction.** `STEP_NAMES` and its derived maps (`CONFIG_STEP_REGISTRY`, `BUILDER_STEP_NAMES`, `SPEC_STEP_TYPES`) are now built by walking the `.step.yaml` `registry:` blocks (`build_registry_from_interfaces()`) instead of being read from a standalone table. The interface file is the single source of truth for step metadata as well as I/O.
- **`step_catalog/sdk_bindings.py` (new) — lazy SAIS-SDK step-class thunks.** The four SDK-delegation steps (Cradle / Redshift / Registration / DataUploading) need a live SAIS `*Step` class injected as a knob (not serializable to YAML). These are resolved through lazy thunks so the package imports and discovers cleanly **offline** (without the SAIS SDK installed); the SDK class is materialized only when actually building one of those steps.
- **`validation/alignment/validators/registry_binding_validator.py` (new) — the B3 boundary.** `RegistryBindingValidator` replaces the five deleted per-step-type validators with one check that proves a step is *constructible*: the handler resolves, the builder loads/synthesizes, and the config covers the fields the handler reads. If B3 passes, the step can be built at runtime.
- **`cursus steps` CLI + `steps.*` MCP tools (new).** `cursus steps io <Step>` and `cursus steps patterns <Step>` expose, for a step, its resolved inputs/outputs/dependencies and which construction pattern + knobs it binds — read from the same `io_view` the build uses (`steps/interfaces/io_view.py`, new), so the view provably cannot drift from behavior.
- **`cursus strategies` CLI + `strategies.*` MCP tools (new).** `cursus strategies axes | list | show | for --sagemaker-step-type <T> | knobs` make the strategy library self-describing and queryable — the replacement for "open the builder class to see how a step is built" now that there is no per-step class to open. Backed by `STRATEGY_REGISTRY`.
- The MCP tool surface grows from 8 to **10 namespaces** (adds `steps`, `strategies`).
- **`STEP_NAME` is now a first-class attribute on `StepBuilderBase`** (`core/base/builder_base.py`); `_get_step_name()` prefers it over stripping the class name, since a synthesized/shared shell's class name no longer carries the canonical step name.

### Changed

- **Step builders are synthesized, not written.** Authoring a new step is now: one `.step.yaml` (with a `registry:` block) + one config class — no builder file. The difference between, say, a Processing step and a Training step is one string (`sagemaker_step_type`) in the interface. `O(1)` step addition; zero builder boilerplate.
- **Alignment validation re-grounded on three boundaries (FZ 31e1d3h).** The old four-level model assumed three separately-maintained artifacts; with one self-aligning `.step.yaml` and source-less builders, that premise is gone. Validation now proves the single `StepInterface` object against the three things construction *can't* self-check: **B1 script↔interface fidelity** (AST, the former Level 1), **B2 cross-step DAG-resolvability** (the former Level 3, now also owning the SageMaker property-path check folded in from the old Level 2), and **B3 registry↔handler↔config binding** (`RegistryBindingValidator`). The ~14 `inspect.getsource(builder_class)` scans in the old validators — which now return identical shell text for every step and carry zero signal — are deleted.
- **`StepCatalog` gains import-free routability + a duck-typed SDK output accessor.** `has_builder_provider()` answers "is this step buildable?" without importing a class; `get_sdk_output_location(built_step, output_type=None)` reads a built MODS step's own output locations generically (this let `CradleDataLoading` drop its bespoke helpers and become a pure shell like the rest).
- **`cursus.steps.builders` is now a lazy (PEP-562) module** rather than ~45 eager `from .builder_x import X` statements. Importing the package no longer imports every builder (and no longer fails when an optional SDK builder's dependency is missing).

### Removed

- **All 45 per-step `builder_*.py` files** under `steps/builders/` (xgboost / pytorch / lightgbm(+mt) training·model·eval·inference, tabular_preprocessing, cradle / redshift / edx / registration / data-uploading SDK-delegation, bedrock ×3, model_calibration / percentile / metrics / wiki, risk_table_mapping, stratified_sampling, package, payload, temporal ×3, currency_conversion, missing_value_imputation, feature_selection, pseudo_label_merge, dummy_*, tokenizer_training, …). They are synthesized at runtime; their names remain importable via the lazy surface.
- **Seven validation modules** made redundant by the boundary redesign: `validation/alignment/core/contract_spec_alignment.py` (the old Level-2 Contract↔Spec tier — now a construction-time Pydantic invariant, i.e. a tautology to re-check), the five per-step-type validators (`processing_step_validator.py`, `training_step_validator.py`, `transform_step_validator.py`, `createmodel_step_validator.py`, `step_type_specific_validator.py`), and `validator_factory.py` that dispatched to them. Replaced by the single `RegistryBindingValidator`.
- **`registry/step_names.yaml`** — the standalone step-name table is gone; `STEP_NAMES` is now derived from the `.step.yaml` `registry:` blocks (see `interface_registry_loader.py`).
- **`ValidationLevel.CONTRACT_SPEC` (value 2)** removed from the enum. The surviving members keep their names and non-contiguous integer values (`SCRIPT_CONTRACT=1`, `SPEC_DEPENDENCY=3`, `BUILDER_CONFIG=4`) so `ValidationLevel(1/3/4)` coercion and existing references keep working — only the value-2 member is gone.

### Fixed

- **`ProcessingHandler` 2B passed both `step_args` and `processor` to `ProcessingStep`.** In the `processor.run()`→`step_args` assembly mode, the handler's shared `common` kwargs dict carried `processor=processor`, so the constructed `ProcessingStep(step_args=…, **common)` supplied both sides of SageMaker's mutual-exclusion check and raised `ValueError: either step_args or processor need to be given, but not both.` at build time. The `processor` kwarg now lives only on the `code=` (2A) branch; the `step_args` (2B) branch passes `step_args` alone. This surfaced only at real pipeline execution because unit tests mock `ProcessingStep` (the check lives in the SDK constructor, below the seam mocks replace); a regression test now drives the **real** `ProcessingStep` (processor on a `PipelineSession` so `run()` defers offline) for both assembly modes.

### Notes

- **Migration:** for typical pipeline authoring, no changes are required — the DAG-compiler API and every `.step.yaml`/config are unchanged. The breaking surface is for code that **imported a per-step builder module by path** (`from cursus.steps.builders.builder_xgboost_training_step import XGBoostTrainingStepBuilder`) — import the name from the package instead (`from cursus.steps.builders import XGBoostTrainingStepBuilder`), which the lazy surface resolves to the synthesized class. Code that **subclassed or monkeypatched a per-step builder in place** must move that per-step behavior into the step's `.step.yaml` (`patterns:` knobs) or a handler.
- **SageMaker step-construction constraints:** every pipeline step type enforces a `step_args` **XOR** object-handle rule (`processor`/`estimator`/`transformer`/`model`/`tuner`) in its constructor; a handler must pass exactly one, and `step_args` must come from the matching deferred call (`processor.run()` / `estimator.fit()` / etc.) on a `PipelineSession`. The `ProcessingHandler` fix above is the one place this was violated.
