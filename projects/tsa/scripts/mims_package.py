...
if __name__ == "__main__":
    ...
    shutil.copy(MODEL_PATH / "isSmall_0_OrderFeature.pt", WORKING_DIRECTORY)
    ...
    shutil.copy(CONFIG_PATH / "preprocessor.pkl", WORKING_DIRECTORY)
    ...
    shutil.copy(
        DEPENDENCIES_PATH / "TemporalMultiheadAttentionDelta.py", CODE_DIRECTORY
    )
    shutil.copy(DEPENDENCIES_PATH / "params.py", CODE_DIRECTORY)
    shutil.copy(SCRIPT_PATH / "pytorch_inference_handler.py", CODE_DIRECTORY)
    shutil.copy(DEPENDENCIES_PATH / "CategoricalTransformer.py", CODE_DIRECTORY)
    shutil.copy(CONFIG_PATH / "cat_to_index.json", WORKING_DIRECTORY)
    shutil.copy(CONFIG_PATH / "default_value_dict.json", WORKING_DIRECTORY)

    make_tarfile(OUTPUT_PATH / "mims_model.tar.gz", WORKING_DIRECTORY)
