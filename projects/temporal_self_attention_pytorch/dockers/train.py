...
class TSATrainer:
    ...
    def _compute_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if isinstance(self.model, TwoSeqMoEOrderFeatureAttentionClassifier) or (
            hasattr(self.model, "module")
            and isinstance(self.model.module, TwoSeqMoEOrderFeatureAttentionClassifier)
        ):
            # Two sequence model
            ...
            scores, _ = self.model(
                batch["x_seq_cat"], batch["x_seq_num"], batch["x_engineered"], time_seq
            )
...
def load_data(
    data_path: str, use_two_seq: bool = False
) -> Tuple[TSADataset, TSADataset, TSADataset]:
    ...
    if use_two_seq:
        ...
        train_X_seq_cat_ccid = np.load(
            os.path.join(data_path, "train_X_seq_cat_ccid_v0.npy")
        )
        train_X_seq_num_ccid = np.load(
            os.path.join(data_path, "train_X_seq_num_ccid_v0.npy")
        )
        vali_X_seq_cat_ccid = np.load(
            os.path.join(data_path, "vali_X_seq_cat_ccid_v0.npy")
        )
        vali_X_seq_num_ccid = np.load(
            os.path.join(data_path, "vali_X_seq_num_ccid_v0.npy")
        )
        cali_X_seq_cat_ccid = np.load(
            os.path.join(data_path, "cali_X_seq_cat_ccid_v0.npy")
        )
        cali_X_seq_num_ccid = np.load(
            os.path.join(data_path, "cali_X_seq_num_ccid_v0.npy")
        )

        train_dataset = TSADataset(
            train_X_seq_cat,
            train_X_seq_num,
            train_X_num,
            train_Y,
        ...
        train_dataset = TSADataset(
            train_X_seq_cat, train_X_seq_num, train_X_num, train_Y
        )
    ...
    logger.info(
        f"Loaded datasets - Train: {len(train_dataset)}, Val: {len(val_dataset)}, Cali: {len(cali_dataset)}"
    )
...
def main():
    ...
    if rank == 0:  # Only save on main process
        history_path = (
            Path(config.get("checkpoint_dir", "./checkpoints"))
        ...
        logger.info(
            f"Training completed. Best validation AUC: {trainer.best_val_auc:.6f}"
        )
