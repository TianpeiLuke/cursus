...
class TwoSeqMoEOrderFeatureAttentionClassifier(torch.nn.Module):
    ...
    def forward(
        ...
        ccid_threshold = 0.05
        ccid_keep_idx = (
            (gate_scores[:, 1] > ccid_threshold)
            .nonzero()
            .squeeze(-1)
            .to(x_seq_cat_cid.device)
        )
    ...
    def get_sequence_embeddings(
        ...
        embeddings["feature"] = self.feature_attention(
    ...
    def get_model_size(self) -> dict:
        ...
        return {
            ...
            "cid_attention_parameters": sum(
                p.numel() for p in self.order_attention_cid.parameters()
            ),
            "ccid_attention_parameters": sum(
                p.numel() for p in self.order_attention_ccid.parameters()
            ),
            "feature_attention_parameters": sum(
                p.numel() for p in self.feature_attention.parameters()
            ),
            "gate_parameters": sum(p.numel() for p in self.gate_emb.parameters())
            + sum(p.numel() for p in self.gate_score.parameters()),
