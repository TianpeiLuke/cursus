...
class OrderAttentionModule(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.embedding = nn.Embedding(
            self.n_embedding + 2, self.embedding_table_dim, padding_idx=0
        )
    ...
    def forward(
        ...
        x_cat_agg = self.feature_aggregation_cat(x_cat_all.permute(0, 1, 3, 2)).squeeze(
            -1
        )  # [B, L, embed_dim]
        ...
        x_num_all = self.embedding(num_indices) * x_num.unsqueeze(
            -1
        )  # [B, L, n_num_features, embed_dim]
        x_num_agg = self.feature_aggregation_num(x_num_all.permute(0, 1, 3, 2)).squeeze(
            -1
        )  # [B, L, embed_dim]
        ...
        if self.use_time_seq and time_seq is not None:
            time_seq = torch.cat(
                [time_seq, torch.zeros(B, 1, 1, device=time_seq.device)], dim=1
            ).permute(1, 0, 2)  # [L+1, B, 1]
...
class FeatureAttentionModule(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.embedding_engineered = nn.Embedding(
            self.n_engineered_num_features + 1, self.embedding_table_dim, padding_idx=0
        )
    ...
    def forward(
        ...
        if self.n_engineered_num_features > 0 and x_engineered is not None:
            engineered_indices = torch.arange(
                1, self.n_engineered_num_features + 1, device=x_cat.device
            )

            x_engineered_emb = self.embedding_engineered(
                engineered_indices
            ) * x_engineered.unsqueeze(-1)

            if self.engineered_emb_tbl_bias is not None:
                x_engineered_emb = (
                    x_engineered_emb + self.engineered_emb_tbl_bias[None, :, :]
                )
