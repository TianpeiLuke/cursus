...
def generate_tsa_score_files(
    ...
    score_data = {
        "score": preds_np,
        "percentile": [
            np.searchsorted(percentiles, score, side="right") - 1 for score in preds_np
        ],
    }
    ...
    plt.hist(
        preds_np[targets_np == 0], bins=50, alpha=0.7, label="Non-Fraud", density=True
    )
...
def plot_tsa_performance_curves(
    ...
    plt.subplot(1, 3, 2)
    plt.plot(
        recall_test, precision_test, color="red", lw=2, label=f"Test AP = {ap_test:.3f}"
    )
    plt.plot(
        recall_val, precision_val, color="blue", lw=2, label=f"Val AP = {ap_val:.3f}"
    )
    plt.xlabel("Recall")
...
def analyze_gate_scores(gate_scores: Tensor, output_path: str, stage: str = "test"):
    """
