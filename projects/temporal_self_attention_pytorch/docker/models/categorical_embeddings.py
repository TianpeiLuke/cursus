...
class MultiCategoricalEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if output_strategy == "concat":
            self.output_dim = sum(
                config["embedding_dim"] for config in feature_configs.values()
            )
            ...
            if len(set(dims)) > 1:
                raise ValueError(
                    f"All embedding dimensions must be equal for {output_strategy} strategy"
                )
...
def create_pytorch_categorical_transformer(
    vocab_sizes: Dict[str, int],
...
def create_multi_categorical_embedding(
    feature_configs: Dict[str, Dict[str, Any]],
    output_strategy: str = "concat",
    dropout: float = 0.0,
