...
def main(
    ...
    try:
        ...
        if job_type == "training":
            ...
            # Non-training job types: check for {job_type}/ subdirectory first (Cursus convention),
            # then fall back to flat glob at root
            log(
                f"Non-training job type ({job_type}) detected - processing single dataset"
            )

            # First: check for {job_type}/{job_type}_processed_data.{ext} (standard convention)
            job_type_dir = input_path / job_type
            input_files = []
            if job_type_dir.exists() and job_type_dir.is_dir():
                input_files = list(job_type_dir.glob("*.csv")) + list(
                    job_type_dir.glob("*.parquet")
                )
                if input_files:
                    log(f"Found {len(input_files)} file(s) in {job_type}/ subdirectory")

            # Fallback: flat glob at input root
            if not input_files:
                input_files = list(input_path.glob("*.csv")) + list(
                    input_path.glob("*.parquet")
                )
            ...
            for input_file in input_files:
                ...
                job_type_output_dir.mkdir(parents=True, exist_ok=True)
                output_base = job_type_output_dir / f"{job_type}_processed_data"
