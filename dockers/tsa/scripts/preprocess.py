...
if __name__ == "__main__":
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    np.save(
        file=os.path.join(out_dir, "{}_X_seq_cat_v{}.npy".format(dataset, p)), arr=train_X_seq_cat
    )
    np.save(
        file=os.path.join(out_dir, "{}_X_seq_num_v{}.npy".format(dataset, p)), arr=train_X_seq_num
    )
    np.save(file=os.path.join(out_dir, "{}_X_num_v{}.npy".format(dataset, p)), arr=train_X_num)
    np.save(file=os.path.join(out_dir, "{}_Y_v{}.npy".format(dataset, p)), arr=train_Y)

    del train_binned_downsampled_data, train_X_seq_cat, train_X_seq_num, train_X_num, train_Y
    #####################
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    np.save(
        file=os.path.join(out_dir, "{}_X_seq_cat_v{}.npy".format(dataset, p)), arr=cali_X_seq_cat
    )
    np.save(
        file=os.path.join(out_dir, "{}_X_seq_num_v{}.npy".format(dataset, p)), arr=cali_X_seq_num
    )
    np.save(file=os.path.join(out_dir, "{}_X_num_v{}.npy".format(dataset, p)), arr=cali_X_num)
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    np.save(
        file=os.path.join(out_dir, "{}_X_seq_cat_v{}.npy".format(dataset, p)), arr=vali_X_seq_cat
    )
    np.save(
        file=os.path.join(out_dir, "{}_X_seq_num_v{}.npy".format(dataset, p)), arr=vali_X_seq_num
    )
    np.save(file=os.path.join(out_dir, "{}_X_num_v{}.npy".format(dataset, p)), arr=vali_X_num)
