...
def iteration_single_seq(
    ...
    x_engineered = batch["x_engineered_num"].to(device) if use_engineered_features else None

    pred, _ = model(x_seq_cat, x_seq_num, x_engineered, time_seq, attn_mask, key_padding_mask)
...
def evaluation_single_seq(
    ...
    if args.local_rank <= 0:
        ...
        if valid_epochs_loss is not None:
            ...
            logger.info("average_validation_loss={};".format(np.average(valid_epoch_loss)))
        if args.local_rank == 0:
            dist.barrier()
...
def print_performance(perf, logger, measures=None, printif=True, writeif=False, **kargs):
    """
    ...
    if printif:
        ...
        for mea in measures:
            logger.info(mea + " " * (maxchrlen - len(mea)) + " {:.4f}".format(perf[mea]))
        logger.info("")
