...
def simple_save(A_list, A_name, dataset, out_dir="/opt/ml/processing/output", p="0"):
    ...
    i = 1
    for A_i in A_list:
        print("remove", os.path.join(data_dir, "{}_{}_v0_algo-{}.npy".format(dataset, A_name, i)))
        del A_i
...
if __name__ == "__main__":
    ...
    for i in range(1, 3):
        ...
        X_seq_cat_cid_chunk = np.load(
            os.path.join(data_dir, "{}_cid_X_seq_cat_v0_algo-{}.npy".format(dataset, i)),
            mmap_mode="r",
        )
        X_seq_num_cid_chunk = np.load(
            os.path.join(data_dir, "{}_cid_X_seq_num_v0_algo-{}.npy".format(dataset, i)),
            mmap_mode="r",
        )
        X_seq_cat_ccid_chunk = np.load(
            os.path.join(data_dir, "{}_ccid_X_seq_cat_v0_algo-{}.npy".format(dataset, i)),
            mmap_mode="r",
        )
        X_seq_num_ccid_chunk = np.load(
            os.path.join(data_dir, "{}_ccid_X_seq_num_v0_algo-{}.npy".format(dataset, i)),
            mmap_mode="r",
        )
        X_num_chunk = np.load(
            os.path.join(data_dir, "{}_X_num_v0_algo-{}.npy".format(dataset, i)), mmap_mode="r"
        )
        Y_chunk = np.load(
            os.path.join(data_dir, "{}_Y_v0_algo-{}.npy".format(dataset, i)), mmap_mode="r"
        )
