...
def load_data_single_seq(args, batch_size, data_version):
    ...
    if args.use_small_batch:
        ...
        x_engineered_train = np.load(
            os.path.join(f"{args.train_data_folder}_X_num_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        x_seq_num_train = np.load(
            os.path.join(f"{args.train_data_folder}_X_seq_num_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        x_seq_cat_train = np.load(
            os.path.join(f"{args.train_data_folder}_X_seq_cat_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]

        x_engineered_vali = np.load(
            os.path.join(f"{args.vali_data_folder}_X_num_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        x_seq_num_vali = np.load(
            os.path.join(f"{args.vali_data_folder}_X_seq_num_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        x_seq_cat_vali = np.load(
            os.path.join(f"{args.vali_data_folder}_X_seq_cat_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]

        x_engineered_cali = np.load(
            os.path.join(f"{args.cali_data_folder}_X_num_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        x_seq_num_cali = np.load(
            os.path.join(f"{args.cali_data_folder}_X_seq_num_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        x_seq_cat_cali = np.load(
            os.path.join(f"{args.cali_data_folder}_X_seq_cat_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]

        train_Y = np.load(
            os.path.join(f"{args.train_data_folder}_Y_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        vali_Y = np.load(
            os.path.join(f"{args.vali_data_folder}_Y_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
        cali_Y = np.load(
            os.path.join(f"{args.cali_data_folder}_Y_{data_version}.npy"), mmap_mode="r+"
        )[: 2 * N]
    else:
    ...
    if args.local_rank >= 0:
        ...
        vali_dataloader = DataLoader(dataset=vali_dataset, batch_size=batch_size, shuffle=False)
        cali_dataloader = DataLoader(dataset=cali_dataset, batch_size=batch_size, shuffle=False)
