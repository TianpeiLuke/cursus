...
def processing_training_data_by_chunk(training_files, tag):
    ...
    #         train_data
    #     )
    train_X_seq_cat, train_X_seq_num, train_X_num, train_Y = parallel_data_parsing(train_data)
...
def processing_calibration_data_by_chunk(calib_files, tag):
    ...
    cali_X_seq_cat, cali_X_seq_num, cali_X_num, cali_Y = parallel_data_parsing(calib_data)
...
def processing_testing_data_by_chunk(test_files, tag):
    ...
    vali_X_seq_cat, vali_X_seq_num, vali_X_num, vali_Y = parallel_data_parsing(test_data)
...
def chunk_processing(data_path, dataset, num_chunk):
    ...
    for i_chunk in range(num_chunk):
        ...
        print(X_seq_cat_chunk.shape, X_seq_num_chunk.shape, X_num_chunk.shape, Y_chunk.shape)
        ...
        np.save(
            file=os.path.join(out_dir, "{}_X_seq_cat_chunk_{}.npy".format(dataset, i_chunk)),
            arr=X_seq_cat_chunk,
        )
        np.save(
            file=os.path.join(out_dir, "{}_X_seq_num_chunk_{}.npy".format(dataset, i_chunk)),
            arr=X_seq_num_chunk,
        )
        np.save(
