...
def df_category_risk(
    ...
    for col in col_names:
        ...
        df_ratio = df_col.groupby(level=0, group_keys=False).apply(
            lambda x: (
                (x + smoothing_factor * global_pos_ratio)
                / (float(x.sum()) + smoothing_factor)
                if x.shape[0] == 2
                else (
                    (x + smoothing_factor * global_pos_ratio)
                    / (float(x[1:].sum()) + smoothing_factor)
                    if x.shape[0] > 2
