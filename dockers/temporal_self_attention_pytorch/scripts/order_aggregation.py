...
class OrderAggregator:
    ...
    def _load_preprocessing_configs(self):
        ...
        for key in list(self.default_value_dict):
            self.default_value_dict[key.replace(".", "__DOT__")] = self.default_value_dict[key]
    
    def aggregate_order_sequence(self, 
                                input_data: Dict[str, Any],
                                seq_cat_otf_vars: List[str],
                                seq_cat_vars: List[str],
                                seq_num_otf_vars: List[str],
                                seq_num_vars: List[str]) -> Tuple[bool, Optional[np.ndarray], Optional[np.ndarray]]:
        """
        ...
        try:
            ...
            if not self._validate_input_data(input_data, seq_cat_otf_vars, seq_cat_vars, 
                                           seq_num_otf_vars, seq_num_vars):
                return False, None, None
            
            # Check if historical data is available and valid
            no_history_flag = self._check_history_availability(input_data, seq_cat_otf_vars)
    ...
    def _validate_input_data(self, input_data: Dict[str, Any], 
                           ...
                           seq_cat_vars: List[str],
                           seq_num_otf_vars: List[str], 
                           seq_num_vars: List[str]) -> bool:
        """Validate that input data contains all required fields."""
        if not isinstance(input_data, dict):
            return False
        
        # Check for required sequence variables
        required_vars = seq_cat_otf_vars + seq_cat_vars + seq_num_otf_vars + seq_num_vars
    ...
    def _check_history_availability(self, input_data: Dict[str, Any], 
                                  seq_cat_otf_vars: List[str]) -> bool:
        """Check if historical sequence data is available and valid."""
        # Check if primary sequence field is empty
        primary_field = "payment_risk.bfs_order_cat_seq_by_cid.c_billingaddrlatlongconfidence_seq"
    ...
    def _process_categorical_sequences(self, input_data: Dict[str, Any],
                                     seq_cat_otf_vars: List[str],
                                     seq_cat_vars: List[str],
                                     no_history_flag: bool) -> np.ndarray:
    ...
    def _process_numerical_sequences(self, input_data: Dict[str, Any],
                                   seq_num_otf_vars: List[str],
    ...
    def _extract_current_order_data(self, input_data: Dict[str, Any], 
                                  var_list: List[str], 
                                  categorical: bool = True) -> np.ndarray:
    ...
    def _extract_sequence_data(self, input_data: Dict[str, Any],
                             otf_vars: List[str], 
                             vars: List[str],
                             categorical: bool = True) -> np.ndarray:
    ...
    def _apply_numerical_scaling(self, seq_num_mtx: np.ndarray) -> np.ndarray:
        ...
        seq_num_mtx[:, :-2] = (seq_num_mtx[:, :-2] * np.array(self.seq_num_scale_) + 
    ...
    def _compute_time_deltas(self, seq_num_mtx: np.ndarray) -> np.ndarray:
        ...
        if seq_num_mtx.shape[0] > 0:
            ...
            seq_num_mtx[:, time_col_idx] = np.clip(seq_num_mtx[:, time_col_idx], 
                                                  0, max_time_diff)
        
        return seq_num_mtx
    
    def _apply_sequence_padding(self, sequence_mtx: np.ndarray, 
                              no_history_flag: bool) -> np.ndarray:
        """Apply padding to reach target sequence length."""
        ...
        if pad_width > 0:
            sequence_mtx = np.pad(sequence_mtx, padding, mode='constant', constant_values=0)
    ...
    def process_order_batch(self, 
                           ...
                           seq_num_otf_vars: List[str],
                           seq_num_vars: List[str],
                           num_workers: int = 80) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for row in df.itertuples(index=False):
            ...
            results.append(pool.apply_async(
                ...
                args=(input_data, seq_cat_otf_vars, seq_cat_vars, 
                     seq_num_otf_vars, seq_num_vars)
            ))
        ...
        if X_seq_cat_list:
            ...
            X_seq_num = np.empty((0, self.seq_len, len(seq_num_vars) - 1 + 1))  # +1 for keep flag
        
        return X_seq_cat, X_seq_num


def create_order_aggregator(seq_len: int = DEFAULT_SEQ_LEN,
                           config_path: str = "/opt/ml/processing/input/config") -> OrderAggregator:
    """
...
def mtx_from_dict_fill_default(input_data: Dict[str, Any], 
                              var_list_otf: List[str], 
                              var_list: List[str], 
                              map_dict: Dict[str, Any]) -> np.ndarray:
...
def arr_from_dict_fill_default(input_data: Dict[str, Any], 
                              var_list: List[str], 
                              map_dict: Dict[str, Any]) -> np.ndarray:
