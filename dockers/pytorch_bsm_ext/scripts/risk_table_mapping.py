    ...
    def fit(self, df: pd.DataFrame, smooth_factor: float = 0, count_threshold: int = 0):
        ...
        for var in self.variables:
            ...
            self.risk_tables[var] = {
                "varName": var,
                "type": "categorical",
                "mode": self.metadata.loc[self.metadata.varname == var, "datatype"].iat[
                    0
                ],
...
class MissingValueImputation:
    ...
    def fit(self, df: pd.DataFrame):
        ...
        for var in self.numeric_variables:
            if var in df.columns:
                impute_strategy = self.metadata.loc[
                    self.metadata["varname"] == var, "impute_strategy"
                ].iat[0]
                if impute_strategy and impute_strategy != "none":
                    imputer = SimpleImputer(
                        missing_values=np.nan, strategy=impute_strategy
                    )
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for var, imputer in self.imputers.items():
            if var in df_transformed.columns:
                df_transformed[var] = imputer.transform(
                    df_transformed[var].values.reshape(-1, 1)
                )
        return df_transformed
...
def main(
    ...
    if job_type == "training":
        ...
        train_df, holdout_df = train_test_split(
            ...
            random_state=42,
            stratify=df_transformed[tag],
        )
        test_df, val_df = train_test_split(
            holdout_df,
            test_size=test_val_ratio,
            random_state=42,
            stratify=holdout_df[tag],
        )
        ...
        for split_name, split_df in [
            ...
            split_df.to_csv(
                split_output_dir / f"{split_name}_processed_data.csv", index=False
            )
    ...
    # --- Logic for Inference/Validation modes ---
        ...
        logger.info(
            f"Running in '{job_type}' mode: fitting on train, transforming all splits."
        )
        # Load all splits: fit on train, transform all
        train_df = pd.read_csv(Path(input_dir) / "train" / "train_processed_data.csv")
        val_df = pd.read_csv(
            Path(input_dir) / "validation" / "validation_processed_data.csv"
        )
