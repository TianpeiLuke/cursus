...
from typing import (
    ...
    Dict,
    Optional,
)  # If you use Dict or Optional elsewhere for type hinting
...
WORKING_DIRECTORY = Path(
    "/tmp/mims_packaging_directory"
)  # Using /tmp for broader compatibility
CODE_DIRECTORY = WORKING_DIRECTORY / "code"
MODEL_FILE = "model.pth"  # Expected PyTorch model file
MODEL_ARTIFACT_FILE = (
    "model_artifacts.pth"  # Additional artifacts (e.g., tokenizer, config)
)
...
def check_file_exists(path: Path, description: str) -> bool:
    ...
    if exists:
        try:
            ...
            FileNotFoundError
        ):  # Race condition if file is deleted between exists() and stat()
            exists = False
...
def copy_file_robust(src: Path, dst: Path):
    ...
    if not check_file_exists(src, "Source file for copy"):
        logger.warning(
            f"Source file {src} does not exist or is not a file. Skipping copy."
        )
        return False
    try:
...
def copy_scripts(src_dir: Path, dst_dir: Path):
    ...
    if not src_dir.exists() or not src_dir.is_dir():
        logger.warning(
            f"Source scripts directory '{src_dir}' does not exist. Skipping script copy."
        )
        return
    ...
    for item in src_dir.rglob("*"):  # rglob handles recursive
        if item.is_file():
            ...
            )  # Ensure sub-directory structure is created
    ...
    if copied_any:
        ...
        logger.info(
            f"No files found to copy from '{src_dir}' or copy failed for all files."
        )
...
def extract_tarfile(tar_path: Path, extract_path: Path):
    ...
    with tarfile.open(
        tar_path, "r:*"
    ) as tar:  # "r:*" auto-detects compression (gz, bz2, etc.)
...
def create_tarfile(
    output_tar_path: Path, base_dir_to_tar_from: Path, items_to_include: List[Path]
):
    """
    ...
    with tarfile.open(output_tar_path, "w:gz") as tar:
        for item_path_in_working_dir in items_to_include:
            if not item_path_in_working_dir.exists():
                logger.warning(
                    f"Item '{item_path_in_working_dir}' does not exist. Skipping from tar."
                )
                continue
            ...
            try:
                arcname = item_path_in_working_dir.relative_to(
                    base_dir_to_tar_from
                ).as_posix()
            except (
                ValueError
            ):  # If item_path_in_working_dir is not relative to base_dir_to_tar_from (should not happen with current logic)
                arcname = (
                    item_path_in_working_dir.name
                )  # Fallback to just the item name
                logger.warning(
                    f"Item '{item_path_in_working_dir}' not relative to base '{base_dir_to_tar_from}'. Using arcname: '{arcname}'"
                )
...
def main():
    ...
    list_directory_contents(
        MODEL_PATH, "Model input directory (/opt/ml/processing/input/model)"
    )
    list_directory_contents(
        SCRIPT_PATH, "Script input directory (/opt/ml/processing/input/script)"
    )
    ...
    if check_file_exists(
        input_model_tar, "Input model.tar.gz from /opt/ml/processing/input/model"
    ):
        extract_tarfile(
            input_model_tar, MODEL_PATH
        )  # Extract contents into MODEL_PATH itself
        # This means files like model.pth, model.onnx (if in tar)
        # will now be at MODEL_PATH/model.pth, MODEL_PATH/model.onnx
    else:
        logger.warning(
            f"Input model.tar.gz not found at '{input_model_tar}'. Any required model files must be directly in '{MODEL_PATH}'."
        )
    ...
    def try_copy_to_working_dir(
        filename: str, source_dir: Path, work_dir: Path, tar_list: List[Path]
    ):
        source_file = source_dir / filename
        if check_file_exists(
            source_file, f"Checking for '{filename}' in '{source_dir}'"
        ):
            ...
            logger.info(
                f"'{filename}' not found in '{source_dir}', will not be included."
            )

    # Conditionally copy model.pth, model_artifacts.pth, and model.onnx
    try_copy_to_working_dir(
        MODEL_FILE, MODEL_PATH, WORKING_DIRECTORY, items_for_output_tar
    )
    try_copy_to_working_dir(
        MODEL_ARTIFACT_FILE, MODEL_PATH, WORKING_DIRECTORY, items_for_output_tar
    )
    try_copy_to_working_dir(
        ONNX_MODEL_FILE, MODEL_PATH, WORKING_DIRECTORY, items_for_output_tar
    )  # Check for ONNX file
    ...
    if CODE_DIRECTORY.exists() and any(CODE_DIRECTORY.iterdir()):
        ...
        logger.info(
            f"Directory '{CODE_DIRECTORY.name}' will be included in the output tar."
        )
    else:
        logger.warning(
            f"Code directory '{CODE_DIRECTORY}' is empty or does not exist, not adding to tar."
        )
    ...
    if items_for_output_tar:
        ...
        logger.warning(
            "No items were prepared for packaging. Output model.tar.gz will not be created."
        )

    # --- Step 5: Final verification ---
    logger.info("\n--- Final state ---")
    list_directory_contents(
        WORKING_DIRECTORY, "Working directory content before cleanup (optional)"
    )
    ...
    if output_tar_file.exists():
        ...
        logger.warning(
            f"Final output tar file '{output_tar_file}' was not created (likely no items to tar)."
        )
