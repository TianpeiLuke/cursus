...
class OrderFeatureAttentionClassifier(torch.nn.Module):
    ...
    def forward(
        ...
        if self.use_time_seq:
            x_order = self.order_attention(x_cat, x_num, time_seq, attn_mask, key_padding_mask)
        else:
            x_order = self.order_attention(x_cat, x_num, None, attn_mask, key_padding_mask)
    ...
    def get_embeddings(
        ...
        if self.use_time_seq:
            embeddings['order'] = self.order_attention(x_cat, x_num, time_seq, attn_mask, key_padding_mask)
        else:
            embeddings['order'] = self.order_attention(x_cat, x_num, None, attn_mask, key_padding_mask)
    ...
    def get_model_size(self) -> dict:
        ...
        return {
            ...
            'embedding_parameters': self.embedding.weight.numel() + self.embedding_engineered.weight.numel(),
            'order_attention_parameters': sum(p.numel() for p in self.order_attention.parameters()),
