...
class FeatureAggregator:
    ...
    def _load_feature_configs(self):
        ...
        for key in list(self.categorical_map):
            self.categorical_map[key.replace(".", "__DOT__")] = self.categorical_map[key]
    ...
    def aggregate_categorical_features(self, 
                                     categorical_data: np.ndarray,
                                     feature_names: List[str],
                                     numerical_cat_vars_indices: Optional[List[int]] = None) -> np.ndarray:
        """
    ...
    def aggregate_numerical_features(self, 
                                   numerical_data: np.ndarray,
                                   feature_type: str = "sequence") -> np.ndarray:
        """
        ...
        if feature_type == "sequence":
            ...
            scaled_data = (numerical_data * np.array(self.seq_num_scale_) + 
                          np.array(self.seq_num_min_))
        elif feature_type == "engineered":
            # Apply engineered feature scaling
            scaled_data = (numerical_data * np.array(self.num_static_scale_) + 
                          np.array(self.num_static_min_))
        else:
            # Default: no scaling
    ...
    def aggregate_engineered_features(self, 
                                    input_data: Dict[str, Any],
                                    dense_num_vars: List[str]) -> np.ndarray:
        """
        ...
        scaled_features = self.aggregate_numerical_features(
            dense_array.reshape(1, -1), 
            feature_type="engineered"
        )
        
        return scaled_features.flatten()
    
    def compute_feature_interactions(self, feature_embeddings: torch.Tensor) -> torch.Tensor:
        """
    ...
    def create_feature_aggregation_mlp(self, 
                                     input_dim: int,
    ...
    def process_feature_batch(self, 
                            df: pd.DataFrame,
                            categorical_vars: List[str],
                            numerical_vars: List[str],
                            engineered_vars: List[str],
                            numerical_cat_vars_indices: Optional[List[int]] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        for _, row in df.iterrows():
            ...
            cat_data = np.array([row_data.get(var, "") for var in categorical_vars]).reshape(1, -1)
            ...
            num_data = np.array([
                float(row_data.get(var, 0.0)) if row_data.get(var, "") not in ["", "My Text String"] 
                else 0.0 for var in numerical_vars
            ]).reshape(1, -1)
            num_scaled = self.aggregate_numerical_features(num_data, feature_type="sequence")
...
def create_feature_aggregator(config_path: str = "/opt/ml/processing/input/config") -> FeatureAggregator:
    """
