...
class LangGraphBedrockOrchestrator:
    ...
    def _track_usage(self, state: WorkflowState, usage: Dict[str, int], latency: float) -> None:
        ...
        total_latency = (stats["average_latency"] * (stats["total_requests"] - 1) + latency)
    ...
    async def _orchestrator_node(self, state: WorkflowState) -> WorkflowState:
        ...
        logger.info(f"Orchestrator node: Step {state['current_step']}, Phase {state['current_phase']}")
        ...
        if not response.success:
            ...
            try:
                ...
                if self._validate_decision_structure(decision_data):
                    ...
                    logger.info(f"Claude decision: {decision['action_type']} -> {decision['target_agent']}")
                else:
...
"""
    ...
    async def run_workflow(self, initial_context: Dict[str, Any], thread_id: str = "default") -> Dict[str, Any]:
    ...
    async def run_single_step(self, context: Dict[str, Any], thread_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            ...
            if current_state.values:
                ...
                return {
                    ...
                    "progress_percentage": (len(state.get("completed_steps", [])) / 7) * 100,
