...
| **Container entry point** | `tsa_model_eval.py` |
| **Interface file** | `steps/interfaces/tsa_model_eval.step.yaml` |
...
1. Loads trained TSA PyTorch model and configuration from model directory
2. Loads and processes evaluation data from numpy arrays (sequences + static features)
3. Generates predictions for dual-task learning objectives
4. Computes comprehensive performance metrics with business impact analysis
5. Creates visualizations comparing task performance
6. Saves predictions and metrics to separate output directories
...
- /opt/ml/processing/input/eval_data: Evaluation data directory containing numpy arrays:
  - {prefix}_X_num_{version}.npy: Static numerical features
  - {prefix}_cid_X_seq_num_{version}.npy: Customer ID numerical sequences
  - {prefix}_cid_X_seq_cat_{version}.npy: Customer ID categorical sequences
  - {prefix}_Y_{version}.npy: Multi-column labels and amounts
...
- ENABLE_CPU_OPTIMIZATION: Enable CPU-specific optimizations (default: "true")
  ...
  * Automatically detects CPU and applies optimizations:
    · Intel MKL threading (20-30% speedup): Optimizes thread count and enables MKL-DNN
    · TorchScript JIT compilation (30-50% speedup): Compiles model to optimized machine code
    · Optimized batch sizing (10-20% speedup): Reduces batch size to 64 for better CPU cache utilization
    · CPU-specific data loading (5-10% speedup): Disables pin_memory for faster CPU data transfer
...
- EVAL_BATCH_SIZE: Override batch_size for evaluation (default: auto-detect or use hyperparameters.json)
  ...
  * Auto-detects optimal size based on instance type if not specified:
    ...
    · CPU instances: 64-128 (cache-friendly)
...
|-------|------|----------|----------------------|
| `model_input` | `model_artifacts` | yes | [TSATraining](tsa_training.md), [PyTorchTraining](pytorch_training.md), [DummyTraining](dummy_training.md), [PyTorchModel](pytorch_model.md) |
| `processed_data` | `processing_output` | yes | [TSAPreprocessing](tsa_preprocessing.md), [TabularPreprocessing](tabular_preprocessing.md), ProcessingStep |
