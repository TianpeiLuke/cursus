...
class TestExecutionDocumentGeneratorIntegration:
    ...
    def expected_result_path(self):
        ...
        return (
            ...
            / "test_data"
            / "execute_doc_lukexie-AtoZ-xgboost-NA_2.0.0.json"
        )
    ...
    def test_fill_execution_document_integration(
        ...
        result = generator.fill_execution_document(
            xgboost_dag, sample_execution_document
        )
        ...
        cradle_steps = ["CradleDataLoading-Training", "CradleDataLoading-Calibration"]
    ...
    def test_fill_execution_document_cradle_data_mapping(
        self, config_path, sample_execution_document, expected_result, xgboost_dag
    ):
        """
        ...
        if training_step in result["PIPELINE_STEP_CONFIGS"]:
            ...
            if "STEP_CONFIG" not in step_config:
                pytest.skip(
                    "Cradle dependencies not available - STEP_CONFIG not populated"
                )
            ...
            if "dataSources" in step_config:
                ...
                mds_source = next(
                    (
                        ...
                        if ds["dataSourceName"] in ["RAW_MDS_NA", "RAW_MDS"]
                    ),
                    None,
                )
                if mds_source:
                ...
                tags_source = next(
                    (ds for ds in data_sources if ds["dataSourceName"] == "TAGS"), None
                )
                if tags_source:
                    ...
                    if tags_source["dataSourceType"] == "EDX":
                        ...
                        assert (
                            "andesDataSourceProperties" in tags_source
                            or "edxDataSourceProperties" in tags_source
                        )

    def test_fill_execution_document_registration_mapping(
        self, config_path, sample_execution_document, expected_result, xgboost_dag
    ):
        """
        ...
        if registration_step in result["PIPELINE_STEP_CONFIGS"]:
            step_config = result["PIPELINE_STEP_CONFIGS"][registration_step][
                "STEP_CONFIG"
            ]
    ...
    def test_dag_node_mapping(
        self, config_path, sample_execution_document, xgboost_dag
    ):
        """
    ...
    def test_error_handling_invalid_config_path(self):
        ...
        with pytest.raises(
            Exception
        ):  # Should raise ExecutionDocumentGenerationError or similar
            ExecutionDocumentGenerator("nonexistent_config.json")

    def test_error_handling_missing_helpers(
        self, config_path, sample_execution_document, xgboost_dag
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "CradleDataLoading-Calibration",
                ["WORKFLOW_INPUT", "CradleDataLoadingStep"],
            ),
            ("XGBoostTraining", "TRAINING_STEP"),
            (
                "Registration-NA",
                ["PROCESSING_STEP", "MimsModelRegistrationProcessingStep"],
            ),
        ],
    )
    def test_step_type_assignment(
        self,
        config_path,
    ...
    def test_save_actual_output(
        self, config_path, sample_execution_document, xgboost_dag, project_root
    ):
        """
        ...
        output_path = (
            Path(__file__).parent / "test_data" / "test_output_execution_document.json"
        )
    ...
    def test_debug_load_configs(self, config_path, project_root):
        ...
        try:
            ...
            for config_name, config_obj in loaded_configs.items():
                ...
                for attr in key_attrs:
                    if hasattr(config_obj, attr):
                        ...
                        print(
                            f"  {attr}: {type(value).__name__} = {str(value)[:100]}..."
                        )
                    ...
                    if hasattr(config_obj, "data_sources_spec"):
                        ...
                        if hasattr(ds_spec, "data_sources"):
                            ...
                            if hasattr(data_sources, "value"):
                                print(
                                    f"  data_sources.value type: {type(data_sources.value).__name__}"
                                )
                                print(
                                    f"  data_sources.value length: {len(data_sources.value) if hasattr(data_sources.value, '__len__') else 'N/A'}"
                                )

                                if (
                                    hasattr(data_sources.value, "__iter__")
                                    and len(data_sources.value) > 0
                                ):
                                    first_ds = data_sources.value[0]
                                    print(
                                        f"  First data source type: {type(first_ds).__name__}"
                                    )
                                    if hasattr(first_ds, "data_source_name"):
                                        print(
                                            f"  First data source name: {first_ds.data_source_name}"
                                        )
                                    if hasattr(first_ds, "data_source_type"):
                                        print(
                                            f"  First data source type: {first_ds.data_source_type}"
                                        )
            ...
            # Create temporary file for debug output
            temp_debug_file = tempfile.NamedTemporaryFile(
