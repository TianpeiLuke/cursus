...
class TestExecutionDocumentGenerator:
    ...
    def test_init_config_loading_failure(self, mock_load_configs):
        ...
        with pytest.raises(
            ExecutionDocumentGenerationError, match="Configuration loading failed"
        ):
    ...
    def test_identify_relevant_steps(self, mock_load_configs):
        ...
        generator.config_resolver.resolve_config_for_step = Mock(
            side_effect=mock_resolve_config
        )
    ...
    def test_filter_steps_by_helper(self, mock_load_configs):
        ...
        filtered_steps = generator._filter_steps_by_helper(
            ["step1"], generator.cradle_helper
        )
        assert "step1" in filtered_steps

        # Test with registration helper (should not match cradle config)
        filtered_steps = generator._filter_steps_by_helper(
            ["step1"], generator.registration_helper
        )
        assert len(filtered_steps) == 0

    @patch("cursus.steps.configs.utils.load_configs")
    @patch("cursus.steps.configs.utils.build_complete_config_classes")
    def test_fill_execution_document_invalid_structure(
        self, mock_build_classes, mock_load_configs
    ):
    ...
    def test_fill_execution_document_success(self, mock_load_configs):
        ...
        generator.cradle_helper.can_handle_step = Mock(return_value=True)
        generator.cradle_helper.extract_step_config = Mock(
            return_value={"mock_config": "config_for_step1"}
        )
        generator.cradle_helper.get_execution_step_name = Mock(
            return_value="step1-Training"
        )
        ...
        assert (
            result["PIPELINE_STEP_CONFIGS"]["step1-Training"]["STEP_CONFIG"][
                "mock_config"
            ]
            == "config_for_step1"
        )
    ...
    @patch("cursus.steps.configs.utils.load_configs")
    @patch("cursus.steps.configs.utils.build_complete_config_classes")
    def test_get_config_for_step_resolver_success(
        self, mock_build_classes, mock_load_configs
    ):
        ...
        generator.config_resolver.resolve_config_for_step.assert_called_once_with(
            "step1", generator.configs
        )

    @patch("cursus.steps.configs.utils.load_configs")
    @patch("cursus.steps.configs.utils.build_complete_config_classes")
    def test_get_config_for_step_fallback_direct_match(
        self, mock_build_classes, mock_load_configs
    ):
    ...
    @patch("cursus.steps.configs.utils.load_configs")
    @patch("cursus.steps.configs.utils.build_complete_config_classes")
    def test_get_config_for_step_fallback_pattern_match(
        self, mock_build_classes, mock_load_configs
    ):
    ...
    def test_names_match(self, mock_build_classes, mock_load_configs):
        ...
        assert (
            generator._names_match("data_loading_step", "data_loading_config") is True
        )
        assert (
            generator._names_match("data-loading-step", "data_loading_config") is True
        )
    ...
    @patch("cursus.steps.configs.utils.load_configs")
    @patch("cursus.steps.configs.utils.build_complete_config_classes")
    def test_is_execution_doc_relevant_fallback(
        self, mock_build_classes, mock_load_configs
    ):
    ...
    def test_conditional_helper_processing(self, mock_load_configs):
        ...
        execution_doc = {"PIPELINE_STEP_CONFIGS": {"step1": {}}}
