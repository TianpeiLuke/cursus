...
class TestCradleDataLoadingHelper:
    ...
    def test_extract_step_config_success(self):
        ...
        with patch.object(
            helper, "_build_request", return_value=mock_request
        ) as mock_build:
            with patch.object(
                helper, "_get_request_dict", return_value=mock_request_dict
            ) as mock_get_dict:
                mock_config = Mock()
    ...
    def test_extract_step_config_build_request_failure(self):
        ...
        with patch.object(
            helper, "_build_request", side_effect=ValueError("Build failed")
        ):
            mock_config = Mock()

            with pytest.raises(
                ExecutionDocumentGenerationError,
                match="Cradle configuration extraction failed",
            ):
    ...
    def test_extract_step_config_get_request_dict_failure(self):
        ...
        with patch.object(helper, "_build_request", return_value=mock_request):
            with patch.object(
                helper,
                "_get_request_dict",
                side_effect=ImportError("coral_utils not available"),
            ):
                mock_config = Mock()
    ...
    def test_build_request_missing_required_attrs(self):
        ...
        with pytest.raises(
            ValueError, match="CradleDataLoadingConfig missing required attribute"
        ):
            helper._build_request(mock_config)
    ...
    def test_build_request_success_mds(self):
        ...
        with patch.object(
            helper, "_build_request", return_value=expected_request
        ) as mock_build:
    ...
    def test_get_request_dict_success(self):
        ...
        with patch.object(
            helper, "_get_request_dict", return_value=expected_dict
        ) as mock_get_dict:
    ...
    def test_get_request_dict_conversion_failure(self):
        ...
        with patch.object(
            helper,
            "_get_request_dict",
            side_effect=ValueError("Failed to convert request to dict"),
        ):
