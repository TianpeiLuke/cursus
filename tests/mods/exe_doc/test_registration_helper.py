...
class TestRegistrationHelper:
    ...
    def test_extract_step_config_success(self):
        ...
        with patch.object(
            helper, "_get_image_uri", return_value="test-image-uri"
        ) as mock_get_uri:
            with patch.object(
                helper, "_create_execution_doc_config", return_value={"test": "config"}
                ...
                mock_create_config.assert_called_once_with(
                    "test-image-uri", {"registration": mock_config}
                )
    ...
    def test_extract_step_config_failure(self):
        ...
        with patch.object(
            helper, "_get_image_uri", side_effect=ValueError("Image URI failed")
        ):
            with pytest.raises(
                ExecutionDocumentGenerationError,
                match="Registration configuration extraction failed",
            ):
    ...
    def test_get_image_uri_success(self, mock_retrieve):
        ...
        assert (
            result
            == "123456789012.dkr.ecr.us-east-1.amazonaws.com/pytorch-inference:1.8.1-gpu-py3"
        )
