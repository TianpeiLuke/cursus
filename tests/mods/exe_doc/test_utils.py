...
class TestDetermineStepType:
    ...
    def test_cradle_step_type_from_registry(
        self, mock_get_sagemaker_type, mock_get_config_registry
    ):
        ...
        mock_get_config_registry.return_value = {
            "CradleDataLoadingConfig": "CradleDataLoading"
        }
    ...
    @patch("cursus.registry.step_names.get_config_step_registry")
    @patch("cursus.registry.step_names.get_sagemaker_step_type")
    def test_registration_step_type_from_registry(
        self, mock_get_sagemaker_type, mock_get_config_registry
    ):
    ...
    @patch("cursus.registry.step_names.get_sagemaker_step_type")
    def test_step_name_resolution_fallback(
        self, mock_get_sagemaker_type, mock_get_canonical_name, mock_get_config_registry
    ):
...
class TestMergeExecutionDocuments:
    ...
    def test_merge_documents_with_overlap(self):
        ...
        additional_doc = {
            "PIPELINE_STEP_CONFIGS": {"step1": {"STEP_CONFIG": {"key2": "value2"}}}
        }
    ...
    def test_merge_documents_invalid_base(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid base execution document structure"
        ):
    ...
    def test_merge_documents_invalid_additional(self):
        ...
        with pytest.raises(
            ValueError, match="Invalid additional execution document structure"
        ):
            merge_execution_documents(base_doc, additional_doc)
