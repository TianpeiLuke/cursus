...
from cursus.api.dag.pipeline_dag_resolver import (
    PipelineDAGResolver,
    PipelineExecutionPlan,
)
from cursus.core.base.step_interface import (
    ContractSection,
    InputPort,
    OutputPort,
    EnvVars,
...
def make_contract(
    entry_point,
    expected_input_paths=None,
    expected_output_paths=None,
    required_env_vars=None,
):
    """Build a ContractSection (unified StepInterface contract) from legacy
    ScriptContract-style kwargs.

    The deleted ScriptContract took flat ``expected_input_paths`` /
    ``expected_output_paths`` dicts and a ``required_env_vars`` list. The unified
    ContractSection stores structured ports keyed by logical name and exposes the
    same ``expected_input_paths`` / ``expected_output_paths`` accessors the resolver
    reads, so this preserves the original test intent (same entry_point, same
    channel -> path mappings).
    """
    return ContractSection(
        entry_point=entry_point,
        inputs={
            name: InputPort(path=path)
            for name, path in (expected_input_paths or {}).items()
        },
        outputs={
            name: OutputPort(path=path)
            for name, path in (expected_output_paths or {}).items()
        },
        env_vars=EnvVars(required=list(required_env_vars or [])),
    )
...
class TestPipelineDAGResolverEnhanced:
    ...
    def setup_method(self):
        ...
        self.complex_resolver = PipelineDAGResolver(
            self.complex_dag, validate_on_init=False
        )
    ...
    def test_init_with_workspace_dirs(self):
        ...
        with patch("cursus.step_catalog.StepCatalog") as mock_catalog_class:
            ...
            resolver = PipelineDAGResolver(
                self.dag, workspace_dirs=workspace_dirs, validate_on_init=False
            )
    ...
    def test_init_with_config_path(self):
        ...
        with patch.object(PipelineDAGResolver, "_load_configs_from_file") as mock_load:
            ...
            resolver = PipelineDAGResolver(
                self.dag, config_path=config_path, validate_on_init=False
            )
    ...
    def test_init_with_available_configs(self):
        ...
        resolver = PipelineDAGResolver(
            self.dag, available_configs=available_configs, validate_on_init=False
        )
    ...
    def test_init_with_metadata(self):
        ...
        resolver = PipelineDAGResolver(
            self.dag, metadata=metadata, validate_on_init=False
        )
    ...
    def test_init_step_catalog_import_error(self):
        ...
        # FIXED: Mock at the actual import location in the source code
        with patch(
            "cursus.step_catalog.StepCatalog",
            side_effect=ImportError("StepCatalog not available"),
        ):
    ...
    def test_create_execution_plan_config_resolver_error(self):
        ...
        mock_config_resolver = Mock()
        mock_config_resolver.resolve_config_map.side_effect = ConfigurationError(
    ...
    def test_create_execution_plan_config_with_dict_conversion(self):
        ...
        assert plan.step_configs["TabularPreprocessing"] == {
            "param1": "value1",
            "param2": "value2",
        }
    ...
    def test_build_data_flow_map_with_contracts(self):
        ...
        tabular_contract = make_contract(
        ...
        with patch.object(
            ...
            with patch.object(
                self.resolver, "_find_compatible_output"
            ) as mock_find_output:
                ...
                assert (
                    data_flow_map["XGBoostTraining"]["training_data"]
                    == "TabularPreprocessing:processed_data"
                )

                # XGBoostModelEval should map model_input to XGBoostTraining:model_output
                assert (
                    data_flow_map["XGBoostModelEval"]["model_input"]
                    == "XGBoostTraining:model_output"
                )
    ...
    def test_discover_step_contract_no_catalog_fallback(self):
        ...
        with patch.object(
            self.resolver, "_discover_step_contract_legacy"
        ) as mock_legacy:
            mock_contract = Mock()
    ...
    def test_discover_step_contract_legacy_success(self):
        ...
        with patch(
            "cursus.api.dag.pipeline_dag_resolver.get_canonical_name_from_file_name"
        ) as mock_canonical:
            with patch.object(
                self.resolver, "_get_step_specification"
            ) as mock_get_spec:
    ...
    def test_get_step_specification_exception(self):
        ...
        # FIXED: Mock at the actual import location in the source code
        with patch(
            "cursus.step_catalog.StepCatalog", side_effect=Exception("Catalog error")
        ):
            result = self.resolver._get_step_specification("test_step")
    ...
    def test_validate_dag_integrity_no_issues(self):
        ...
        with patch.object(self.resolver, "_validate_graph_structure", return_value={}):
            with patch.object(
                self.resolver, "_validate_steps_with_catalog", return_value={}
            ):
                with patch.object(
                    self.resolver, "_validate_component_availability", return_value={}
                ):
                    with patch.object(
                        self.resolver,
    ...
    def test_load_configs_from_file_success(self):
        ...
        with patch("builtins.open", mock_open_with_json(config_data)):
            with patch("pathlib.Path.exists", return_value=True):
                with patch.object(
                    self.resolver, "_instantiate_config_from_catalog"
                ) as mock_instantiate:
    ...
    def test_load_configs_from_file_invalid_json(self):
        ...
        with patch("builtins.open", mock_open_with_content("invalid json")):
            with patch("pathlib.Path.exists", return_value=True):
                with pytest.raises(
                    Exception
                ):  # Should raise ConfigurationError or ValueError
    ...
    def test_instantiate_config_from_catalog_direct_lookup(self):
        ...
        with patch.object(
            self.resolver, "_get_config_class_by_name", return_value=mock_config_class
        ):
            with patch.object(
                self.resolver,
                ...
                result = self.resolver._instantiate_config_from_catalog(
                    "test_step", {"param": "value"}
                )
    ...
    def test_get_config_class_by_name_success(self):
        ...
        with patch("importlib.import_module", return_value=mock_module):
            with patch("inspect.isclass", return_value=True):
                ...
                with patch.object(
                    self.resolver, "_get_config_class_by_name"
                ) as mock_method:
    ...
    def test_create_config_instance_with_from_dict(self):
        ...
        result = self.resolver._create_config_instance(
            mock_config_class, {"param": "value"}
        )
    ...
    def test_create_config_instance_fallback(self):
        ...
        with patch.object(
            mock_config_class,
            "__call__",
            side_effect=[Exception("Failed"), mock_instance],
        ):
            result = self.resolver._create_config_instance(
                mock_config_class, {"param": "value"}
            )
    ...
    def test_get_config_resolution_preview_exception(self):
        ...
        mock_config_resolver = Mock()
        mock_config_resolver.preview_resolution.side_effect = Exception(
            "Preview failed"
        )
