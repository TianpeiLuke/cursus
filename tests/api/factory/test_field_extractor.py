...
class ComplexTestConfig(BaseModel):
    ...
    dict_field: Dict[str, Any] = Field(
        default_factory=dict, description="Dictionary field"
    )
...
class DerivedTestConfig(BaseTestConfig):
    ...
    derived_optional: Optional[bool] = Field(
        default=False, description="Derived optional field"
    )
...
class TestExtractFieldRequirements:
    ...
    def test_extract_complex_config_fields(self):
        ...
        required_req = next(
            req for req in requirements if req["name"] == "required_str"
        )
        ...
        optional_req = next(
            req for req in requirements if req["name"] == "optional_int"
        )
...
class TestValidateFieldValue:
    ...
    def test_validate_optional_field(self):
        ...
        assert (
            validate_field_value(field_req, "") is True
        )  # Empty string OK for optional
...
class TestPrintFieldRequirements:
    ...
    def test_print_mixed_requirements(self, mock_print):
        ...
        required_call = next(
            (call for call in calls if "required_field" in call and "*" in call), None
        )
        ...
        optional_call = next(
            (
                call
                for call in calls
                if "optional_field" in call and "default: 42" in call
            ),
            None,
        )
...
class TestIntegrationWithRealPydanticModels:
    ...
    def test_inheritance_extraction_integration(self):
        ...
        expected_non_inherited_count = len(derived_requirements) - len(
            base_requirements
        )
