...
class TestConfigClassMapper:
    ...
    def test_init_with_registry_success(self):
        ...
        with patch(
            "cursus.core.config_fields.unified_config_manager.get_unified_config_manager"
        ) as mock_get_manager:
    ...
    def test_init_with_registry_import_error(self):
        ...
        with patch(
            ...
            assert (
                not hasattr(mapper, "unified_manager") or mapper.unified_manager is None
            )
    ...
    def test_resolve_node_to_config_class_registry_failure(self):
        ...
        mapper = ConfigClassMapper()
        mapper.resolver_adapter = MagicMock()
        mapper.resolver_adapter.get_config_class.side_effect = Exception(
            "Registry error"
        )
    ...
    def test_resolve_node_to_config_class_pattern_matching(self):
        ...
        with patch.object(mapper, "_matches_node_pattern") as mock_matches:
            mock_matches.side_effect = lambda node, cls: (
                node == "some_node" and cls == "SomeConfigClass"
            )
    ...
    def test_resolve_node_to_config_class_no_match(self):
        ...
        with patch.object(
            mapper, "_infer_config_class_from_node_name", return_value=None
        ):
            result = mapper.resolve_node_to_config_class("unknown_node")
    ...
    def test_matches_node_pattern_direct_substring(self):
        ...
        assert (
            mapper._matches_node_pattern("preprocessing", "PreprocessingConfig") is True
        )
        assert (
            mapper._matches_node_pattern("PreprocessingStep", "preprocessing") is True
        )
    ...
    def test_matches_node_pattern_cleaned_matching(self):
        ...
        assert (
            mapper._matches_node_pattern("preprocessing", "PreprocessingStepConfig")
            is True
        )
    ...
    def test_get_available_config_classes(self):
        ...
        mapper = ConfigClassMapper()
        mapper.config_classes = {"ConfigA": TestConfigA, "ConfigB": TestConfigB}
    ...
    def test_validate_mapping_all_valid(self):
        ...
        config_map = {"node1": TestConfigA, "node2": TestConfigB, "node3": TestConfigC}
    ...
    def test_validate_mapping_non_pydantic_class(self):
        ...
        config_map = {"node1": TestConfigA, "node2": NonPydanticClass}
...
class TestManualConfigClassMapper:
    ...
    def test_init_with_manual_mappings(self):
        ...
        mappings = {"node1": TestConfigA, "node2": TestConfigB}
    ...
    def test_add_mappings(self):
        ...
        mappings = {"node1": TestConfigA, "node2": TestConfigB, "node3": TestConfigC}
...
class TestIntegrationScenarios:
    ...
    def test_complete_dag_mapping_workflow(self):
        ...
        with patch.object(
            mapper, "_matches_node_pattern", side_effect=mock_pattern_match
        ):
            config_map = mapper.map_dag_to_config_classes(dag)
    ...
    def test_mixed_registry_and_manual_mapping(self):
        ...
        mapper = ConfigClassMapper()
        mapper.resolver_adapter = MagicMock()
        mapper.resolver_adapter.get_config_class.side_effect = lambda node: (
            TestConfigA if node == "registry_node" else None
        )
...
def sample_config_classes():
    ...
    return {"ConfigA": TestConfigA, "ConfigB": TestConfigB, "ConfigC": TestConfigC}
...
class TestFixtureUsage:
    ...
    def test_integration_with_fixtures(self, sample_dag, configured_mapper):
        ...
        with patch.object(
            configured_mapper, "_matches_node_pattern", return_value=True
        ):
