...
class TestConfigurationGenerator:
    ...
    def test_init_with_both_configs(self):
        ...
        processing_config = MockBaseProcessingStepConfig(
            project_name="test_project", processing_mode="stream"
        )

        generator = ConfigurationGenerator(
            base_config=base_config, base_processing_config=processing_config
        )
    ...
    def test_extract_config_values_pydantic_v2(self):
        ...
        config = MockBasePipelineConfig(
            project_name="test_project", version="2.0.0", debug=True
        )
    ...
    def test_generate_standalone_config(self):
        ...
        result = generator._generate_standalone_config(
            MockStandaloneConfig, step_inputs
        )
    ...
    def test_generate_with_base_inheritance_using_from_base_config(self):
        ...
        base_config = MockBasePipelineConfig(
            project_name="test_project", version="1.5.0"
        )
        generator = ConfigurationGenerator(base_config=base_config)

        # Mock from_base_config method
        mock_result = MockStepConfigA(
            project_name="test_project", version="1.5.0", step_param_a="test_param"
        )

        with patch.object(
            MockStepConfigA, "from_base_config", return_value=mock_result
        ) as mock_from_base:
            step_inputs = {"step_param_a": "test_param"}

            result = generator._generate_with_base_inheritance(
                MockStepConfigA, step_inputs
            )
    ...
    def test_generate_with_base_inheritance_fallback(self):
        ...
        with patch.object(
            MockStepConfigA,
            "from_base_config",
            side_effect=Exception("from_base_config failed"),
        ):
            step_inputs = {"step_param_a": "test_param"}
    ...
    def test_generate_with_processing_inheritance_using_from_base_config(self):
        ...
        mock_result = MockStepConfigB(
            project_name="test_project", processing_mode="stream", step_param_b=42
        )

        with patch.object(
            MockStepConfigB, "from_base_config", return_value=mock_result
        ) as mock_from_base:
            step_inputs = {"step_param_b": 42}

            result = generator._generate_with_processing_inheritance(
                MockStepConfigB, step_inputs
            )
    ...
    def test_generate_config_instance_with_base_inheritance(self):
        ...
        with patch.object(
            generator, "_inherits_from_processing_config", return_value=False
        ):
            with patch.object(
                generator, "_inherits_from_base_config", return_value=True
            ):
                with patch.object(
                    generator, "_generate_with_base_inheritance"
                ) as mock_generate:
                    mock_generate.return_value = MockStepConfigA(
                        project_name="test_project", step_param_a="test"
                    )

                    step_inputs = {"step_param_a": "test"}
                    result = generator.generate_config_instance(
                        MockStepConfigA, step_inputs
                    )
    ...
    def test_generate_config_instance_with_processing_inheritance(self):
        ...
        # Mock inheritance detection
        with patch.object(
            generator, "_inherits_from_processing_config", return_value=True
        ):
            with patch.object(
                generator, "_generate_with_processing_inheritance"
            ) as mock_generate:
                mock_generate.return_value = MockStepConfigB(
                    project_name="test_project", step_param_b=42
                )
    ...
    def test_generate_all_instances_success(self):
        ...
        mock_config_a = MockStepConfigA(
            project_name="test_project", step_param_a="test_a"
        )
        mock_config_standalone = MockStandaloneConfig(
            standalone_param="test_standalone"
        )
        ...
        with patch.object(generator, "generate_config_instance") as mock_generate:
            ...
            mock_generate.assert_any_call(
                MockStandaloneConfig, {"standalone_param": "test_standalone"}
            )
    ...
    def test_generate_all_instances_with_missing_step_config(self):
        ...
        config_class_map = {"step_a": MockStepConfigA, "step_b": MockStandaloneConfig}
    ...
    def test_generate_all_instances_failure(self):
        ...
        with patch.object(generator, "generate_config_instance") as mock_generate:
            ...
            with pytest.raises(
                ValueError, match="Configuration generation failed for step step_b"
            ):
    ...
    def test_validate_generated_configs_with_errors(self):
        ...
        with patch.object(
            generator, "_check_required_fields", return_value=["Required field missing"]
        ):
            validation_results = generator.validate_generated_configs(configs)
            ...
            assert (
                "Required field missing" in validation_results["MockBasePipelineConfig"]
            )
    ...
    def test_check_required_fields_with_missing_values(self):
        ...
        with patch.object(
            ...
            return_value=["Required field 'missing_field' is missing or empty"],
        ):
            config = MockStepConfigA(project_name="test_project", step_param_a="test")
            errors = generator._check_required_fields(config)
...
class TestIntegrationScenarios:
    ...
    def test_complete_generation_workflow(self):
        ...
        base_config = MockBasePipelineConfig(
            project_name="integration_test", version="2.0.0"
        )
        processing_config = MockBaseProcessingStepConfig(
        ...
        preprocessing_config = next(
            c for c in results if isinstance(c, MockStepConfigA)
        )
        ...
        processing_config_result = next(
            c for c in results if isinstance(c, MockStepConfigB)
        )
        ...
        standalone_config = next(
            c for c in results if isinstance(c, MockStandaloneConfig)
        )
...
class TestFixtureUsage:
    ...
    def test_generator_with_both_fixture(
        self, generator_with_both, base_config, processing_config
    ):
    ...
    def test_integration_with_fixtures(self, generator_with_both):
        ...
        # Mock inheritance detection
        with patch.object(
            generator_with_both, "_inherits_from_processing_config", return_value=True
        ):
            result = generator_with_both.generate_config_instance(
                MockStepConfigB, step_inputs
            )
