...
'''
    ...
    spec_content = {
        ...
        "configuration": {
            ...
            "fields": {
                "instance_type": {"type": "str", "default": "ml.m5.large"},
                "instance_count": {"type": "int", "default": 1},
                "max_runtime_seconds": {"type": "int", "default": 3600},
            },
        },
    }
...
'''
    ...
    (contracts_dir / "sample_processor_contract.json").write_text(
        json.dumps(contract_content, indent=2)
    )
    (specs_dir / "sample_processor_spec.json").write_text(
...
def test_unified_alignment_tester():
    ...
    try:
        ...
        if scripts:
            ...
            print(
                f"   Level 1 (Script ↔ Contract): {result['level1'].get('passed', 'N/A')}"
            )
            print(
                f"   Level 2 (Contract ↔ Spec): {result['level2'].get('passed', 'N/A')}"
            )
            print(
                f"   Level 3 (Spec ↔ Dependencies): {result['level3'].get('passed', 'N/A')}"
            )
            print(
                f"   Level 4 (Builder ↔ Config): {result['level4'].get('passed', 'N/A')}"
            )
            print()
...
def test_individual_levels():
    ...
    try:
        ...
        for level in range(1, 5):
            print(f"\n📊 Testing Level {level}...")
            try:
                report = tester.run_level_validation(
                    level, target_scripts=["sample_processor"]
                )
                summary = tester.get_validation_summary()
                print(f"   Level {level} Status: {summary['overall_status']}")
                print(f"   Tests Run: {summary['total_tests']}")
                print(
                    f"   Issues Found: {summary['critical_issues'] + summary['error_issues'] + summary['warning_issues']}"
                )
            except Exception as e:
