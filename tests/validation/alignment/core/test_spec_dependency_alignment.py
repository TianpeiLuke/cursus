...
from cursus.validation.alignment.core.spec_dependency_alignment import (
    SpecificationDependencyAlignmentTester,
)
from cursus.validation.alignment.validators.dependency_validator import (
    DependencyValidator,
)
...
class TestSpecDependencyAlignment:
    ...
    def test_validate_specification_with_resolved_dependencies(
        self, spec_dependency_alignment, sample_specification, sample_dependency_info
    ):
        ...
        with (
            patch.object(
                spec_dependency_alignment.step_catalog, "load_spec_class"
            ) as mock_load_spec,
            patch.object(
                spec_dependency_alignment.step_catalog, "serialize_spec"
            ) as mock_serialize_spec,
            patch(
                "cursus.validation.alignment.validators.dependency_validator.DependencyValidator"
    ...
    def test_validate_specification_with_unresolved_dependencies(
        self, spec_dependency_alignment, sample_specification
    ):
        ...
        with (
            ...
            patch.object(
                spec_dependency_alignment.dependency_validator,
                "validate_dependency_resolution",
            ) as mock_dep_resolution,
            patch.object(
                spec_dependency_alignment.dependency_validator,
    ...
    def test_validate_specification_with_circular_dependencies(
        ...
        with (
            ...
            mock_circular.return_value = (
                validation_issues  # Circular dependency issues come from this method
            )
            mock_data_types.return_value = []
    ...
    def test_validate_specification_with_missing_specification(
        ...
        with patch.object(
            ...
            assert len(result["issues"]) > 0
            assert any(
                "specification" in issue.get("message", "").lower()
                for issue in result["issues"]
            )

    def test_validate_specification_with_malformed_specification(
        self, spec_dependency_alignment
    ):
    ...
    def test_validate_specification_result_structure(
        self, spec_dependency_alignment, sample_specification, sample_dependency_info
    ):
    ...
    def test_integration_with_dependency_validator(
        self, spec_dependency_alignment, sample_specification
    ):
    ...
    def test_validate_specification_with_complex_dependencies(
        self, spec_dependency_alignment
    ):
    ...
    def test_validate_specification_with_mixed_dependency_types(
        self, spec_dependency_alignment
    ):
        ...
        mixed_spec = {
            ...
            "outputs": [{"logical_name": "mixed_output", "type": "s3_path"}],
        }
    ...
    def test_validate_specification_performance_with_large_spec(
        self, spec_dependency_alignment
    ):
        ...
        large_spec = {
            ...
            "outputs": [
                {"logical_name": f"large_output_{i}", "type": "s3_path"}
                for i in range(50)
            ],
        }
