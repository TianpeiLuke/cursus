...
from cursus.validation.alignment.core.script_contract_alignment import (
...
class TestScriptContractAlignment:
    ...
    def sample_contract(self):
        ...
        return {
            ...
            "optional_env_vars": {"LEARNING_RATE": "0.01", "BATCH_SIZE": "32"},
            "expected_arguments": {
    ...
    def sample_step_info(self):
        ...
        mock_step_info = Mock()
        mock_step_info.file_components = {"script": Mock(path=Path("/test/script.py"))}
    ...
    def test_validate_script_with_valid_main_function(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
        ...
        with (
            patch.object(
                script_contract_alignment.step_catalog, "get_step_info"
            ) as mock_get_step_info,
            patch.object(
                script_contract_alignment, "_load_python_contract"
            ) as mock_load_contract,
            patch(
                "cursus.validation.alignment.analyzer.script_analyzer.ScriptAnalyzer"
            ...
            mock_analyzer_class.assert_called_once_with(
                str(sample_step_info.file_components["script"].path)
            )
            ...
            mock_analyzer.validate_contract_alignment.assert_called_once_with(
                sample_contract
            )

    def test_validate_script_with_missing_main_function(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
        ...
        with (
            ...
            assert any(
                "main function" in issue.get("message", "").lower()
                for issue in critical_issues
            )

    def test_validate_script_with_invalid_signature(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
        ...
        main_function_result = {
            ...
            "issues": [
                "Expected 4 parameters, got 2",
                "Parameter 1: expected 'input_paths', got 'data'",
            ],
        }
        ...
        with (
            ...
            assert len(result["issues"]) > 0
            assert (
                result["script_analysis"]["main_function"]["signature_valid"] is False
            )

    def test_validate_script_with_contract_alignment_issues(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
        ...
        parameter_usage = {
            "input_paths_keys": [
                "training_data",
                "test_data",
            ],  # test_data not in contract
            "output_paths_keys": ["model_artifacts"],
            "environ_vars_keys": [
                "MODEL_TYPE",
                "UNKNOWN_VAR",
            ],  # UNKNOWN_VAR not in contract
        ...
        with (
            ...
            error_issues = [
                issue for issue in result["issues"] if issue.get("severity") == "ERROR"
            ]
            warning_issues = [
                issue
                for issue in result["issues"]
    ...
    def test_validate_script_with_contract_loading_error(
        self, script_contract_alignment, sample_step_info
    ):
    ...
    def test_validate_script_with_script_analyzer_error(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
    ...
    def test_validate_script_result_structure(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
    ...
    def test_integration_with_script_analyzer(
        self, script_contract_alignment, sample_contract, sample_step_info
    ):
    ...
    def test_validate_script_with_complex_parameter_usage(
        self, script_contract_alignment, sample_step_info
    ):
        ...
        complex_contract = {
            "expected_input_paths": {
                f"input_{i}": f"/path/to/input_{i}" for i in range(10)
            },
            "expected_output_paths": {
                f"output_{i}": f"/path/to/output_{i}" for i in range(10)
            },
        ...
        complex_usage = {
            ...
            "environ_vars_keys": [f"ENV_VAR_{i}" for i in range(5)]
            + [f"OPT_VAR_{i}" for i in range(5)],
    ...
    def test_validate_script_performance_with_large_script(
        self, script_contract_alignment, sample_contract
    ):
