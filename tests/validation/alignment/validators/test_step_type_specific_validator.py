...
from cursus.validation.alignment.validators.step_type_specific_validator import (
    StepTypeSpecificValidator,
)
...
class ConcreteStepTypeValidator(StepTypeSpecificValidator):
    ...
    def _validate_step_type_specifics(
        self, step_name: str, builder_class, step_type: str
    ) -> List[Dict[str, Any]]:
...
class TestStepTypeSpecificValidator:
    ...
    def test_validate_builder_config_alignment_priority_system(self, validator):
        ...
        with (
            ...
            patch.object(
                validator, "_apply_step_specific_validation"
            ) as mock_step_specific,
    ...
    def test_apply_universal_validation_with_valid_builder(
        self, validator, sample_builder_class
    ):
        """Test universal validation with valid builder class."""
        step_name = "test_step"

        with (
            patch.object(validator, "_get_builder_class") as mock_get_builder,
    ...
    def test_apply_step_specific_validation_implementation(
        self, validator, sample_builder_class
    ):
        ...
        with (
            patch.object(validator, "_get_builder_class") as mock_get_builder,
            patch(
                "cursus.registry.step_names.get_sagemaker_step_type"
    ...
    def test_resolve_validation_priorities_no_issues(self, validator):
        ...
        result = validator._resolve_validation_priorities(
            universal_result, step_specific_result
        )
        ...
        assert (
            result["priority_resolution"] == "universal_rules_first_then_step_specific"
        )
    ...
    def test_is_method_overridden_detection(self, validator):
        ...
        assert (
            validator._is_method_overridden(DerivedClass, "derived_method") is True
        )  # This is actually True because it exists in the class
    ...
    def test_integration_with_validation_rules(self, validator):
        ...
        with (
            patch(
                "cursus.validation.alignment.validators.step_type_specific_validator.get_universal_validation_rules"
            ) as mock_universal,
            patch(
                "cursus.validation.alignment.validators.step_type_specific_validator.get_step_type_validation_rules"
            ) as mock_step_type,
        ):
