...
class TestValidatorFactory:
    ...
    @patch(
        "cursus.validation.alignment.validators.validator_factory.get_universal_validation_rules"
    )
    @patch(
        "cursus.validation.alignment.validators.validator_factory.get_step_type_validation_rules"
    )
    def test_factory_initialization_with_rules(
        self, mock_step_type_rules, mock_universal_rules
    ):
    ...
    @patch(
        "cursus.validation.alignment.validators.validator_factory.get_validation_ruleset"
    )
    def test_get_validator_for_step_type_success(self, mock_get_ruleset):
        ...
        mock_validator_instance = Mock()
        mock_validator_class.return_value = mock_validator_instance
        self.factory._validator_registry["ProcessingStepBuilderValidator"] = (
    ...
    def test_validate_step_with_priority_system(self, mock_get_ruleset):
        ...
        # Mock step type detection
        with patch(
            "cursus.validation.alignment.validators.validator_factory.get_sagemaker_step_type",
            return_value=step_type,
        ):
            result = self.factory.validate_step_with_priority_system("test_step")

        # Verify validation was called
        mock_validator_instance.validate_builder_config_alignment.assert_called_once_with(
            "test_step"
        )
    ...
    @patch(
        "cursus.validation.alignment.validators.validator_factory.is_step_type_excluded"
    )
    def test_validate_step_excluded_type(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type
    ):
    ...
    def test_validate_step_no_validator(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type
    ):
