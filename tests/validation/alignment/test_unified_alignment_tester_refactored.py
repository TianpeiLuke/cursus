...
class TestUnifiedAlignmentTesterRefactored:
    ...
    def tester(self, mock_workspace_dirs):
        ...
        with (
            ...
            patch(
                "cursus.validation.alignment.unified_alignment_tester.LevelValidators"
            ),
        ):
    ...
    def test_initialization_with_configuration_validation(self, mock_workspace_dirs):
        ...
        with (
            patch(
                "cursus.validation.alignment.unified_alignment_tester.validate_step_type_configuration"
            ) as mock_validate,
            patch("cursus.validation.alignment.unified_alignment_tester.StepCatalog"),
            patch(
                "cursus.validation.alignment.unified_alignment_tester.LevelValidators"
    ...
    def test_initialization_with_configuration_issues(self, mock_workspace_dirs):
        ...
        with (
            ...
            patch(
                "cursus.validation.alignment.unified_alignment_tester.logger"
            ) as mock_logger,
        ):
    ...
    @patch(
        "cursus.validation.alignment.unified_alignment_tester.get_sagemaker_step_type"
    )
    @patch(
        "cursus.validation.alignment.unified_alignment_tester.get_validation_ruleset"
    )
    @patch("cursus.validation.alignment.unified_alignment_tester.is_step_type_excluded")
    def test_run_validation_for_step_excluded_step(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type, tester
    ):
    ...
    @patch("cursus.validation.alignment.unified_alignment_tester.is_step_type_excluded")
    def test_run_validation_for_step_script_based(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type, tester
    ):
        ...
        tester.level_validators.run_level_4_validation = Mock(
            return_value={"passed": True, "issues": []}
        )
    ...
    @patch("cursus.validation.alignment.unified_alignment_tester.is_step_type_excluded")
    def test_run_validation_for_step_non_script(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type, tester
    ):
    ...
    @patch("cursus.validation.alignment.unified_alignment_tester.is_step_type_excluded")
    def test_run_validation_for_step_with_failures(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type, tester
    ):
        ...
        tester.level_validators.run_level_3_validation = Mock(
            return_value={
                "status": "ERROR",
                "issues": [
                    {"severity": "ERROR", "message": "Dependency validation failed"}
                ],
            }
        )
        tester.level_validators.run_level_4_validation = Mock(
    ...
    def test_discover_scripts_integration(self, tester):
        ...
        tester.step_catalog.list_available_steps = Mock(
            return_value=mock_available_steps
        )
    ...
    @patch("cursus.validation.alignment.unified_alignment_tester.is_step_type_excluded")
    def test_run_full_validation_with_mixed_step_types(
        self, mock_is_excluded, mock_get_ruleset, mock_get_step_type, tester
    ):
        """Test full validation with mixed step types."""
        # Mock script discovery method that _discover_all_steps actually calls
        tester.step_catalog.list_available_steps = Mock(
            return_value=["processing_script", "create_model_step", "base_config"]
        )
        ...
        base_result = next(
            r for r in results.values() if r["step_name"] == "base_config"
        )
        assert base_result["status"] == "EXCLUDED"
        assert (
            "validation_results" not in base_result
            or len(base_result.get("validation_results", {})) == 0
        )
    ...
    def test_performance_optimization_level_skipping(self, tester):
        ...
        with (
            patch(
                "cursus.validation.alignment.unified_alignment_tester.get_sagemaker_step_type"
            ) as mock_get_step_type,
            patch(
                "cursus.validation.alignment.unified_alignment_tester.get_validation_ruleset"
            ) as mock_get_ruleset,
            patch(
                "cursus.validation.alignment.unified_alignment_tester.is_step_type_excluded"
            ) as mock_is_excluded,
        ):
