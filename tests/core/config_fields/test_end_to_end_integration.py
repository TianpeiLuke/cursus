...
class TestEndToEndIntegrationUnified:
    ...
    def test_unified_save_config_to_target_format(self):
        ...
        saved_result = self.unified_manager.save(
            original_configs, str(self.config_file)
        )
        ...
        assert expected_top_level_keys == actual_top_level_keys, (
            f"Top-level keys mismatch. Expected: {expected_top_level_keys}, Got: {actual_top_level_keys}"
        )
        ...
        assert expected_metadata_keys.issubset(actual_metadata_keys), (
            f"Missing metadata keys. Expected: {expected_metadata_keys}, Got: {actual_metadata_keys}"
        )
        ...
        assert expected_config_keys == actual_config_keys, (
            f"Configuration keys mismatch. Expected: {expected_config_keys}, Got: {actual_config_keys}"
        )

        # Verify we have the expected number of specific configs
        specific_configs = configuration["specific"]
        assert len(specific_configs) == 3, (
            f"Should have 3 specific configs, got {len(specific_configs)}"
        )
        ...
        for step_name, config_data in specific_configs.items():
            assert "__model_type__" in config_data, (
                f"Config {step_name} missing __model_type__"
            )
            assert isinstance(config_data["__model_type__"], str), (
                f"__model_type__ should be string for {step_name}"
            )
        ...
        assert expected_types.issubset(actual_types), (
            f"Missing expected config types. Expected: {expected_types}, Got: {actual_types}"
        )
    ...
    def test_unified_load_config_complete_reconstruction(self):
        ...
        assert "specific" in loaded_result, (
            "Loaded result should have 'specific' section"
        )
        ...
        for field in derived_fields:
            assert field in all_field_names, (
                f"Derived field '{field}' should exist in loaded data"
            )

        # Step 5: Verify specific data has our configs
        specific_data = loaded_result["specific"]
        assert len(specific_data) == 3, (
            f"Should have 3 specific configs, got {len(specific_data)}"
        )

        # Step 6: Verify each specific config has expected structure
        for step_name, config_data in specific_data.items():
            assert isinstance(config_data, dict), (
                f"Config data for {step_name} should be a dict"
            )
            assert "__model_type__" in config_data, (
                f"Config {step_name} should have __model_type__"
            )
    ...
    def test_round_trip_integrity_with_complex_objects(self):
        ...
        assert processing_step is not None, (
            "Should find ProcessingStepConfigBase in loaded data"
        )

        # Step 4: Verify complex field values are preserved
        # Check that processing-specific fields are preserved
        assert "processing_source_dir" in processing_step, (
            "Processing config should have processing_source_dir"
        )
        assert processing_step["processing_source_dir"] == original_processing_dir, (
            "processing_source_dir should be preserved"
        )

        assert "processing_instance_type_large" in processing_step, (
            "Processing config should have processing_instance_type_large"
        )
        assert (
            processing_step["processing_instance_type_large"] == original_instance_type
        ), "processing_instance_type_large should be preserved"

        assert "processing_volume_size" in processing_step, (
            "Processing config should have processing_volume_size"
        )
        assert processing_step["processing_volume_size"] == original_volume_size, (
            "processing_volume_size should be preserved"
        )
        ...
        assert training_step is not None, (
            "Should find XGBoostTrainingConfig in loaded data"
        )

        # XGBoostTrainingConfig should have training-specific fields (not hyperparameters)
        assert "training_entry_point" in training_step, (
            "Training config should have training_entry_point"
        )
        assert training_step["training_entry_point"] == "xgboost_training.py", (
            "training_entry_point should be preserved"
        )

        assert "training_instance_type" in training_step, (
            "Training config should have training_instance_type"
        )
        assert training_step["training_instance_type"] == "ml.m5.4xlarge", (
            "training_instance_type should be preserved"
        )
    ...
    def test_shared_vs_specific_field_categorization(self):
        ...
        assert len(all_specific_fields) > 0, (
            "Should have some fields that are specific to individual configs"
        )
    ...
    def test_metadata_completeness_and_accuracy(self):
        ...
        assert len(config_types) == 3, (
            f"Should have exactly 3 config type entries, got {len(config_types)}"
        )
        ...
        for field_name, sources in field_sources.items():
            assert isinstance(sources, list), (
                f"Field sources for '{field_name}' should be a list"
            )
            assert len(sources) > 0, (
                f"Field '{field_name}' should have at least one source"
            )
    ...
    def test_field_sources_inverted_index_accuracy(self):
        ...
        for field_name in shared_fields.keys():
            assert field_name in field_sources, (
                f"Shared field '{field_name}' should be in field_sources"
            )
            sources = field_sources[field_name]
            assert len(sources) > 1, (
                f"Shared field '{field_name}' should have multiple sources, got {sources}"
            )
        ...
        for step_name, config_data in specific_configs.items():
            for field_name in config_data.keys():
                if not field_name.startswith("__"):  # Skip metadata fields
                    assert field_name in field_sources, (
                        f"Specific field '{field_name}' should be in field_sources"
                    )
                    sources = field_sources[field_name]
                    assert step_name in sources, (
                        f"Step '{step_name}' should be in sources for field '{field_name}'"
                    )
    ...
    def test_error_handling_unified_manager(self):
        ...
        try:
            ...
            assert isinstance(e, (ValueError, TypeError, IndexError, AttributeError)), (
    ...
    def test_performance_and_file_size_unified(self):
        ...
        assert save_time < 10.0, (
            f"Save operation took too long: {save_time:.2f} seconds"
        )
        ...
        assert file_size < 1024 * 1024, (
            f"Config file seems too large: {file_size} bytes"
        )  # Less than 1MB
        ...
        assert load_time < 10.0, (
            f"Load operation took too long: {load_time:.2f} seconds"
        )
        ...
        for step_name, config_data in specific_data.items():
            if "hyperparameters" in config_data:
                ...
                if hasattr(hyperparams, "full_field_list"):
                    ...
                    assert hasattr(hyperparams, "num_round"), (
                        "Should preserve hyperparameter attributes"
                    )
                    assert hasattr(hyperparams, "max_depth"), (
                        "Should preserve hyperparameter attributes"
                    )
                    assert isinstance(hyperparams.full_field_list, list), (
                        "Field list should remain a list"
                    )
                elif isinstance(hyperparams, dict):
                    # It's a dictionary representation
                    assert "full_field_list" in hyperparams, (
                        "Should preserve nested list fields"
                    )
                    assert isinstance(hyperparams["full_field_list"], list), (
                        "Field list should remain a list"
                    )
                    ...
                    assert False, (
                        f"Unexpected hyperparameters type: {type(hyperparams)}, value: {hyperparams}"
                    )
    ...
    def test_unified_manager_vs_target_format_exact_match(self):
        ...
        assert set(saved_data.keys()) == {"metadata", "configuration"}, (
            "Top-level structure should match target"
        )

        # 2. Metadata structure should match
        metadata = saved_data["metadata"]
        required_metadata_keys = {"created_at", "config_types", "field_sources"}
        assert required_metadata_keys.issubset(set(metadata.keys())), (
            "Metadata should have required keys"
        )

        # 3. Configuration structure should match
        configuration = saved_data["configuration"]
        assert set(configuration.keys()) == {"shared", "specific"}, (
            "Configuration should have shared and specific"
        )
        ...
        for field_name, sources in field_sources.items():
            ...
            assert all(isinstance(source, str) for source in sources), (
                f"All sources should be strings for {field_name}"
            )
