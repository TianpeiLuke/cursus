...
class TestStepCatalogAwareConfigFieldCategorizer:
    ...
    @patch(
        "cursus.core.config_fields.step_catalog_aware_categorizer.get_unified_config_manager"
    )
    def test_init_basic_functionality_following_guides(self, mock_get_unified_manager):
        ...
        assert (
            len(categorizer.field_info["sources"]["author"]) >= 1
        )  # Should appear in configs

    @patch(
        "cursus.core.config_fields.step_catalog_aware_categorizer.get_unified_config_manager"
    )
    def test_init_with_enhanced_features_following_guides(
        self, mock_get_unified_manager
    ):
    ...
    def test_init_unified_manager_failure_following_guides(
        self, mock_get_unified_manager
    ):
        ...
        mock_get_unified_manager.side_effect = Exception(
            "Manager initialization failed"
        )

        # Create categorizer - should handle exception gracefully
        categorizer = StepCatalogAwareConfigFieldCategorizer(
            config_list=self.configs, workspace_root=self.test_workspace_root
        )
    ...
    def test_get_workspace_field_mappings_no_unified_manager_following_guides(
        self, mock_get_unified_manager
    ):
        ...
        categorizer = StepCatalogAwareConfigFieldCategorizer(
            config_list=self.configs, project_id=self.test_project_id
        )
    ...
    def test_get_workspace_field_mappings_with_unified_manager_following_guides(
        self, mock_get_unified_manager
    ):
        ...
        mock_config_class.model_fields = {"annotated_field": mock_field_info}
    ...
    def test_categorize_field_with_step_catalog_context_workspace_mapping_following_guides(
        self,
    ):
    ...
    def test_categorize_field_with_step_catalog_context_framework_mapping_following_guides(
        self,
    ):
    ...
    def test_categorize_field_with_tier_aware_categorization_following_guides(
        self, mock_get_unified_manager
    ):
    ...
    def test_get_enhanced_categorization_info_following_guides(
        self, mock_get_unified_manager
    ):
        ...
        assert (
            info["framework_field_mappings_count"] > 0
    ...
    def test_get_enhanced_categorization_info_step_catalog_exception_following_guides(
        self,
    ):
        ...
        problematic_catalog.workspace_root = Mock(
            side_effect=Exception("Access failed")
        )

        categorizer = StepCatalogAwareConfigFieldCategorizer(
            config_list=self.configs, step_catalog=problematic_catalog
        )
    ...
    def test_tier_aware_categorization_exception_handling_following_guides(self):
        ...
        categorizer.unified_manager.get_field_tiers.side_effect = Exception(
            "Tier extraction failed"
        )
    ...
    def test_framework_field_mappings_completeness_following_guides(self):
        ...
        expected_categories = {"framework_specific", "cloud_specific", "ml_framework"}
    ...
    def test_workspace_field_mappings_with_model_fields_exception_following_guides(
        self,
    ):
        ...
        type(mock_config_class).model_fields = PropertyMock(
            side_effect=AttributeError("No model_fields")
        )
...
class TestStepCatalogAwareCategorizerIntegration:
    ...
    def test_integration_with_real_workspace_structure_following_guides(
        self, mock_get_unified_manager, temp_workspace
    ):
...
class TestStepCatalogAwareCategorizerCompatibility:
    ...
    def test_enhanced_features_optional_following_guides(
        self, mock_get_unified_manager
    ):
