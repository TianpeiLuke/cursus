...
class TestUnifiedConfigManager:
    ...
    )  # ✅ Category 1: Source location, not import location
    def test_step_catalog_lazy_loading_success(self, mock_step_catalog_class):
        ...
        mock_step_catalog_class.assert_called_once_with(
    ...
    @patch("cursus.step_catalog.StepCatalog")
    def test_get_config_classes_fallback_to_auto_discovery(
        self, mock_step_catalog_class, mock_auto_discovery_class
    ):
        ...
        with patch.object(
            self.manager, "_get_basic_config_classes", return_value=expected_classes
        ):
            # Test discovery
    ...
    def test_get_config_classes_final_fallback(self):
        ...
        with patch(
            "cursus.step_catalog.StepCatalog",
            side_effect=ImportError("No step catalog"),
        ):
            with patch(
                "cursus.step_catalog.config_discovery.ConfigAutoDiscovery",
                side_effect=ImportError("No auto discovery"),
            ):
                with patch.object(
...
class TestUnifiedConfigManagerFileOperations:
    ...
    @patch(
        "cursus.core.config_fields.step_catalog_aware_categorizer.StepCatalogAwareConfigFieldCategorizer"
    )
    @patch(
    ...
    @patch("os.makedirs")
    def test_save_success_following_guides(
        self, mock_makedirs, mock_file, mock_serializer_class, mock_categorizer_class
    ):
        ...
        mock_serializer.generate_step_name.side_effect = [
            "TestStep1",
            "TestStep2",
        ]  # Exactly 2 calls
    ...
    @patch(
        ...
        read_data='{"metadata": {"created_at": "2023-01-01T00:00:00"}, "configuration": {"shared": {"shared_field": "shared_value"}, "specific": {"TestConfig1": {"field1": "value1"}}}}',
    )
    ...
    def test_load_success_following_guides(self, mock_exists, mock_file):
        ...
        with patch.object(
            self.manager, "get_config_classes", return_value=mock_config_classes
        ):
    ...
    def test_verify_essential_structure_invalid_type(self):
        ...
        with pytest.raises(
            ValueError, match="Merged configuration must be a dictionary"
        ):
...
class TestUnifiedConfigManagerSerialization:
    ...
    def test_serialize_with_tier_awareness_nested_dict(self):
        ...
        test_obj = {"level1": {"level2": {"field": "value"}}}
    ...
    def test_serialize_with_tier_awareness_circular_reference(self):
        ...
        with patch.object(
            self.manager.simple_tracker, "enter_object", return_value=True
        ):
            result = self.manager.serialize_with_tier_awareness(test_obj)
    ...
    def test_serialize_with_tier_awareness_primitive_types(self):
        ...
        test_cases = [None, "string", 42, 3.14, True, False]
