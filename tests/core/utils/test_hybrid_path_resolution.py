...
class TestThreeDeploymentScenarios:
    ...
    def test_scenario_1_lambda_mods_bundled_deployment(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mods_project_dir = (
                package_root / "mods_pipeline_adapter" / "dockers" / "xgboost_atoz"
            )
            ...
            with (
                patch.object(
                    self.resolver, "_package_location_discovery"
                ) as mock_package_discovery,
                patch.object(
                    self.resolver, "_working_directory_discovery"
                ) as mock_wd_discovery,
            ):
                ...
                def mock_package_discovery_func(project_root_folder, relative_path):
                    if (
                        ...
                        and relative_path == "dockers/xgboost_atoz"
                    ):
                        return str(mods_project_dir)
                    elif (
                        project_root_folder == "fraud_detection"
                        and relative_path == "scripts"
                    ):
                ...
                def mock_wd_discovery_func(project_root_folder, relative_path):
                    return (
                        None  # Always fails in Lambda/MODS because runtime is cut off
                    )
                ...
                resolved_mods = self.resolver.resolve_path(
                    "mods_pipeline_adapter", "dockers/xgboost_atoz"
                )
                assert resolved_mods == str(mods_project_dir)

                # Test fraud detection resolution
                resolved_fraud = self.resolver.resolve_path(
                    "fraud_detection", "scripts"
                )
                ...
                wd_resolved = self.resolver._working_directory_discovery(
                    "mods_pipeline_adapter", "dockers/xgboost_atoz"
                )
                assert (
                    wd_resolved is None
                )  # Should fail because runtime is completely separated
    ...
    def test_scenario_2_development_monorepo(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project_root = (
                Path(temp_dir) / "Users" / "tianpeixie" / "github_workspace" / "cursus"
            )
            ...
            xgboost_pda_target_file = (
                xgboost_pda_project_dir / "tabular_preprocessing.py"
            )
            ...
            with (
                patch(
                    "cursus.core.utils.hybrid_path_resolution.__file__",
                    mock_cursus_file,
                ),
                patch("pathlib.Path.cwd", return_value=demo_dir),
            ):
                # Test PyTorch project resolution (docker subdirectory)
                resolved_pytorch = self.resolver.resolve_path(
                    "project_pytorch_bsm_ext", "docker"
                )
                ...
                resolved_xgboost_atoz = self.resolver.resolve_path(
                    "project_xgboost_atoz", "."
                )
                ...
                resolved_xgboost_pda = self.resolver.resolve_path(
                    "project_xgboost_pda", "materials"
                )
                ...
                package_resolved = self.resolver._package_location_discovery(
                    "project_xgboost_pda", "materials"
                )
    ...
    def test_scenario_3_pip_installed_separated(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            site_packages = (
                Path(temp_dir) / "usr" / "local" / "lib" / "python3.x" / "site-packages"
            )
            ...
            with (
                ...
                package_resolved = self.resolver._package_location_discovery(
                    "my_project", "dockers/xgboost_atoz"
                )
                assert (
                    package_resolved is None
                )  # Should fail because user files not in system site-packages
                ...
                resolved = self.resolver.resolve_path(
                    "my_project", "dockers/xgboost_atoz"
                )
    ...
    def test_hybrid_resolution_strategy_progression(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                strategy1_result = self.resolver._package_location_discovery(
                    "my_project", "scripts"
                )
                assert strategy1_result is None

                # Verify Strategy 2 succeeds
                strategy2_result = self.resolver._working_directory_discovery(
                    "my_project", "scripts"
                )
    ...
    def test_working_directory_discovery_direct(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("pathlib.Path.cwd", return_value=Path(temp_dir)):
                ...
                resolved = self.resolver._working_directory_discovery(
...
class TestHybridResolutionConfig:
    ...
    def test_environment_variable_override(self):
        ...
        with patch.dict(os.environ, {"CURSUS_HYBRID_RESOLUTION_MODE": "fallback_only"}):
            assert (
                HybridResolutionConfig.get_hybrid_resolution_mode() == "fallback_only"
            )
...
class TestResolveHybridPath:
    ...
    def test_fallback_only_mode(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.dict(
                    os.environ, {"CURSUS_HYBRID_RESOLUTION_MODE": "fallback_only"}
                ),
...
class TestHybridResolutionMetrics:
    ...
    def test_metrics_collection(self):
        ...
        assert final_metrics["total_attempts"] > initial_metrics.get(
            "total_attempts", 0
        )
