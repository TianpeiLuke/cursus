...
class TestPipelineExecutionTempDirIntegration:
    ...
    def test_end_to_end_parameter_flow_with_custom_execution_prefix(self):
        ...
        with (
            ...
            patch(
                "cursus.steps.configs.utils.detect_config_classes_from_json"
            ) as mock_detect_classes,
        ):
            # Setup mocks
    ...
    def test_end_to_end_parameter_flow_with_assembler_integration(self):
        ...
        with patch(
            "cursus.core.assembler.pipeline_assembler.StepCatalog"
        ) as mock_catalog:
        ...
        for step_name, builder in assembler.step_builders.items():
            ...
            assert (
                builder.execution_prefix.default_value
                == "s3://assembler-test-bucket/execution"
            )
    ...
    def test_step_builder_base_output_path_resolution_with_execution_prefix(self):
        ...
        execution_param = ParameterString(
            name="EXECUTION_S3_PREFIX", default_value="s3://param-test-bucket/execution"
        )
    ...
    def test_output_generation_with_join_pattern(self):
        ...
        custom_execution_param = ParameterString(
            name="EXECUTION_S3_PREFIX", default_value="s3://join-test-bucket/execution"
        )
        ...
        for output_name, output_path in outputs.items():
            assert isinstance(output_path, Join), (
                f"Output {output_name} should use Join, got {type(output_path)}"
            )
    ...
    def test_parameter_type_compatibility(self):
        ...
        param_string = ParameterString(
            name="EXECUTION_S3_PREFIX", default_value="s3://param-string-test/execution"
        )
    ...
    def test_multiple_parameter_handling(self):
        ...
        execution_param = ParameterString(
            name="EXECUTION_S3_PREFIX", default_value="s3://multi-param-test/execution"
        )
        kms_param = ParameterString(
            name="KMS_ENCRYPTION_KEY_PARAM", default_value="test-kms-key"
        )
        security_param = ParameterString(
            name="SECURITY_GROUP_ID", default_value="sg-12345"
        )
    ...
    def test_parameter_extraction_logic(self):
        ...
        execution_param = ParameterString(
            name="EXECUTION_S3_PREFIX", default_value="s3://test/execution"
        )
    ...
    def test_logging_integration(self):
        ...
        with patch("cursus.core.base.builder_base.logger") as mock_logger:
            ...
            execution_param = ParameterString(
                name="EXECUTION_S3_PREFIX", default_value="s3://logging-test/execution"
            )
