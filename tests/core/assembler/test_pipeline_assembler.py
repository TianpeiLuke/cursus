...
from cursus.core.assembler.pipeline_assembler import (
    PipelineAssembler,
...
class MockStepBuilder(StepBuilderBase):
    ...
    def __init__(
        ...
        # Create a real StepInterface (unified replacement for StepSpecification +
        # OutputSpec/DependencySpec) to avoid mock interference. Dependencies and
        # outputs are dicts keyed by logical_name; logical_name auto-populates.
...
class TestPipelineAssembler:
    ...
    def simple_config_map(self):
        ...
        return {"step1": MockConfig(), "step2": MockConfig()}
    ...
    def test_init_missing_step_builders_raises_error(
        self, simple_dag, simple_config_map
    ):
    ...
    def test_generate_outputs_creates_join_objects(
        ...
        for output_name, output_path in outputs.items():
            assert isinstance(output_path, Join), (
                f"Output {output_name} should be Join object"
            )
    ...
    def test_init_dangling_dag_edges_construct_gracefully(
        self, simple_config_map, mock_step_catalog
    ):
        """A DAG with a dangling edge constructs without raising an opaque KeyError.

        Previously PipelineDAG.__init__ blindly indexed adjacency by edge endpoints and
        raised KeyError when an edge referenced an unknown node. That was a bug — dangling
        edges are now tolerated at construction (the unknown endpoint is not added to
        nodes) and surfaced with a clear ValueError by the serializer's validation layer.
        """
        ...
        # The dangling endpoint is not promoted to a real node.
        assert "step3" not in invalid_dag.nodes
        # Validation catches it with a clear, actionable message.
        from cursus.api.dag import PipelineDAGWriter

        with pytest.raises(ValueError, match="non-existent"):
            PipelineDAGWriter(invalid_dag)._validate_dag()

    def test_init_default_parameters_handling(
    ...
    def test_instantiate_step_error_handling(
        ...
        assembler.step_builders["step1"].create_step = Mock(
            side_effect=Exception("Step creation failed")
        )
    ...
    def test_generate_pipeline_topological_sort_error(
        ...
        with patch.object(
            problematic_dag,
            "topological_sort",
            side_effect=ValueError("Topological sort failed"),
        ):
            assembler = PipelineAssembler(
                dag=problematic_dag,
                config_map=simple_config_map,
    ...
    def test_propagate_messages_compatibility_scoring(
        ...
        mock_dependency_resolver._calculate_compatibility.return_value = (
