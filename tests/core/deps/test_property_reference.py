...
from cursus.core.base.step_interface import OutputDecl
...
class TestPropertyReference:
    ...
    def setup_method(self):
        ...
        self.processing_output_spec = OutputDecl(
        ...
        self.training_output_spec = OutputDecl(
    ...
    def test_init_and_validation(self):
        ...
        prop_ref = PropertyReference(
            step_name="processing_step", output_spec=self.processing_output_spec
        )
        ...
        with pytest.raises(ValueError):
            PropertyReference(step_name="", output_spec=self.processing_output_spec)
    ...
    def test_parse_property_path(self):
        ...
        prop_ref = PropertyReference(
            step_name="test_step", output_spec=self.processing_output_spec
        )
        ...
        path_parts = prop_ref._parse_property_path(
            "properties.TransformOutput.Outputs[0].S3OutputPath"
        )
        ...
        path_parts = prop_ref._parse_property_path(
            "properties.DescribeEvaluationJobResponse.Statistics['accuracy']"
        )
        assert len(path_parts) == 2
        ...
        assert "Config" in parts or any(
            p[0] == "Config" if isinstance(p, tuple) else False for p in parts
        )
        ...
        assert "Sub" in parts or any(
            p[0] == "Sub" if isinstance(p, tuple) else False for p in parts
        )
        ...
        assert "Value" in parts or any(
            p[0] == "Value" if isinstance(p, tuple) else False for p in parts
        )
    ...
    def test_to_sagemaker_property(self):
        ...
        prop_ref = PropertyReference(
            step_name="training_step", output_spec=self.training_output_spec
        )
        ...
        # Test metrics output
        prop_ref = PropertyReference(
            step_name="eval_step", output_spec=self.metrics_output_spec
        )
        sagemaker_prop = prop_ref.to_sagemaker_property()
        assert sagemaker_prop == {
        ...
        prop_ref = PropertyReference(
            step_name="transform_step", output_spec=self.transform_output_spec
        )
        sagemaker_prop = prop_ref.to_sagemaker_property()
        assert sagemaker_prop == {
    ...
    def test_get_property_value(self):
        ...
        test_obj.ProcessingOutputConfig.Outputs["processed_data"] = type(
            "Output", (), {}
        )()
        test_obj.ProcessingOutputConfig.Outputs["processed_data"].S3Output = type(
            "S3Output", (), {}
        )()
        test_obj.ProcessingOutputConfig.Outputs[
            "processed_data"
        ].S3Output.S3Uri = "s3://test-bucket/output"
    ...
    def test_to_runtime_property(self):
        ...
        with pytest.raises(ValueError):
            missing_prop_ref = PropertyReference(
                step_name="missing_step", output_spec=self.processing_output_spec
            )
    ...
    def _create_mock_processing_step(self):
        ...
        mock_step.properties.ProcessingOutputConfig.Outputs = {"processed_data": Mock()}
        mock_step.properties.ProcessingOutputConfig.Outputs[
            "processed_data"
        ].S3Output = Mock()
        mock_step.properties.ProcessingOutputConfig.Outputs[
            "processed_data"
        ].S3Output.S3Uri = "s3://test-bucket/processed-data"
    ...
    def _create_mock_transform_step(self):
        ...
        mock_step.properties.TransformOutput.Outputs[
            0
        ].S3OutputPath = "s3://test-bucket/predictions"
