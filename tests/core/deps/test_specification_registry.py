...
Migrated off the deleted legacy classes (StepSpecification / DependencySpec /
OutputSpec) onto the unified ``StepInterface`` model. Step specifications are now
built via the dict-based ``StepInterface`` constructor and dependency probes used
for ``find_compatible_outputs`` are ``DependencyDecl`` instances.
"""

import pytest
from .test_helpers import reset_all_global_state
from cursus.core.deps.specification_registry import SpecificationRegistry
from cursus.core.base.step_interface import StepInterface, DependencyDecl
...
class TestSpecificationRegistry:
    ...
    def setup_method(self):
        ...
        # Create test specifications (StepInterface, dict-based)
        self.data_loading_spec = StepInterface(
        ...
        self.preprocessing_spec = StepInterface(
        ...
        self.evaluation_spec = StepInterface(
    ...
    def test_get_specifications_by_type(self):
        ...
        preprocessing_specs = self.registry.get_specifications_by_type(
            "PreprocessingStep"
        )
    ...
    def test_find_compatible_outputs(self):
        ...
        # Create dependency decl to match
        dependency_spec = DependencyDecl(
    ...
    def test_compatibility_checking(self):
        ...
        incompatible_dep = DependencyDecl(
    ...
    def test_multiple_compatible_outputs(self):
        ...
        data_loading_spec1 = StepInterface(
            step_type="DataLoadingStep",
            node_type=self.node_type_source,
            contract={},
            spec={
        ...
        data_loading_spec2 = StepInterface(
            step_type="DataLoadingStep",
            node_type=self.node_type_source,
            contract={},
            spec={
        ...
        # Create dependency decl that matches both outputs
        dep_spec = DependencyDecl(
            logical_name="data_input",
            type=self.dependency_type,
            required=True,
    ...
    def test_data_type_compatibility(self):
        ...
        # Create specs with outputs of different data types
        string_spec = StepInterface(
        ...
        # Create dependency decls with different data types
        string_dep = DependencyDecl(
            logical_name="string_dep",
            type=self.dependency_type,
            data_type="String",
        )

        s3_dep = DependencyDecl(
            logical_name="s3_dep",
            type=self.dependency_type,
            data_type="S3Uri",
        )
    ...
    def test_find_compatible_outputs_detailed(self):
        ...
        # Create dependency decl to search for
        dep_spec = DependencyDecl(
