...
class TestSemanticMatcher:
    def test_calculate_similarity_with_aliases(self):
        ...
        output_spec = iface.outputs["processed_data"]
        ...
        test_cases = [
            ...
            (
                ...
                0.1,
                "processed_data",
            ),  # No good match (defaults to logical)
        ]
        ...
        for dep_name, expected_min_score, expected_best_match in test_cases:
            ...
            assert score >= expected_min_score, (
                f"Score for '{dep_name}' below expectation"
            )
    ...
    def test_name_normalization(self):
        ...
        test_cases = [
            ...
            (
                "TrainingData",
                "trainingdata",
            ),  # No spaces in camelCase after normalization
        ...
        for input_name, expected_output in test_cases:
            ...
            assert normalized == expected_output, f"Failed to normalize '{input_name}'"
    ...
    def test_individual_similarity_metrics(self):
        ...
        string_sim = matcher._calculate_string_similarity(
            "training data", "training dataset"
        )
        assert string_sim >= 0.85
        assert string_sim <= 0.95  # Allow some flexibility in the implementation

        # Token similarity
        assert (
            matcher._calculate_token_similarity("training data", "data training") == 1.0
        )  # Same tokens, different order = perfect match

        token_sim = matcher._calculate_token_similarity(
            "training data", "training model"
        )
        assert abs(token_sim - 0.33) < 0.01  # 1 common token out of 3 unique tokens

        # Semantic similarity
        semantic_sim = matcher._calculate_semantic_similarity(
            "training data", "train dataset"
        )
        assert semantic_sim >= 0.25  # Should have some matches due to synonyms
        assert semantic_sim <= 0.8  # But not a perfect match

        # Substring similarity
        substring_sim = matcher._calculate_substring_similarity(
            "training", "training data"
        )
        assert substring_sim >= 0.57  # At least 8/14 characters
        assert substring_sim <= 0.65  # Allow for implementation variations
    ...
    def test_synonym_matching(self):
        ...
        # Test synonym pairs
        assert matcher._are_synonyms("model", "artifact")
        assert matcher._are_synonyms("data", "dataset")
        assert matcher._are_synonyms("config", "parameters")

        # Test non-synonym pairs
        assert not matcher._are_synonyms("model", "data")
        assert not matcher._are_synonyms("training", "evaluation")
    ...
    def test_find_best_matches(self):
        ...
        assert len(matches) >= 1  # Should match at least "training_dataset"
        assert matches[0][0] == "training_dataset"  # Best match should be this

        # Search with low threshold
        matches_low_threshold = matcher.find_best_matches(
            "training_data", candidates, threshold=0.3
        )
        assert len(matches_low_threshold) > len(matches)  # Should match more items

        # Search with no matches
        no_matches = matcher.find_best_matches(
            "completely_different", candidates, threshold=0.7
        )
        assert len(no_matches) == 0  # Should find no matches
    ...
    def test_similarity_explanation(self):
        ...
        assert "normalized_names" in explanation
        assert "string_similarity" in explanation
        assert "token_similarity" in explanation
        assert "semantic_similarity" in explanation
        assert "substring_similarity" in explanation

        # Check explanation values
        assert explanation["overall_score"] > 0.7
        assert explanation["normalized_names"][0] == "training data"
    ...
    def test_edge_cases(self):
        ...
        # Empty strings
        assert matcher.calculate_similarity("", "data") == 0.0
        assert matcher.calculate_similarity("data", "") == 0.0
        assert matcher.calculate_similarity("", "") == 0.0

        # Identical strings
        assert matcher.calculate_similarity("data", "data") == 1.0

        # Names with only stop words
        assert matcher.calculate_similarity("the and of", "data") < 0.3

        # Special characters
        assert matcher.calculate_similarity("data-set!", "dataset") > 0.4
    ...
    def test_integration_with_dependency_resolver(self):
        ...
        consumer_spec = StepInterface(
            step_type="ModelStep",
            node_type=NodeType.SINK,
            contract={},
            spec={
                "dependencies": {
                    # Not exact match, but semantically similar
                    "training_dataset": {
                        "type": DependencyType.PROCESSING_OUTPUT,
                        "required": True,
                        "data_type": "S3Uri",
        ...
        assert "training_dataset" in resolved
        assert resolved["training_dataset"].step_name == "data_step"
    ...
    def test_direct_name_matching(self):
        ...
        preprocessing_spec = StepInterface(
        ...
        evaluation_spec = StepInterface(
            step_type="EvaluationStep",
            node_type=NodeType.SINK,
            contract={},
            spec={
                "dependencies": {
                    # Direct match with preprocessing output
                    "processed_data": {
                        "type": DependencyType.PROCESSING_OUTPUT,
                        "required": True,
                        "data_type": "S3Uri",
        ...
        resolved = resolver.resolve_step_dependencies(
            "evaluation", ["preprocessing", "training"]
        )
        ...
        assert "processed_data" in resolved
        assert resolved["processed_data"].step_name == "preprocessing"
        assert resolved["processed_data"].output_spec.logical_name == "processed_data"
    ...
    def test_domain_specific_synonyms(self):
        ...
        high_similarity_pairs = [("training", "train"), ("preprocessing", "preprocess")]
        ...
        low_similarity_pairs = [
            (
                "model",
                "artifact",
            ),  # These are synonyms in context but not lexically similar
