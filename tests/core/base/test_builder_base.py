...
class TestStepBuilderBase:
    ...
    def test_set_execution_prefix(self, config):
        ...
        param = ParameterString(
            name="EXECUTION_S3_PREFIX", default_value="s3://test-bucket/execution"
        )
...
class TestStepBuilderBaseAdvanced:
    ...
    def test_init_with_spec_contract_alignment_validation_success(
        self, config, mock_spec_with_contract
    ):
    ...
    def test_init_with_spec_contract_alignment_validation_failure(self, config):
        ...
        mock_spec.validate_contract_alignment = Mock()
        mock_spec.validate_contract_alignment.return_value = Mock(
            is_valid=False, errors=["Contract mismatch", "Missing required field"]
        )
        mock_spec.script_contract = Mock()
    ...
    def test_init_contract_from_spec_priority(self, config):
        ...
        with patch.object(
            config, "get_script_contract", return_value=Mock(name="config_contract")
        ):
            builder = ConcreteStepBuilder(config=config, spec=mock_spec)
    ...
    def test_step_names_property_fallback_to_traditional_registry(self, config):
        ...
        with patch(
            "cursus.registry.hybrid.manager.HybridRegistryManager",
            side_effect=ImportError("No hybrid registry"),
        ):
            step_names = builder.STEP_NAMES
    ...
    def test_step_names_property_final_fallback_empty_dict(self, config):
        ...
        with patch(
            "cursus.registry.hybrid.manager.HybridRegistryManager",
            side_effect=ImportError("No hybrid"),
        ):
            with patch(
                "cursus.registry.step_names.BUILDER_STEP_NAMES", {}
            ):  # Return empty dict instead of raising ImportError
    ...
    def test_get_step_name_unknown_step_type_warning(self, config):
        ...
        with patch.object(
            type(builder), "STEP_NAMES", new_callable=lambda: property(lambda self: {})
        ):
            with patch.object(builder, "log_warning") as mock_log_warning:
                step_name = builder._get_step_name()
    ...
    def test_get_property_path_format_error_handling(self, config):
        ...
        mock_output_spec = Mock()
        mock_output_spec.logical_name = "test_output"
        mock_output_spec.property_path = (
            "Steps.{missing_key}.Properties.{output_descriptor}"
        )
        mock_spec.outputs = {"output1": mock_output_spec}
        ...
        with patch("cursus.core.base.builder_base.logger") as mock_logger:
            ...
            path = builder.get_property_path(
                "test_output", {"output_descriptor": "data"}
            )
    ...
    def test_get_property_path_general_format_error_handling(self, config):
        ...
        mock_output_spec.property_path = (
            "Steps.{invalid_format"  # Missing closing brace
        )
        mock_spec.outputs = {"output1": mock_output_spec}
        ...
        with patch("cursus.core.base.builder_base.logger") as mock_logger:
            ...
            mock_logger.warning.assert_called_once()
            assert (
                "Error formatting property path" in mock_logger.warning.call_args[0][0]
            )
    ...
    def test_safe_logging_methods_with_exception_handling(self, config):
        ...
        with patch(
            "cursus.core.base.builder_base.safe_value_for_logging",
    ...
    def test_get_environment_variables_missing_required_warning(self, config):
        ...
        with patch.object(builder, "log_warning") as mock_log_warning:
            ...
            assert (
                "Required environment variable 'MISSING_VAR' not found"
    ...
    def test_get_environment_variables_optional_with_defaults_debug_log(self, config):
        ...
        with patch.object(builder, "log_debug") as mock_log_debug:
            ...
            assert (
                "Using default value for optional environment variable"
    ...
    def test_get_base_output_path_logs_execution_prefix_usage(self, config):
        ...
        with patch.object(builder, "log_info") as mock_log_info:
            ...
            mock_log_info.assert_called_once()
            assert (
                "Using execution_prefix for base output path"
    ...
    def test_get_base_output_path_logs_config_fallback(self, config):
        ...
        with patch.object(builder, "log_debug") as mock_log_debug:
            ...
            mock_log_debug.assert_called_once()
            assert (
                "No execution_prefix set, using config.pipeline_s3_loc"
                in mock_log_debug.call_args[0][0]
            )
    ...
    def test_get_required_dependencies_no_spec_raises_error(self, config):
        ...
        with pytest.raises(
            ValueError,
            match="Step specification is required for dependency information",
        ):
    ...
    def test_get_outputs_no_spec_raises_error(self, config):
        ...
        with pytest.raises(
            ValueError, match="Step specification is required for output information"
        ):
            builder.get_outputs()
    ...
    def test_get_dependency_resolver_creates_new_when_none(self, config):
        ...
        with patch.object(builder, "log_debug") as mock_log_debug:
            with patch(
                "cursus.core.base.builder_base.create_dependency_resolver"
            ) as mock_create_resolver:
                with patch(
                    "cursus.core.base.builder_base.SemanticMatcher"
                ) as mock_semantic_matcher:
                    mock_resolver = Mock()
                    ...
                    assert any(
                        "Created new dependency resolver" in msg for msg in debug_calls
                    )

    def test_extract_inputs_from_dependencies_no_dependency_resolver_available(
        self, config
    ):
        ...
        with patch(
            "cursus.core.base.builder_base.DEPENDENCY_RESOLVER_AVAILABLE", False
        ):
            with pytest.raises(
                ValueError, match="UnifiedDependencyResolver not available"
            ):
    ...
    def test_extract_inputs_from_dependencies_no_spec_raises_error(self, config):
        ...
        with pytest.raises(
            ValueError, match="Step specification is required for dependency extraction"
        ):
            builder.extract_inputs_from_dependencies([])
    ...
    def test_extract_inputs_from_dependencies_success(self, config):
        ...
        with patch.object(
            builder, "_enhance_dependency_steps_with_specs"
        ) as mock_enhance:
    ...
    def test_enhance_dependency_steps_with_specs_with_existing_spec(self, config):
        ...
        builder._enhance_dependency_steps_with_specs(
            mock_resolver, [mock_step], available_steps
        )

        # Should register the existing specification
        mock_resolver.register_specification.assert_called_once_with(
            "test_step", mock_step._spec
        )
    ...
    def test_enhance_dependency_steps_with_specs_processing_outputs_dict(self, config):
        ...
        mock_outputs.items = Mock(
            return_value=[("output1", mock_output1), ("output2", mock_output2)]
        )
