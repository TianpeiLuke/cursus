...
def test_step_name_derivation():
    ...
    for config_class_name, expected_step_name in test_cases.items():
        ...
        try:
            ...
            if derived_step_name == expected_step_name:
                ...
                print(
                    f"❌ Step name derivation: FAIL (got '{derived_step_name}', expected '{expected_step_name}')"
                )
    ...
    assert success_count >= total_count * 0.8, (
        f"Too many failures: {success_count}/{total_count} working"
    )
...
def test_step_catalog_contract_discovery():
    ...
    assert success_count >= total_count * 0.5, (
        f"Too many contract discovery failures: {success_count}/{total_count} working"
    )
...
def test_registry_integration():
    ...
    try:
        ...
        if found_steps == 0:
            ...
            assert found_steps >= len(expected_step_names) * 0.5, (
                f"Too few expected steps found: {found_steps}/{len(expected_step_names)}"
            )
