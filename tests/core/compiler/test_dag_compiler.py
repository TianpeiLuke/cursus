...
class TestDagCompiler:
    ...
    def test_compile_with_custom_pipeline_name(
        self, sample_dag, temp_config_file, mock_session, mock_role
    ):
        """
        ...
        with (
            patch("cursus.core.compiler.dag_compiler.Path") as mock_path,
            patch(
                "cursus.core.compiler.dag_compiler.StepCatalog"
            ) as mock_catalog_class,
            patch(
                "cursus.core.compiler.dag_compiler.StepConfigResolver"
            ) as mock_resolver_class,
            patch(
                "cursus.core.compiler.dag_compiler.ValidationEngine"
...
class TestCompileDagToPipeline:
    ...
    def test_compile_dag_to_pipeline_invalid_dag(
        self, temp_config_file, mock_session, mock_role
    ):
        """
    ...
    def test_compile_dag_to_pipeline_empty_dag(
        self, temp_config_file, mock_session, mock_role
    ):
        """
    ...
    def test_compile_dag_to_pipeline_missing_config_file(
        self, simple_dag, mock_session, mock_role
    ):
        """
    ...
    def test_compile_dag_to_pipeline_success(
        self, simple_dag, temp_config_file, mock_session, mock_role
    ):
        """
...
class TestPipelineDAGCompilerInit:
    ...
    def test_compiler_init_default_parameters(self, temp_config_file):
        ...
        with patch("cursus.core.compiler.dag_compiler.Path") as mock_path:
            ...
            param_names = [
                p.name for p in compiler.pipeline_parameters if hasattr(p, "name")
            ]
...
class TestPipelineDAGCompilerValidation:
    ...
    def test_preview_resolution_success(self, mock_resolver, mock_catalog, mock_path):
        ...
        mock_catalog_instance._config_class_to_step_type.side_effect = lambda x: (
            x.replace("Config", "")
        )
...
class TestPipelineDAGCompilerUtilityMethods:
    ...
    @patch("cursus.core.compiler.dag_compiler.StepCatalog")
    @patch("cursus.core.compiler.dag_compiler.StepConfigResolver")
    def test_validate_config_file_success(self, mock_resolver, mock_catalog, mock_path):
    ...
    @patch("cursus.core.compiler.dag_compiler.StepCatalog")
    @patch("cursus.core.compiler.dag_compiler.StepConfigResolver")
    def test_validate_config_file_failure(self, mock_resolver, mock_catalog, mock_path):
    ...
    def test_compiler_init_with_pipeline_parameters(self):
        ...
        custom_params = [
            ParameterString(
                name="EXECUTION_S3_PREFIX", default_value="s3://custom-bucket/execution"
            ),
            ParameterString(
                name="KMS_ENCRYPTION_KEY_PARAM", default_value="custom-key"
            ),
        ]
