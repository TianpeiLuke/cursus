...
class TestDualSearchSpaceArchitecture:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for workspace in [workspace1, workspace2]:
                ...
                for project in ["alpha", "beta"]:
                    project_dir = (
                        projects_dir / project / "src" / "cursus_dev" / "steps"
                    )
    ...
    def test_multiple_workspace_directories(
        self, temp_package_root, temp_workspace_dirs
    ):
    ...
    def test_separation_of_concerns_enforcement(
        self, temp_package_root, temp_workspace_dirs
    ):
        ...
        with patch.object(StepCatalog, "_find_package_root", return_value=package_root):
            ...
            assert (
                catalog._discover_package_components is not None
            )  # System discovers its own components

            # User responsibilities (explicit)
            assert (
                catalog.workspace_dirs == workspace_dirs
            )  # User must provide workspace dirs
            assert (
                catalog._discover_workspace_components is not None
            )  # System uses user-provided dirs
...
class TestConfigAutoDiscoveryDualSpace:
    ...
    def test_package_discovery_only(self, temp_dual_structure):
        ...
        package_root, workspace_root, package_configs, workspace_configs = (
            temp_dual_structure
        )
...
""")
    ...
    def test_workspace_discovery_with_package(self, temp_dual_structure):
        ...
        with patch.object(discovery, "_scan_config_directory") as mock_scan:
            ...
            with patch.object(
                discovery, "_discover_workspace_configs"
            ) as mock_workspace:
    ...
    def test_hyperparameter_discovery_dual_space(self, temp_dual_structure):
        ...
        with patch.object(discovery, "_scan_hyperparams_directory") as mock_scan:
            ...
            with patch.object(
                discovery, "_discover_workspace_hyperparams"
            ) as mock_workspace:
...
class TestSearchSpaceIntegration:
    ...
    def test_step_catalog_config_discovery_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                StepCatalog, "_find_package_root", return_value=Path(".")
            ):
                catalog = StepCatalog(workspace_dirs=workspace_dir)

                # Test config class discovery delegation
                with patch.object(
                    catalog.config_discovery, "discover_config_classes"
                ) as mock_discover:
                ...
                with patch.object(
                    catalog.config_discovery, "build_complete_config_classes"
                ) as mock_build:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    try:
                        ...
                        assert False, (
                            "Index building should handle workspace errors gracefully"
                        )

                # Test graceful handling of config discovery errors
                with patch.object(
                    catalog.config_discovery,
...
class TestDeploymentScenarios:
    ...
    def test_pypi_deployment_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                StepCatalog, "_find_package_root", return_value=cursus_package
            ):
    ...
    def test_source_deployment_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                StepCatalog, "_find_package_root", return_value=src_cursus
            ):
    ...
    def test_submodule_deployment_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                StepCatalog, "_find_package_root", return_value=cursus_submodule
            ):
