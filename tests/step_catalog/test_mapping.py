...
class TestStepCatalogMapper:
    ...
    def test_get_builder_for_config_with_valid_config(self, mapping_instance):
        ...
        with patch.object(
            mapping_instance.step_catalog, "load_builder_class"
        ) as mock_load:
            mock_builder = Mock()
    ...
    def test_get_builder_for_config_with_none(self, mapping_instance):
        ...
        with patch.object(
            mapping_instance.step_catalog, "load_builder_class", return_value=None
        ):
    ...
    def test_fallback_config_to_step_type(self, mapping_instance):
        ...
        assert (
            mapping_instance._fallback_config_to_step_type("BatchTransformConfig")
            == "BatchTransform"
        )
        assert (
            mapping_instance._fallback_config_to_step_type("XGBoostTrainingConfig")
            == "XGBoostTraining"
        )
        assert (
            mapping_instance._fallback_config_to_step_type("PyTorchModelConfig")
            == "PyTorchModel"
        )
        ...
        assert (
            mapping_instance._fallback_config_to_step_type("SomeStepConfig") == "Some"
        )  # Removes both "Step" and "Config"
        assert (
            mapping_instance._fallback_config_to_step_type("NoConfigSuffix")
            == "NoConfigSuffix"
        )
    ...
    def test_legacy_alias_resolution(self, mapping_instance):
        ...
        assert (
            mapping_instance.resolve_legacy_aliases("ModelRegistration")
            == "Registration"
        )
        assert (
            mapping_instance.resolve_legacy_aliases("PytorchTraining")
            == "PyTorchTraining"
        )
        ...
        assert (
            mapping_instance.resolve_legacy_aliases("BatchTransform")
            == "BatchTransform"
        )
    ...
    def test_is_step_type_supported(self, mapping_instance):
        ...
        mapping_instance.step_catalog._step_index = {
            "BatchTransform": Mock(),
        ...
        # Test legacy aliases
        assert (
            mapping_instance.is_step_type_supported("MIMSPackaging") == False
        )  # Package not in mock index
    ...
    def test_validate_builder_availability(self, mapping_instance):
        ...
        with patch.object(
            mapping_instance, "get_builder_for_step_type", side_effect=mock_get_builder
        ):
...
class TestPipelineConstructionInterface:
    ...
    def test_get_builder_map(self, pipeline_interface):
        ...
        with patch.object(
            pipeline_interface.mapper,
            "list_supported_step_types",
            return_value=["BatchTransform", "XGBoostTraining"],
        ):
            with patch.object(
                pipeline_interface.mapper, "get_builder_for_step_type"
            ) as mock_get_builder:
                mock_builder = Mock()
...
class TestIntegrationScenarios:
    ...
    def test_full_config_to_builder_resolution_flow(self):
        ...
        with patch.object(
            mapping.step_catalog, "load_builder_class", return_value=mock_builder
        ):
    ...
    def test_legacy_alias_resolution_flow(self):
        ...
        with patch.object(
            ...
            result = mapping.get_builder_for_step_type(
                "MIMSPackaging"
            )  # Uses existing legacy alias
    ...
    def test_mapping_with_malformed_config_names(self):
        ...
        test_cases = [
            ...
            (
                "StepConfig",
                "",
            ),  # Removes both "Step" and "Config", leaving empty string
            (
                "VeryLongConfigNameThatShouldStillWork",
                "VeryLongConfigNameThatShouldStillWork",
            ),
        ]
...
class TestPerformanceCharacteristics:
    ...
    def test_large_scale_step_type_resolution(self):
        ...
        with patch.object(
            mapping.step_catalog, "load_builder_class", return_value=None
        ):
