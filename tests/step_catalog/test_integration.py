...
class TestEndToEndWorkflow:
    ...
    def realistic_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create sample files
            (scripts_dir / "data_preprocessing.py").write_text(
                "# Data preprocessing script"
            )
            (scripts_dir / "model_training.py").write_text("# Model training script")
            (contracts_dir / "data_preprocessing_contract.py").write_text(
...
""")
            ...
            (workspace_steps / "custom_preprocessing.py").write_text(
    ...
    def test_complete_discovery_workflow(self, realistic_workspace):
        ...
        with patch.object(
            StepCatalog,
            "_find_package_root",
            return_value=realistic_workspace / "src" / "cursus",
        ):
