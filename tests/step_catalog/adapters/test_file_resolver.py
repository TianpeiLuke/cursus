...
class TestFlexibleFileResolverAdapter:
    ...
    def test_init(self, temp_workspace):
        ...
        with patch(
            "cursus.step_catalog.adapters.file_resolver.StepCatalog"
        ) as mock_catalog:
    ...
    def test_extract_base_name(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            assert (
                adapter._extract_base_name("XGBoostTraining", "script")
                == "xgboost_training"
            )
            assert (
                adapter._extract_base_name("DataLoading", "contract") == "data_loading"
            )
    ...
    def test_extract_base_name_from_spec(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            assert (
                adapter.extract_base_name_from_spec(Path("test_step_spec.py"))
                == "test_step"
            )
            assert (
                adapter.extract_base_name_from_spec(Path("xgboost_training_spec.py"))
                == "xgboost_training"
            )
            assert (
                adapter.extract_base_name_from_spec(Path("simple_spec.py")) == "simple"
            )
            assert (
                adapter.extract_base_name_from_spec(Path("no_spec_suffix.py"))
                == "no_spec_suffix"
            )
    ...
    def test_find_spec_constant_name(self, temp_workspace):
        ...
        with patch(
            ...
            with patch.object(
                adapter, "find_spec_file", return_value="/path/to/test_step_spec.py"
            ):
...
class TestDeveloperWorkspaceFileResolverAdapter:
    ...
    def test_init_with_workspace_root(self, temp_workspace):
        ...
        with patch(
            ...
            adapter = DeveloperWorkspaceFileResolverAdapter(
                workspace_root=temp_workspace, developer_id="test_dev"
            )
    ...
    def test_init_with_project_id_alias(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            adapter = DeveloperWorkspaceFileResolverAdapter(
                workspace_root=temp_workspace, project_id="test_project"
            )
    ...
    def test_validate_workspace_structure_invalid_root(self):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            with pytest.raises(ValueError, match="Workspace root does not exist"):
                DeveloperWorkspaceFileResolverAdapter(
                    workspace_root=Path("/nonexistent/path"), developer_id="test_dev"
                )
    ...
    def test_validate_workspace_structure_invalid_developer(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            with pytest.raises(ValueError, match="Developer workspace does not exist"):
                DeveloperWorkspaceFileResolverAdapter(
                    workspace_root=temp_workspace, developer_id="nonexistent_dev"
                )
    ...
    def test_setup_workspace_paths(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            assert (
                adapter.contracts_dir
                == temp_workspace / "developers" / "test_dev" / "contracts"
            )
            assert (
                adapter.specs_dir
                == temp_workspace / "developers" / "test_dev" / "specs"
            )
            assert (
                adapter.builders_dir
                == temp_workspace / "developers" / "test_dev" / "builders"
            )
            assert (
                adapter.scripts_dir
                == temp_workspace / "developers" / "test_dev" / "scripts"
            )
            assert (
                adapter.configs_dir
                == temp_workspace / "developers" / "test_dev" / "configs"
            )

            # Check shared workspace paths
            assert (
                adapter.shared_contracts_dir == temp_workspace / "shared" / "contracts"
            )
    ...
    def test_find_workspace_file_shared_fallback(self, temp_workspace):
        ...
        shared_contract = (
            temp_workspace / "shared" / "contracts" / "test_step_contract.py"
        )
    ...
    def test_find_file_in_directory_contract_patterns(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            result = adapter._find_file_in_directory(
                str(contracts_dir), "test_step", None, [".py"]
            )
    ...
    def test_find_file_in_directory_spec_patterns(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            result = adapter._find_file_in_directory(
                str(specs_dir), "test_step", None, [".py"]
            )
    ...
    def test_find_file_in_directory_builder_patterns(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            result = adapter._find_file_in_directory(
                str(builders_dir), "test_step", None, [".py"]
            )
    ...
    def test_find_file_in_directory_not_found(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            result = adapter._find_file_in_directory(
                str(contracts_dir), "nonexistent_step", None, [".py"]
            )
    ...
    def test_find_file_in_directory_nonexistent_dir(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            result = adapter._find_file_in_directory(
                "/nonexistent/dir", "test_step", None, [".py"]
            )
    ...
    def test_switch_developer_single_mode(self):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            with pytest.raises(
                ValueError, match="Cannot switch developer in single workspace mode"
            ):
                adapter.switch_developer("new_dev")
...
class TestIntegrationScenarios:
    ...
    def temp_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            contracts_dir.mkdir()
            (contracts_dir / "xgboost_training_contract.py").write_text(
                "# XGBoost training contract"
            )

            specs_dir = dev_workspace / "specs"
            specs_dir.mkdir()
            (specs_dir / "xgboost_training_spec.py").write_text(
                "# XGBoost training spec"
            )

            builders_dir = dev_workspace / "builders"
            builders_dir.mkdir()
            (builders_dir / "builder_xgboost_training.py").write_text(
                "# XGBoost training builder"
            )

            scripts_dir = dev_workspace / "scripts"
            scripts_dir.mkdir()
            (scripts_dir / "xgboost_training.py").write_text(
            ...
            (shared_contracts / "data_loading_contract.py").write_text(
                "# Shared data loading contract"
            )
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_name_normalization_edge_cases(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.file_resolver.StepCatalog"):
            ...
            assert (
                adapter._normalize_name("test-name.with_eval")
