...
class TestStepConfigResolverAdapter:
    ...
    def test_init_with_workspace_and_threshold(self, temp_workspace):
        ...
        with patch(
            ...
            adapter = StepConfigResolverAdapter(
                workspace_root=temp_workspace, confidence_threshold=0.8
            )
    ...
    def test_constants(self):
        ...
        assert any(
            ".*train.*" in pattern
            for pattern in StepConfigResolverAdapter.STEP_TYPE_PATTERNS.keys()
        )
    ...
    def test_direct_name_matching_with_job_type(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            adapter = StepConfigResolverAdapter()
            adapter._metadata_mapping = {
                "custom_node_training": "XGBoostTrainingConfig"
            }
    ...
    def test_calculate_config_type_confidence(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            confidence = adapter._calculate_config_type_confidence(
                "xgboost_training", mock_configs["training_config"]
            )
    ...
    def test_semantic_matching(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            assert (
                len(matches) >= 0
            )  # May or may not find matches depending on config names
    ...
    def test_config_class_to_step_type_special_cases(self, mock_configs):
        ...
        with patch(
            ...
            assert (
                adapter._config_class_to_step_type("CradleDataLoadingConfig")
                == "CradleDataLoading"
            )
            assert (
                adapter._config_class_to_step_type("PackageStepConfig")
                == "MIMSPackaging"
            )
            assert (
                adapter._config_class_to_step_type("PackageConfig") == "MIMSPackaging"
            )
    ...
    def test_calculate_job_type_boost(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            boost = adapter._calculate_job_type_boost(
                "training_step", mock_configs["training_config"]
            )
    ...
    def test_calculate_job_type_boost_no_job_type(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            boost = adapter._calculate_job_type_boost(
                "training_step", config_without_job_type
            )
    ...
    def test_resolve_single_node_direct_match(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            config, confidence, method = adapter._resolve_single_node(
                "training_config", mock_configs
            )
            ...
            with patch.object(adapter, "_job_type_matching", return_value=[]):
                with patch.object(adapter, "_semantic_matching", return_value=[]):
                    with patch.object(adapter, "_pattern_matching", return_value=[]):
                        ...
                        ):  # Should raise ResolutionError or ValueError
    ...
    def test_resolve_single_node_best_match(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            with patch.object(
                adapter,
                "_job_type_matching",
                return_value=[(mock_configs["training_config"], 0.8, "job_type")],
            ):
                with patch.object(
                    adapter,
                    "_semantic_matching",
                    return_value=[(mock_configs["eval_config"], 0.6, "semantic")],
                ):
                    with patch.object(
                        adapter,
                        "_pattern_matching",
                        return_value=[(mock_configs["data_config"], 0.7, "pattern")],
                    ):
                        config, confidence, method = adapter._resolve_single_node(
                            "test_node", mock_configs
                        )
    ...
    def test_resolve_config_for_step_fallback(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            with patch.object(
                ...
                return_value=(mock_configs["training_config"], 0.8, "test"),
            ):
    ...
    def test_resolve_config_for_step_error(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            with patch.object(
                adapter, "_direct_name_matching", side_effect=Exception("Test error")
            ):
    ...
    def test_preview_resolution_error_handling(self, mock_configs):
        ...
        with patch(
            ...
            with patch.object(
                adapter, "_resolve_single_node", side_effect=Exception("Catalog error")
            ):
    ...
    def test_parse_node_name_job_first_pattern(self):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            assert result["job_type"] == "training"
            assert (
                result.get("config_type") == "CradleDataLoading"
            )  # Should be inferred from task_map
    ...
    def test_job_type_matching_enhanced_with_config_type(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            matches = adapter._job_type_matching_enhanced(
                "training", mock_configs, "XGBoostTrainingConfig"
            )
            ...
            assert (
                confidence >= 0.9
            )  # Should have higher confidence due to exact config type match
    ...
    def test_job_type_matching_enhanced_no_match(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            matches = adapter._job_type_matching_enhanced(
                "nonexistent_job_type", mock_configs
            )
...
class TestIntegrationScenarios:
    ...
    def test_complete_dag_resolution_workflow(self, realistic_configs):
        ...
        with patch(
            ...
            mock_catalog = Mock()
            mock_catalog.get_step_info.return_value = (
                None  # Force fallback to other methods
            )
            mock_catalog_class.return_value = mock_catalog
    ...
    def test_metadata_driven_resolution(self, realistic_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            assert (
                result["metadata_mapping"]["custom_training_step"]
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_malformed_metadata(self, mock_configs):
        ...
        with patch("cursus.step_catalog.adapters.config_resolver.StepCatalog"):
            ...
            # Should handle gracefully
            result = adapter.preview_resolution(
                ["test_node"], mock_configs, malformed_metadata
            )
