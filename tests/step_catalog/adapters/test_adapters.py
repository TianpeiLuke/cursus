...
class TestContractDiscoveryEngineAdapter:
    ...
    def test_discover_all_contracts(self, mock_workspace_root, mock_step_info):
        ...
        adapter.catalog.list_available_steps = Mock(
            return_value=["test_step", "other_step"]
        )
        adapter.catalog.get_step_info = Mock(
            side_effect=lambda name: mock_step_info if name == "test_step" else None
        )
    ...
    def test_discover_contracts_with_scripts(self, mock_workspace_root, mock_step_info):
        ...
        adapter.catalog.discover_contracts_with_scripts = Mock(
            return_value=["test_step"]
        )
    ...
    def test_extract_contract_reference_from_spec(
        self, mock_workspace_root, mock_step_info
    ):
    ...
    def test_build_entry_point_mapping(self, mock_workspace_root, mock_step_info):
        ...
        assert mapping["test_step"] == str(
            mock_step_info.file_components["script"].path
        )
...
class TestContractDiscoveryManagerAdapter:
    ...
    def test_discover_contract(self, mock_workspace_root, mock_step_info):
        ...
        adapter.catalog.get_step_info = Mock(return_value=mock_step_info)
        adapter.catalog.load_contract_class = Mock(
            return_value=None
        )  # No actual contract loaded in test
        ...
        mock_contract = type(
            ...
            (),
            {
                "expected_input_paths": {
                    "input1": "/opt/ml/input/data",
                    "input2": "/opt/ml/input/config",
                }
            },
        )()
    ...
    def test_adapt_path_for_local_testing(self, mock_workspace_root):
        ...
        result = adapter._adapt_path_for_local_testing(
            "/opt/ml/input/data", Path("/test/base"), "input"
        )
...
class TestDeveloperWorkspaceFileResolverAdapter:
    ...
    @patch(
        "cursus.step_catalog.adapters.file_resolver.DeveloperWorkspaceFileResolverAdapter._validate_workspace_structure"
    )
    def test_workspace_aware_contract_discovery(
        self, mock_validate, mock_workspace_root, mock_step_info
    ):
        ...
        adapter = DeveloperWorkspaceFileResolverAdapter(
            mock_workspace_root, "project_alpha"
        )
...
class TestWorkspaceDiscoveryManagerAdapter:
    ...
    def test_enhanced_discover_components_functionality(
        self, mock_workspace_root, mock_step_info
    ):
