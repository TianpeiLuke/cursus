...
class TestWorkspaceDiscoveryManagerAdapter:
    ...
    def temp_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (dev_workspace / "contracts" / "test_step_contract.py").write_text(
                "# Test contract"
            )
            (dev_workspace / "specs" / "test_step_spec.py").write_text("# Test spec")
            (dev_workspace / "builders" / "builder_test_step.py").write_text(
                "# Test builder"
            )
            ...
            (shared_workspace / "contracts" / "shared_contract.py").write_text(
                "# Shared contract"
            )
    ...
    def test_init(self, temp_workspace):
        ...
        with patch(
            "cursus.step_catalog.adapters.workspace_discovery.StepCatalog"
        ) as mock_catalog:
    ...
    def test_discover_filesystem_components_core_workspace(self, temp_workspace):
        ...
        with patch(
            ...
            (core_workspace / "contracts" / "core_contract.py").write_text(
                "# Core contract"
            )
    ...
    def test_scan_component_directory(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            adapter._scan_component_directory(
                contracts_dir, "contracts", "dev1", inventory
            )
    ...
    def test_scan_component_directory_no_duplicates(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            contracts_dir = temp_workspace / "dev1" / "contracts"
            inventory = {
                "contracts": {"dev1": {"dev1:test_step": {"existing": "component"}}},
    ...
    def test_scan_component_directory_error_handling(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            adapter._scan_component_directory(
                nonexistent_dir, "contracts", "dev1", inventory
            )
    ...
    def test_extract_step_name_from_file(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            assert (
                adapter._extract_step_name_from_file(
                    "test_step_contract.py", "contracts"
                )
                == "test_step"
            )
            assert (
                adapter._extract_step_name_from_file("test_step_spec.py", "specs")
                == "test_step"
            )
            assert (
                adapter._extract_step_name_from_file(
                    "builder_test_step_step.py", "builders"
                )
                == "test_step"
            )
            assert (
                adapter._extract_step_name_from_file(
                    "config_test_step_step.py", "configs"
                )
                == "test_step"
            )
            assert (
                adapter._extract_step_name_from_file("test_script.py", "scripts")
                == "test_script"
            )

            # Test invalid patterns
            assert (
                adapter._extract_step_name_from_file("invalid.py", "contracts") is None
            )
    ...
    def test_get_file_resolver(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            with patch(
                "cursus.step_catalog.adapters.file_resolver.DeveloperWorkspaceFileResolverAdapter"
            ) as mock_resolver:
                adapter = WorkspaceDiscoveryManagerAdapter(temp_workspace)
    ...
    def test_get_statistics_error_handling(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            with patch.object(
                adapter, "get_discovery_summary", side_effect=Exception("Test error")
            ):
                stats = adapter.get_statistics()
    ...
    def test_is_cache_valid(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            adapter._cache_timestamp["expired"] = (
                time.time() - 400
            )  # Older than cache_expiry (300)
...
class TestIntegrationScenarios:
            ...
            for dev_id in ["dev1", "dev2"]:
                ...
                for component_type in [
                    ...
                    if component_type == "contracts":
                        (component_dir / f"{dev_id}_step_contract.py").write_text(
                            f"# {dev_id} contract"
                        )
                    elif component_type == "specs":
                        (component_dir / f"{dev_id}_step_spec.py").write_text(
                            f"# {dev_id} spec"
                        )
                    elif component_type == "builders":
                        (component_dir / f"builder_{dev_id}_step_step.py").write_text(
                            f"# {dev_id} builder"
                        )
                    elif component_type == "scripts":
                        (component_dir / f"{dev_id}_script.py").write_text(
                            f"# {dev_id} script"
                        )
                    elif component_type == "configs":
                        (component_dir / f"config_{dev_id}_step_step.py").write_text(
                            f"# {dev_id} config"
                        )
    ...
    def test_complete_workspace_discovery_workflow(self, comprehensive_workspace):
        ...
        with patch(
            ...
            mock_step_info1.file_components = {
                "contracts": Mock(
                    path=MagicMock(
                        spec=Path, __str__=lambda x: "/path/to/dev1_contract.py"
                    )
                ),
                "scripts": Mock(
                    path=MagicMock(
                        spec=Path, __str__=lambda x: "/path/to/dev1_script.py"
                    )
                ),
            }
            ...
            mock_step_info2.file_components = {
                "specs": Mock(
                    path=MagicMock(spec=Path, __str__=lambda x: "/path/to/dev2_spec.py")
                )
            }
    ...
    def test_workspace_discovery_with_filesystem_fallback(
        self, comprehensive_workspace
    ):
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_filesystem_discovery_with_permission_errors(self, temp_workspace):
        ...
        with patch(
            ...
            with patch(
                ...
                adapter._scan_component_directory(
                    temp_workspace / "restricted", "contracts", "test", inventory
                )
    ...
    def test_step_name_extraction_edge_cases(self, temp_workspace):
        ...
        with patch("cursus.step_catalog.adapters.workspace_discovery.StepCatalog"):
            ...
            assert (
                adapter._extract_step_name_from_file(
                    "test_step_with_underscores_contract.py", "contracts"
                )
                == "test_step_with_underscores"
            )

            # Test with numbers
            assert (
                adapter._extract_step_name_from_file("step123_contract.py", "contracts")
                == "step123"
            )
...
class TestPerformanceAndScalability:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i in range(10):
                ...
                for component_type in ["contracts", "specs", "builders"]:
                    ...
                    for j in range(5):
                        if component_type == "contracts":
                            (component_dir / f"step{j}_contract.py").write_text(
                                f"# Contract {j}"
                            )
                        elif component_type == "specs":
                            (component_dir / f"step{j}_spec.py").write_text(
                                f"# Spec {j}"
                            )
                        elif component_type == "builders":
                            (component_dir / f"builder_step{j}_step.py").write_text(
                                f"# Builder {j}"
                            )
    ...
    def test_large_workspace_discovery_performance(self, large_workspace):
        ...
        with patch(
            ...
            mock_catalog = Mock()
            mock_catalog.workspace_dirs = [
                large_workspace / f"dev{i}" for i in range(10)
            ]
    ...
    def test_component_discovery_with_many_files(self, large_workspace):
        ...
        with patch(
            ...
            assert (
                result["metadata"]["total_components"] >= 15
            )  # 5 files × 3 component types
