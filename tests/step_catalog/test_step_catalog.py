...
class TestUS1QueryByStepName:
    ...
    def test_get_step_info_metrics_update(self, catalog_with_mock_index):
        ...
        assert (
            catalog_with_mock_index.metrics["avg_response_time"] >= 0
        )  # May be 0 for very fast operations
...
class TestUS3MultiWorkspaceDiscovery:
    ...
    def catalog_with_workspaces(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            catalog._step_index = {
                ...
                "workspace_step": StepInfo(
                    step_name="workspace_step", workspace_id="project_alpha"
                ),
                "training_step": StepInfo(
                    step_name="training_step", workspace_id="core"
                ),
                "validation_step": StepInfo(
                    step_name="validation_step", workspace_id="core"
                ),
            }
    ...
    def test_list_available_steps_all(self, catalog_with_workspaces):
        ...
        with patch.object(
            catalog_with_workspaces, "_deduplicate_and_filter_concrete_steps"
        ) as mock_dedupe:
    ...
    def test_list_available_steps_by_workspace(self, catalog_with_workspaces):
        ...
        with patch.object(
            ...
            mock_dedupe.side_effect = lambda steps: (
                steps
            )  # Return steps without filtering

            core_steps = catalog_with_workspaces.list_available_steps(
                workspace_id="core"
            )
            workspace_steps = catalog_with_workspaces.list_available_steps(
    ...
    def test_list_available_steps_by_job_type(self, catalog_with_workspaces):
        ...
        with patch.object(
            ...
            training_steps = catalog_with_workspaces.list_available_steps(
                job_type="training"
            )
...
class TestUS4EfficientScaling:
    ...
    def test_search_steps_with_job_type_filter(self, catalog_with_search_data):
        ...
        catalog_with_search_data._step_index["preprocessing_training"] = StepInfo(
            step_name="preprocessing_training", workspace_id="core"
        )

        results = catalog_with_search_data.search_steps(
            "preprocessing", job_type="training"
        )
...
class TestUS5ConfigAutoDiscovery:
    ...
    def test_discover_config_classes(self, catalog_with_config_discovery):
        ...
        catalog_with_config_discovery.config_discovery.discover_config_classes.assert_called_once_with(
            "test_project"
        )
    ...
    def test_build_complete_config_classes(self, catalog_with_config_discovery):
        ...
        result = catalog_with_config_discovery.build_complete_config_classes(
            "test_project"
        )

        catalog_with_config_discovery.config_discovery.build_complete_config_classes.assert_called_once_with(
            "test_project"
        )
        assert result == mock_result
...
class TestNewDiscoveryMethods:
    ...
    def test_list_steps_with_scripts_basic(self, catalog_with_script_components):
        ...
        with patch.object(
            catalog_with_script_components, "list_available_steps"
        ) as mock_list:
    ...
    def test_list_steps_with_scripts_with_workspace_filter(
        self, catalog_with_script_components
    ):
    ...
    def test_list_steps_with_scripts_with_job_type_filter(
        self, catalog_with_script_components
    ):
    ...
    def test_list_steps_with_scripts_error_handling(
        self, catalog_with_script_components
    ):
    ...
    def test_list_steps_with_scripts_sorted_output(
        self, catalog_with_script_components
    ):
...
class TestDeduplicationFunctionality:
    ...
    def test_deduplicate_and_filter_concrete_steps(self, catalog_with_deduplication):
        ...
        with patch(
            "cursus.registry.step_names.get_step_names", return_value=mock_registry
        ):
            ...
            result = catalog_with_deduplication._deduplicate_and_filter_concrete_steps(
                input_steps
            )
    ...
    def test_resolve_to_canonical_name(self, catalog_with_deduplication):
        ...
        result = catalog._resolve_to_canonical_name(
            "currency_conversion", mock_registry
        )
...
class TestAdditionalUtilityMethods:
    ...
    def catalog_with_variants(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            catalog._step_index = {
                ...
                "data_loading_training": StepInfo(
                    step_name="data_loading_training", workspace_id="core"
                ),
                "data_loading_validation": StepInfo(
                    step_name="data_loading_validation", workspace_id="core"
                ),
                "data_loading_testing": StepInfo(
                    step_name="data_loading_testing", workspace_id="core"
                ),
    ...
    def test_get_metrics_report(self, catalog_with_variants):
        ...
        catalog_with_variants.metrics["last_index_build"] = datetime(
            2023, 1, 1, 12, 0, 0
        )

        # Add workspace to _workspace_steps to match expected count
        catalog_with_variants._workspace_steps = {
            "core": ["data_loading", "other_step"]
        }
...
class TestIndexBuilding:
    ...
    def test_build_index_with_registry(self, catalog_for_indexing):
        ...
        with patch(
            "cursus.registry.step_names.get_step_names", return_value=mock_step_names
        ):
            ...
            assert (
                catalog._step_index["test_step"].registry_data
                == mock_step_names["test_step"]
            )
    ...
    def test_build_index_registry_import_error(self, catalog_for_indexing):
        ...
        with patch(
            "cursus.registry.step_names.STEP_NAMES",
    ...
    def test_extract_step_name_script(self, catalog_for_indexing):
        ...
        assert (
            catalog._extract_step_name("data_preprocessing.py", "script")
            == "data_preprocessing"
        )
        assert (
            catalog._extract_step_name("model_training.py", "script")
            == "model_training"
        )
    ...
    def test_extract_step_name_contract(self, catalog_for_indexing):
        ...
        assert (
            catalog._extract_step_name("data_preprocessing_contract.py", "contract")
            == "data_preprocessing"
        )
        assert (
            catalog._extract_step_name("model_training_contract.py", "contract")
            == "model_training"
        )
    ...
    def test_extract_step_name_spec(self, catalog_for_indexing):
        ...
        assert (
            catalog._extract_step_name("data_preprocessing_spec.py", "spec")
            == "data_preprocessing"
        )
        assert (
            catalog._extract_step_name("model_training_spec.py", "spec")
            == "model_training"
        )
    ...
    def test_extract_step_name_builder(self, catalog_for_indexing):
        ...
        assert (
            catalog._extract_step_name("builder_data_preprocessing_step.py", "builder")
            == "data_preprocessing"
        )
        assert (
            catalog._extract_step_name("builder_model_training_step.py", "builder")
            == "model_training"
        )
    ...
    def test_extract_step_name_config(self, catalog_for_indexing):
        ...
        assert (
            catalog._extract_step_name("config_data_preprocessing_step.py", "config")
            == "data_preprocessing"
        )
        assert (
            catalog._extract_step_name("config_model_training_step.py", "config")
            == "model_training"
        )
...
class TestErrorHandlingAndResilience:
    ...
    def test_graceful_degradation_on_index_build_failure(
        self, catalog_with_error_conditions
    ):
    ...
    def test_metrics_update_on_error(self, catalog_with_error_conditions):
        ...
        with patch.object(
            catalog, "_ensure_index_built", side_effect=Exception("Test error")
        ):
...
class TestLegacyAliasesSupport:
    ...
    def catalog_with_aliases(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            catalog._step_index = {
                "BatchTransform": StepInfo(
                    step_name="BatchTransform", workspace_id="core"
                ),
                "XGBoostTraining": StepInfo(
                    step_name="XGBoostTraining", workspace_id="core"
                ),
    ...
    def test_complete_workflow_simulation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                StepCatalog,
                "_find_package_root",
                return_value=workspace_root / "src" / "cursus",
            ):
                catalog = StepCatalog(workspace_dirs=workspace_root)

                # Mock registry to avoid import issues
                mock_registry = {
                    "data_preprocessing": {"config_class": "DataPreprocessingConfig"}
                }
    ...
    def test_multi_workspace_realistic_scenario(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                ...
                with patch(
                    "cursus.registry.step_names.get_step_names", return_value={}
                ):
                    # Force index build
                    catalog._build_index()
                ...
                with patch.object(
                    catalog, "_deduplicate_and_filter_concrete_steps"
                ) as mock_dedupe:
                    mock_dedupe.side_effect = lambda steps: (
                        steps
                    )  # Return steps without filtering
                    ...
                    workspace_steps = catalog.list_available_steps(
                        workspace_id="steps"
                    )  # Workspace ID is "steps" (directory name)
