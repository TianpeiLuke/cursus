...
def clear_catalog_cache():
    """Clear the get_catalog() lru_cache singleton around every test.

    The CLI now resolves its StepCatalog through a process-wide
    ``cursus.cli._shared.get_catalog`` lru_cache singleton. Clearing it before and
    after each test prevents a real (or mocked) catalog from leaking between tests.
    """
    if CLI_AVAILABLE:
...
def mock_step_catalog():
    ...
    mock_step_info.file_components = {
        "script": Mock(
            path="/path/to/script.py", file_type="script", modified_time=None
        ),
        "config": Mock(
            path="/path/to/config.py", file_type="config", modified_time=None
        ),
    }
...
class TestCatalogCLIBasics:
    ...
    def test_catalog_cli_no_args(self, runner):
        ...
        assert (
            ...
            or "Step catalog management commands" in result.output
        )
...
class TestListStepsCommand:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_list_steps_with_workspace_filter(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
        ...
        mock_step_catalog.list_available_steps.assert_called_with(
            workspace_id="my_workspace", job_type=None
        )

    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_list_steps_with_framework_filter(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
        """Test list command with framework filter."""
        mock_catalog_class.return_value = mock_step_catalog
        mock_step_catalog.detect_framework.side_effect = lambda x: (
            "xgboost" if "XGBoost" in x else None
        )
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_list_steps_json_format(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
...
class TestSearchStepsCommand:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_search_steps_json_format(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_search_steps_with_job_type(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
        ...
        result = runner.invoke(
            catalog_cli, ["search", "training", "--job-type", "validation"]
        )

        assert result.exit_code == 0
        mock_step_catalog.search_steps.assert_called_with(
            "training", job_type="validation"
        )
...
class TestShowStepCommand:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_show_step_with_variants(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
...
class TestComponentsCommand:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_show_components_with_type_filter(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
        ...
        result = runner.invoke(
            catalog_cli, ["components", "TestStep", "--type", "script"]
        )
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_show_components_json_format(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
        ...
        result = runner.invoke(
            catalog_cli, ["components", "TestStep", "--format", "json"]
        )
...
class TestFrameworksCommand:
    ...
    def test_list_frameworks_basic(self, mock_catalog_class, mock_step_catalog, runner):
        ...
        mock_catalog_class.return_value = mock_step_catalog
        mock_step_catalog.detect_framework.side_effect = lambda x: (
            "xgboost" if "XGBoost" in x else "pytorch" if "PyTorch" in x else None
        )
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_list_frameworks_json_format(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
...
class TestWorkspacesCommand:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_list_workspaces_json_format(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
...
class TestMetricsCommand:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_show_metrics_json_format(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
...
class TestDiscoverCommand:
    ...
    def test_discover_workspace_basic(
        self, mock_catalog_class, mock_step_catalog, runner, temp_workspace
    ):
        ...
        result = runner.invoke(
            catalog_cli, ["discover", "--workspace-dir", temp_workspace]
        )
    ...
    @patch("cursus.step_catalog.StepCatalog")
    def test_discover_workspace_json_format(
        self, mock_catalog_class, mock_step_catalog, runner, temp_workspace
    ):
        ...
        result = runner.invoke(
            catalog_cli,
            ["discover", "--workspace-dir", temp_workspace, "--format", "json"],
        )
...
class TestCLIIntegrationScenarios:
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_step_discovery_workflow(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_framework_analysis_workflow(
        self, mock_catalog_class, mock_step_catalog, runner
    ):
    ...
    @patch("cursus.cli.catalog_cli.get_catalog")
    def test_workspace_management_workflow(
        self, mock_catalog_class, mock_step_catalog, runner, temp_workspace
    ):
