...
class TestRegistryCLIBasics:
    ...
    def test_registry_cli_no_args(self, runner):
        ...
        assert (
            "Usage:" in result.output or "Registry management commands" in result.output
        )
...
class TestInitWorkspaceCommand:
    ...
    def test_init_workspace_invalid_id(self, runner):
        ...
        # Invalid workspace ID is an error path: the command now exits nonzero
        # (raise SystemExit(1)) instead of returning with exit code 0.
        assert result.exit_code != 0
    ...
    def test_init_workspace_existing_without_force(self, runner, temp_workspace):
        ...
        # Creating an existing workspace without --force is an error path: the
        # command now exits nonzero (raise SystemExit(1)) instead of returning 0.
...
class TestListStepsCommand:
    ...
    def test_list_steps_with_workspace(self, mock_get_context, mock_get_steps, runner):
        ...
        result = runner.invoke(
            registry_cli, ["list-steps", "--workspace", "test_workspace"]
        )

        assert result.exit_code == 0
        assert (
            "📂 Available Steps (workspace: test_workspace) (3 total):" in result.output
        )
...
class TestValidateRegistryCommand:
    ...
    @patch("cursus.registry.get_all_step_names")
    @patch("cursus.registry.get_workspace_context")
    def test_validate_registry_with_workspace(
        self, mock_get_context, mock_get_steps, runner
    ):
        ...
        result = runner.invoke(
            registry_cli, ["validate-registry", "--workspace", "test_workspace"]
        )
    ...
    @patch("cursus.registry.get_all_step_names")
    @patch("cursus.registry.get_workspace_context")
    def test_validate_registry_with_conflicts(
        self, mock_get_context, mock_get_steps, mock_manager_class, runner
    ):
        ...
        mock_manager_class.return_value = mock_manager
        mock_manager.get_step_conflicts.return_value = {
            "ConflictStep": [Mock(), Mock()]
        }
        mock_manager.get_registry_status.return_value = {"core": {"step_count": 2}}
...
class TestResolveStepCommand:
    ...
    def test_resolve_step_success(self, mock_manager_class, mock_get_context, runner):
        ...
        result = runner.invoke(
            registry_cli, ["resolve-step", "TestStep", "--workspace", "test_workspace"]
        )
    ...
    @patch("cursus.registry.get_workspace_context")
    def test_resolve_step_fallback(
        self, mock_get_context, mock_get_builder, mock_get_config, runner
    ):
        ...
        with patch(
            "cursus.registry.hybrid.manager.UnifiedRegistryManager",
...
class TestValidationStatusCommand:
    ...
    def test_validation_status_basic(self, runner):
        ...
        assert result.exit_code == 0
        assert (
            "Validation System Status" in result.output
        ...
        assert (
            "Performance Metrics:" in result.output
        )  # Remove emoji to avoid encoding issues
...
class TestResetValidationMetricsCommand:
    ...
    def test_reset_validation_metrics_confirmed(self, runner):
        ...
        result = runner.invoke(registry_cli, ["reset-validation-metrics"], input="y\n")
    ...
    def test_reset_validation_metrics_cancelled(self, runner):
        ...
        result = runner.invoke(registry_cli, ["reset-validation-metrics"], input="n\n")
...
class TestCLIIntegrationScenarios:
    ...
    def test_workspace_creation_workflow(self, runner, temp_workspace):
        ...
        assert workspace_dir.exists()
        assert (
            workspace_dir / "src" / "cursus_dev" / "registry" / "workspace_registry.py"
        ).exists()
        assert (workspace_dir / "README.md").exists()
    ...
    @patch("cursus.registry.get_all_step_names")
    @patch("cursus.registry.get_workspace_context")
    def test_registry_validation_workflow(
        self, mock_get_context, mock_get_steps, runner
    ):
