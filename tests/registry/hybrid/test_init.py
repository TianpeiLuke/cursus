...
class TestHybridRegistryImports(unittest.TestCase):
    ...
    def test_utility_function_imports(self):
        ...
        self.assertEqual(
            format_step_not_found_error.__name__, "format_step_not_found_error"
        )
        self.assertEqual(
            format_registry_load_error.__name__, "format_registry_load_error"
        )
    ...
    def test_star_import(self):
        ...
        star_imports = {
            name: getattr(hybrid_module, name) for name in hybrid_module.__all__
        }
...
class TestModuleStructure(unittest.TestCase):
    ...
    def test_module_categories(self):
        ...
        for model_name in data_models:
            ...
            self.assertTrue(
                hasattr(model_class, "__bases__"), f"{model_name} should be a class"
            )
        ...
        for class_name in registry_classes:
            ...
            self.assertTrue(
                hasattr(registry_class, "__bases__"), f"{class_name} should be a class"
            )
...
class TestModuleIntegrity(unittest.TestCase):
    ...
    def test_module_completeness(self):
        ...
        has_models = any(
            name in hybrid.__all__ for name in ["StepDefinition", "ResolutionContext"]
        )
        has_manager = any(
            name in hybrid.__all__
            for name in ["UnifiedRegistryManager", "CoreStepRegistry"]
        )
        has_utils = any(
            name in hybrid.__all__
            for name in ["load_registry_module", "from_legacy_format"]
        )
