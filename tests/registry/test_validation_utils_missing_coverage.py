...
class TestGetPerformanceMetrics:
    ...
    def test_get_performance_metrics_after_validations(self):
        ...
        )  # Could be 0.0 for very fast operations
    ...
    def test_get_performance_metrics_target_validation(self):
        ...
        with patch(
            "cursus.registry.validation_utils._validation_stats",
            {
...
class TestGetValidationStatus:
    ...
    def test_get_validation_status_performance_integration(self):
        ...
        assert (
            status["current_performance"]["average_time_ms"]
            == direct_metrics["average_time_ms"]
        )
        assert (
            status["current_performance"]["target_met"] == direct_metrics["target_met"]
        )
        assert (
            status["current_performance"]["total_validations"]
            == direct_metrics["total_validations"]
        )
        assert (
            status["current_performance"]["cache_hit_rate"]
            == direct_metrics["cache_stats"]["hit_rate"]
        )
